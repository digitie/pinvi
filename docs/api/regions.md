...
```http
GET /regions/covering-point?lon=129.118&lat=35.155&boundary_level=legal_dong
```
...
```http
GET /regions/within-radius?lon=129.118&lat=35.155&radius_m=2000&boundary_level=sigungu
```
...
- `covering-point` → `POST /v2/reverse {lon,lat,radius_m,include_region:true}` →
  ...
  후보는 제외.
- `within-radius` → `POST /v2/regions/within-radius {lon,lat,radius_m,boundary_level}` →
  반경 내 행정구역 후보 목록을 pass-through한다(`docs/architecture/geocoding-open-decisions.md`
  D5 결정됨).
