...
- **저장**: `app.mcp_tokens` (Alembic migration Sprint 6) — `user_id`,
  `token_hash` (Argon2id), `scopes TEXT[]`, `name TEXT` (사용자가 부여),
  `expires_at`, `last_used_at`, `revoked_at`.
- **발급 UI**: `/users/me/mcp-tokens` (사용자 본인) / `/admin/mcp-tokens`
  (관리자 — 전체 / revoke).
...
1. JWT signature 검증 (`MCP_JWT_SECRET` — 일반 access 토큰과 별 secret)
...
`python-krtour-map` `search()` 위임. 결과는 사용자 권한 / 동의 범위 내에서만.
...
- `connect-src 'self' wss://api.tripmate.kr` (SSE)
