...
- Kakao Local API 검색 adapter는 `docs/data-sources.md`의 캐시/저장 정책과 `docs/api/places.md` 계약을 먼저 확정한 뒤 구현한다.
...
- Kakao 지도 UI와 Local API adapter 동시 구현: 사용자 흐름을 빨리 완성할 수 있지만, provider 저장 정책과 cache key가 흐려질 위험이 있다.
