...
> **상태 (2026-06-12)**: krtour-map의
> `GET /v1/curated-features/{curated_feature_id}/tripmate-copy` 계약을 TripMate가
> 소비한다. 본 절은 해당 copy snapshot 계약과 TripMate 저장 매핑을 기록한다.
...
| 소스 | 의미 |
|------|------|
| TripMate-native 큐레이션 | Admin/운영자가 TripMate 안에서 직접 기획·작성 |
...
| `source_etag` | `app.curated_trip_plans` | krtour copy snapshot etag |
| `source_imported_at` | `app.curated_trip_plans` | 마지막 import 시각 |
| `source_curated_feature_id` | `app.curated_plan_pois` | krtour curated feature 원본 id |
| `source_curated_feature_item_id` | `app.curated_plan_pois` | krtour 하위 item 원본 id |

TripMate는 krtour-map을 HTTP로만 호출하고 Python 패키지 import나 DB 직접 접근을 하지 않는다.
`krtour-ai-agent`는 TripMate curated trip plan 생성 흐름에 관여하지 않는다.

### 7.6 krtour 회신 요청

1. copy snapshot item이 이미지/첨부/media 참조를 포함할 때의 셰입.
2. TripMate가 1:1 import 후 refresh할 때 기존 TripMate plan을 갱신해야 하는지,
