...
- `infra/docker-compose*.yml`에 `observability` profile로 Prometheus `12601`,
  cAdvisor `12602`, Grafana `12605`를 추가했다.
- Grafana datasource/dashboard provisioning과 `/admin/grafana` 기본 URL `12605` 정렬을 완료했다.
...
합의 5건은 kor_travel_map T-217c 회신 대기). 주의: **kor_travel_map admin API base는 12301 `/v1/admin/*`**
이며, `pinvi_kor_travel_map_admin_base_url`도 12301을 기본값으로 둔다.
