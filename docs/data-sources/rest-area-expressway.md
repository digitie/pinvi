...
이 문서는 한국도로공사 OpenAPI 인수인계 기준이다. 관련 구현은 `apps/api/app/etl/rest_area/client.py`, `apps/api/app/etl/rest_area/loader.py`, `dags/rest_area.py`, `docs/architecture/rest-area-schema.md`다.
...
| 인증 파라미터 | `key` |
| 응답 타입 | `type=json` |
| 환경변수 | `TRIPMATE_EXPRESSWAY_API_KEY` |
| timeout | 30초 |
| page size | 구현 최대 100 |
| pagination guard | 1000 page |
| 응답 row | root `list` |
| 실패 처리 | root `code`가 있고 `SUCCESS`가 아니면 실패 |

공통 요청 파라미터:

| 이름 | 필수 | 구현값/설명 |
| --- | --- | --- |
| `key` | Y | 한국도로공사 API key |
| `type` | Y | `json` |
| `numOfRows` | Y | 최대 100 |
| `pageNo` | Y | 1부터 자동 증가 |
