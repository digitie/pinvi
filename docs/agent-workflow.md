...
> 되는가"에 답한다. 사양이 아니라 런북이다. `kor-travel-geo`의
...
- 에이전트별 worktree는 **고정 이름**이다(ADR-017): Claude=`pinvi-claude`,
  Codex=`pinvi-codex`, Antigravity=`pinvi-antigravity`. idle branch는
  `agent/<agent>-idle`.
...
WSL에서 `/mnt/f/dev/pinvi-<agent>`에 `git`을 실행하면 포인터가 환경별 절대경로로
...
```
편집/branch/commit/push/PR  ── NTFS worktree (F:/dev/pinvi-<agent>, Windows git.exe)
        │   (변경을 미러로 NTFS→ext4 단방향 rsync — 미러에서 같은 파일을 직접 편집하지 말 것)
        ▼
검증                         ── WSL ext4 미러 (~/pinvi-workspaces/pinvi-<agent>)
  backend:  cd apps/api && . .venv-wsl/bin/activate
            export PINVI_JWT_SECRET_KEY=... TESTCONTAINERS_RYUK_DISABLED=true TMPDIR=/tmp TMP=/tmp TEMP=/tmp
...
- WSL에서 worktree git 작업이 꼭 필요하면 Windows git을 호출:
  `"/mnt/c/Program Files/Git/cmd/git.exe" -C F:/dev/pinvi-<agent> status -sb`.
- 포인터가 깨지면 Windows에서 `git -C F:/dev/pinvi worktree repair
  F:/dev/pinvi-<agent>` 한 번으로 복구. `prune`은 **Windows에서만**.
...
- `kor-travel-geo` `docs/agent-workflow.md` — 동일 구조 reference.
