...
- 장소 후보는 `python-krtour-map`의 `Feature`, `SourceRecord`, `SourceLink` 계약과 TripMate `media_asset` 구조에 연결할 수 있게 한다.
...
- 장소 후보를 `python-krtour-map` feature에 바로 active로 넣지 않고, 원천 row와 후보 매핑 상태를 먼저 남긴다.
...
| tool | 설명 |
| --- | --- |
| `find_features` | 이름, 주소, 좌표 주변 기존 feature 조회 |
| `upsert_source_record` | YouTube 영상/분석 원천 record 저장 |
| `upsert_feature_candidate` | 장소 후보 저장 또는 갱신 |
| `link_candidate_to_feature` | 검수 후 기존 feature에 연결 |
| `create_draft_feature` | 검수 대상 draft feature 생성 |
...
- 장소명/주소/좌표는 Gemini 결과만 믿지 않고 DB MCP에서 기존 주소 DB와 기존 feature로 검증한다.

## DB 누적 설계

TripMate의 최종 지도 feature 기준은 `python-krtour-map`의 `Feature` 계약이다. YouTube는 공공데이터처럼 권위 원천이 아니므로, 바로 `active` feature로 확정하지 않는 것이 안전하다.

추천 저장 흐름:

1. `python-krtour-map` `SourceRecord`에 영상 원천 record 저장
2. 별도 YouTube 분석 실행 테이블 또는 source metadata에 분석 실행 metadata 저장
3. 장소 후보는 후속 `youtube_place_candidates` 또는 `python-krtour-map` 후보 계약에 `pending`으로 저장
4. 기존 지도 객체와 confidence가 충분히 높으면 연결 후보를 만든다.
5. 사람이 승인하거나 자동 승인 기준을 통과하면 `python-krtour-map` feature draft 또는 active로 승격한다.
6. 대표 프레임은 `media_asset`에 저장하고, 승인된 feature는 feature media 연결 계약으로 연결한다.
...
3. 기존 `python-krtour-map` feature와 이름, 주소, 좌표가 높은 신뢰도로 일치하는 경우 연결 후보 생성
...
2. Gemini 추출 장소를 `python-krtour-map` feature draft로 바로 만들지, 별도 `youtube_place_candidates`에 먼저 둘지.
