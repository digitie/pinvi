...
ADR-015 이후 Pinvi 지도 클라이언트는 VWorld + MapLibre 계열이고, ADR-046 이후 Web
패키지는 `vworld-map-web`이다. 본 절은 SPEC V8 원문 대비 superseded 주의사항으로만
남긴다. Kakao Maps SDK나 Kakao Local 검색 API를 현재 구현에 쓰지 않는다.
