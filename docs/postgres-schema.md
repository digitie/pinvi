...
| `app.ktm_cache_target_events`            | Map at-least-once 결과의 immutable inbox                        | `event_id` PK, state-applied의 `source_event_id` generated UUID, target event tuple partial unique, `cache_target.reconciled`는 네 target tuple이 모두 NULL인 stream receipt, 모든 event의 source/payload fingerprint는 32-byte 필수, exact `event_type` 4종, relay order 양수           |
...
| `app.ktm_cache_target_reconciliation_expectations` | request-bound fixed snapshot과 terminal receipt 결박 | request ID PK, snapshot ID 유일, epoch/count/root/high-watermark 고정, pending/received/invalidated 상태와 receipt event·종결 시각 조건부 CHECK |
| `app.ktm_cache_target_canary_runs`       | production causal canary의 실행별 crash-safe 감사 정본          | run UUID PK, 고정 synthetic target·consumer FK, running target partial unique, command fingerprint·event source command/source+payload fingerprint·claim item/claim terminal tuple composite FK, phase별 explicit all-or-none, final remote control과 cursor 3종·count·Merkle·backlog 3종 all-or-none/equality CHECK |
| `app.ktm_cache_target_boundary_audits`   | generation 7 forward commit의 append-only Pin final receipt 정본 | transaction UUID PK, cutover unique, initial/final writer fence·Map final evidence·prior receipt·canonical request/evidence SHA-256, initial reconciliation/canary composite FK, expected/unexpected count CHECK, UPDATE/DELETE/TRUNCATE 거부 trigger |
| `app.ktm_cache_target_event_claims`      | pull claim별 mutable lease와 ACK prefix                         | reclaim마다 새 claim/lease token, active lease expiry index, 완료 상태와 완료 시각 결박, canary terminal provenance용 claim/consumer/status/cursor/completed tuple unique                                                                  |
| `app.ktm_cache_target_event_claim_items` | claim-event별 delivery/ACK receipt                              | 같은 immutable event의 여러 reclaim 허용, claim 안 cursor/position 유일, payload fingerprint 32바이트, 미ACK gap index, canary용 claim/event/cursor/fingerprint/acked tuple unique                                                         |
...
### 7.3 `app.content_reports` / `app.content_moderation_actions`
...
CREATE TABLE app.content_reports (
  ...
  target_trip_id       uuid REFERENCES app.trips(trip_id) ON DELETE SET NULL,
  target_owner_user_id uuid REFERENCES app.users(user_id) ON DELETE SET NULL,
  reporter_user_id     uuid REFERENCES app.users(user_id) ON DELETE SET NULL,
  reason_code          varchar(32) NOT NULL,
  reason_text          text NOT NULL,
  status               varchar(32) NOT NULL DEFAULT 'received',
  target_snapshot      jsonb NOT NULL DEFAULT '{}'::jsonb,
  evidence             jsonb NOT NULL DEFAULT '{}'::jsonb,
  reviewer_user_id     uuid REFERENCES app.users(user_id) ON DELETE SET NULL,
  ...
  created_at           timestamptz NOT NULL DEFAULT now(),
  updated_at           timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT ck_content_reports_target_type_allowed CHECK (
    target_type IN ('trip', 'comment', 'attachment', 'share_link')
  ),
  CONSTRAINT ck_content_reports_reason_code_allowed CHECK (
    reason_code IN ('spam', 'harassment', 'privacy', 'illegal', 'safety', 'other')
  ),
  CONSTRAINT ck_content_reports_status_allowed CHECK (
    status IN ('received', 'reviewing', 'hidden', 'taken_down', 'rejected', 'appealed', 'restored')
  )
);

CREATE INDEX ix_content_reports_status_created
  ON app.content_reports (status, created_at DESC);
CREATE INDEX ix_content_reports_target
  ON app.content_reports (target_type, target_id);
CREATE INDEX ix_content_reports_reporter_created
  ON app.content_reports (reporter_user_id, created_at DESC)
  WHERE reporter_user_id IS NOT NULL;
CREATE INDEX ix_content_reports_owner_created
  ON app.content_reports (target_owner_user_id, created_at DESC)
  WHERE target_owner_user_id IS NOT NULL;
CREATE INDEX ix_content_reports_trip_created
  ON app.content_reports (target_trip_id, created_at DESC)
  WHERE target_trip_id IS NOT NULL;
CREATE INDEX ix_content_reports_open
  ON app.content_reports (created_at)
  WHERE status IN ('received', 'reviewing', 'appealed');
...
CREATE TABLE app.content_moderation_actions (
  ...
  report_id     uuid NOT NULL REFERENCES app.content_reports(report_id) ON DELETE CASCADE,
  actor_user_id uuid REFERENCES app.users(user_id) ON DELETE SET NULL,
  action        varchar(32) NOT NULL,
  action_reason text NOT NULL,
  before_state  jsonb NOT NULL DEFAULT '{}'::jsonb,
  after_state   jsonb NOT NULL DEFAULT '{}'::jsonb,
  created_at    timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT ck_content_moderation_actions_action_allowed CHECK (
    action IN ('review', 'hide', 'takedown', 'restore', 'reject', 'appeal')
  )
);

CREATE INDEX ix_content_moderation_actions_report_created
  ON app.content_moderation_actions (report_id, created_at DESC);
CREATE INDEX ix_content_moderation_actions_actor_created
  ON app.content_moderation_actions (actor_user_id, created_at DESC)
  WHERE actor_user_id IS NOT NULL;
```

콘텐츠 신고 접수와 moderation 조치 이력을 저장한다. `/users/me/content-reports`는 신고 접수와
사용자 appeal을 만들고, `/admin/moderation`은 숨김/게시중단/복구/기각을 수행한다.
실제 대상 row는 기존 soft state를 재사용한다: `trips.visibility/status/deleted_at`,
`trip_comments.deleted_at`, `curated_plan_attachments.deleted_at`, `trip_share_links.revoked_at`.

### 7.4 `app.retention_runs` / `app.location_access_log_archive`
...
- `app.users.status IN ('pending_delete', 'deleted')`이고 `deleted_at`이 retention cutoff를 지났을 때
  PII 마스킹 잡 schedule (Dagster):
  - `email` → `deleted+<user_id>@deleted.pinvi.local`
  - `nickname` → `null`
  - `avatar_url` → `null`
