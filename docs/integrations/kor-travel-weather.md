...
| 보존             | 3년 immutable                                | 현재 **기본 2일** → 연장 요청(G-3, §4.2)                        |
...
- **보존 기본 2일**(`KOR_TRAVEL_WEATHER_RETENTION_DAYS=2`, `known_at` day partition
  통째 drop). 3년 immutable인 현재 소스와 근본적으로 다르다 → §4.2에서 **연장 요청(C)**
  으로 결정했고 게이트 G-3으로 건다.
...
**사용자 결정 = C.** Pinvi는 `app.trip_day_weather_snapshots` 같은 별도 스냅샷
테이블을 **만들지 않는다.** 보존은 사실을 소유한 서비스가 진다.

근거가 되는 정황: `kor-travel-weather`는 이미 **3년 보존을 목표로 선언**해 뒀다(그쪽
ADR-062). 배포 기본값이 2일일 뿐이므로 C는 새 요구가 아니라 **선언된 목표를 실제
배포에 반영해 달라는 요청**이다. 저장 주체와 보존 책임이 한 곳에 모여 구조도 더 깨끗하다.

> **게이트 G-3**: `kor-travel-weather`의 보존 지평이 Pinvi 여행 기록에 필요한 범위를
> 덮기 전에는 과거 날짜 날씨를 "지원한다"고 말하지 않는다. 해당 저장소 소관이다.
...
2. **외부 설정에 종속된다.** 보존이 되돌아가거나 스토리지 압박으로 줄면 과거 여행
   날씨가 **조용히** 사라진다. → Pinvi 측에 **보존 지평 가드**를 둔다(임계 미만이면
   실패하는 점검, §6-I).
...
- **I. 보존 지평 가드(G-3)** — `kor-travel-weather`의 실효 보존이 Pinvi 여행 기록에
  필요한 범위 미만이면 **실패**한다. 외부 설정 회귀로 과거 여행 날씨가 조용히 사라지는
  것을 막는 유일한 장치다(§4.2 대가 2).
