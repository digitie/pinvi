...
서버가 좌표 범위를 검증하고 canonical `lon`/`lat` payload로 broadcast한다. client message rate 초과
connection은 `RATE_LIMITED` error 전송 후 close grace 동안 broker slot을 유지하고, 실제 close/finally
정리 시점에 slot을 반환한다. 닫히는 중인 socket을 cap에서 빼면 connect→spam→reconnect 반복으로
process FD/메모리 cap을 우회할 수 있기 때문이다.

## 2.2 관측성

Trip WebSocket 계층은 `apps/api/app/services/realtime_metrics.py`에서 Prometheus metric을
등록한다. label은 `trip_id`/`user_id`를 넣지 않고 bounded 값만 사용한다.

| metric                             | label                          | 의미                                                |
| ---------------------------------- | ------------------------------ | --------------------------------------------------- |
| `pinvi_api_ws_active_connections`  | `channel`                      | 현재 process-local broker에 등록된 connection gauge |
| `pinvi_api_ws_connections_total`   | `channel`, `result`, `reason`  | accept/reject 결정 counter                          |
| `pinvi_api_ws_closes_total`        | `channel`, `code`, `reason`    | WebSocket close counter                             |
| `pinvi_api_ws_messages_total`      | `channel`, `direction`, `type` | client/server message counter                       |
| `pinvi_api_ws_broadcasts_total`    | `channel`, `type`, `result`    | broadcast attempt counter                           |
| `pinvi_api_ws_send_failures_total` | `channel`, `reason`            | send timeout/error counter                          |

close event는 `pinvi.websocket.close` 구조화 로그로도 남긴다. log에는 bounded
`code`/`reason`과 디버깅용 `trip_id`/`user_id`를 포함하며, Prometheus label에는 고카디널리티
식별자를 넣지 않는다.
