...
| 환경변수 | 비고 |
|----------|------|
| `TRIPMATE_SENTRY_DSN` | 백엔드 / Dagster |
| `NEXT_PUBLIC_SENTRY_DSN` | 프론트 (빌드 타임 embed) |
| `TRIPMATE_SENTRY_ENVIRONMENT` | `production` / `staging` / `development` |
| `TRIPMATE_SENTRY_RELEASE` | git short sha (CI 주입) |
| `TRIPMATE_SENTRY_TRACES_SAMPLE_RATE` | `0.1` (10%, Odroid 부하 고려) |
| `TRIPMATE_SENTRY_PROFILES_SAMPLE_RATE` | `0.0` (ARM 프로파일링 제한) |
...
def init_sentry():
    if not settings.tripmate_sentry_dsn:
        return
    sentry_sdk.init(
        dsn=settings.tripmate_sentry_dsn,
        environment=settings.tripmate_sentry_environment,
        release=settings.tripmate_sentry_release,
        traces_sample_rate=settings.tripmate_sentry_traces_sample_rate,
        profiles_sample_rate=settings.tripmate_sentry_profiles_sample_rate,
...
```python
# apps/etl/tripmate/etl/sentry_hook.py
...
Sentry.init({
  ...
  environment: process.env.NEXT_PUBLIC_TRIPMATE_ENV,
  release: process.env.NEXT_PUBLIC_TRIPMATE_RELEASE,
...
Telegram 봇 token은 `TRIPMATE_TELEGRAM_BOT_TOKEN_DEFAULT` (위 [`telegram.md`](./telegram.md)).
...
- [ ] `apps/etl/tripmate/etl/sentry_hook.py` (Dagster `run_failure_sensor`)
