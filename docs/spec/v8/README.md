# SPEC V8 — TripMate 적용 노트

본 디렉토리는 외부에서 제공된 **"여행 계획 서비스 SW 개발 명세서 V8"** 6부작을
TripMate v2 컨텍스트로 재구성한 적용 노트다. 원본 docx는 운영자가 보관하고,
...
## SPEC V8 ↔ TripMate v2 ↔ python-krtour-map 책임 매핑

SPEC V8은 v1 시점의 단일 모노레포 가정으로 작성되었다. v2에서는 지도 feature
도메인을 별 저장소 `python-krtour-map`이 소유한다 (ADR-001 / ADR-002 / ADR-003).
...
### `python-krtour-map` 책임 (별 저장소)

- 7 Feature 정규화 / `feature_id` 생성 (D-1 ~ D-13)
- `feature` / `provider_sync` schema DDL (E-3, E-5)
- Record Linkage (D-14, K-4)
- vworld 법정동코드 임포트 본체 (L장 — TripMate는 trigger UI만)
- provider 원천 → DTO 변환 (B장 + Q/R 후속 메모)
- WeatherValue / PriceValue / Notice / Area 시계열 (E-3, E-5)
- krtour-map API/Admin `krtour-map-admin` (별도 패키지, API `9011` / admin `9012`)

### 공유 / 양쪽 모두 참조

- `app` schema(TripMate) ↔ `feature` schema(라이브러리) 사이의 `feature_id` 참조
- `location_access_log` audit chain (O-3) — 호출 측은 TripMate API, schema는
  `app.location_access_log` (TripMate 소유)
- `admin_audit_log` (O-6) — 호출 측은 TripMate Admin, schema는 `app.admin_audit_log`
...
- 원본 SPEC V8은 한국어로 작성된 단일 vendor 문서다. 본 디렉토리는 그 의도를
  TripMate v2 작업 기준으로 정리한 derivative다. 원본과 본 문서가 충돌하면
  사용자 검토 후 `docs/decisions.md`에 결정을 박는다.
- `python-krtour-map` 저장소의 SPEC V8 적용은 그쪽 저장소가 별도로 관리한다.
