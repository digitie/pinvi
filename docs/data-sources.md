...
| address_code | `juso_address_code_standard` | 법정동코드/도로명코드/행정동코드/도로명주소관리번호 기준 | 확정 |
...
- 데이터셋: `법정구역정보`
- 형식: SHP
- 기준 페이지: `https://www.vworld.kr/dtmk/dtmk_ntads_s002.do`
...
- 원본 SHP 좌표계: EPSG:5186 (GRS80)
- raw 레이어: EPSG:5186 원본 geometry 보존
- serving 레이어: EPSG:4326 변환본 생성
- 웹 지도 출력과 API 응답 좌표는 EPSG:4326 사용

저장:

- `region_raw_boundary`: 원본 비교와 재처리를 위한 EPSG:5186 SHP 적재본
- `region_serving_boundary`: 지도 표시와 앱 조회를 위한 EPSG:4326 변환본
- `region_raw_legal_dong_boundary`: 법정동 단위 원본 EPSG:5186 SHP 적재본
- `region_serving_legal_dong_boundary`: 법정동 단위 EPSG:4326 변환본

공간 연산:

- 행정구역 point-in-polygon 판정은 PostGIS에서 수행한다.
- 법정동 단위 point-in-polygon 판정이 필요한 경우 `region_serving_legal_dong_boundary`를 사용한다.
- 장소 좌표는 API/웹 입력 기준 EPSG:4326으로 받는다.
- raw 원본 검증이나 SHP 갱신 비교는 EPSG:5186 raw 레이어 기준으로 수행한다.
- 서비스 조회는 EPSG:4326 serving 레이어를 우선 사용한다.
- “반경 nkm” 리포트는 정확한 원형 거리 검색이 아니라 행정구역 기반 근사로 유지한다.
