...
- provider별 신뢰 신호:
  ...
  - Naver: 미래 작업. `email_verified` 없음 → Pinvi 자체 verify 메일 발송 필요
...
| `PINVI_NAVER_OAUTH_CLIENT_ID` | 미래 작업. 현재 설정해도 provider 목록에 노출하지 않음 |
| `PINVI_NAVER_OAUTH_CLIENT_SECRET` | 미래 작업 |
| `PINVI_KAKAO_OAUTH_REST_API_KEY` | 미래 작업. 현재 설정해도 provider 목록에 노출하지 않음 |
| `PINVI_KAKAO_OAUTH_CLIENT_SECRET` | 미래 작업 |
...
Docker smoke: `http://127.0.0.1:12801/auth/oauth/google/callback` (추가).
...
| 항목 | 값 |
|------|----|
| API base | `https://pinviapi.digitie.mywire.org` |
| Web origin | `https://pinvi.digitie.mywire.org` |

Google Cloud Console 등록값:

| 설정 | 값 |
|------|----|
| 승인된 JavaScript 원본 | `https://pinvi.digitie.mywire.org` |
| 승인된 리다이렉션 URI | `https://pinviapi.digitie.mywire.org/auth/oauth/google/callback` |
...
- 운영 cookie는 `Secure`, `HttpOnly`, `SameSite=Lax`로 내려간다.
  ...
  배포에서 필수값으로 둔다.
- CORS origin은 웹 origin인 `https://pinvi.digitie.mywire.org`만 허용한다.
  API origin(`https://pinviapi.digitie.mywire.org`)이나 wildcard는 허용하지 않는다.
