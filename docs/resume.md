...
Playwright 1.60.0 `install-deps --dry-run chromium`은 Ubuntu 26.04를 지원하지 않았다. Windows
fallback은 N150 Web SSH tunnel 대상 login malformed validation 1건만 통과했다. Admin live
2000/full은 credential 부재, restore staging drill은 staging DB URL 부재로 미실행이다. 다음
작업은 T-259 차단 해소 후 `v0.2.0` tag/GitHub Release 생성이다.
