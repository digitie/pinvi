...
> 정책은 **`kor-travel-map`이 소유** (ADR-003 / ADR-005). 본 문서는 Pinvi가
...
### 2.1 VWorld 지도 (`vworld-map-web` 경유, ADR-046)

- 지도 클라이언트는 내부 라이브러리 `maplibre-vworld-react`의 `vworld-map-web` 사용
  (Kakao Maps SDK 폐기 — ADR-015, 웹 패키지 전환 — ADR-046)
...
- Pinvi 자체 wrapper 만들지 않음 (ADR-005) — 부족 기능은 라이브러리에 PR
...
## 3. 라이브러리 위임 provider (`kor-travel-map` 소유)
...
- Juso.go.kr (`kor-travel-geo` 경유): 도로명/지번

본 저장소는 kor-travel-map OpenAPI 결과 또는 Pinvi 직접 통합 결과를 사용자에게
...
- [ ] 라이브러리 위임 vs Pinvi 직접 호출 결정 (ADR-005 룰)
