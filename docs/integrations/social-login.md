...
현재 구현/운영 범위는 **Google, Naver, Kakao OAuth**다. `/auth/oauth/providers`는 세 provider를
항상 반환하고, 각 provider의 `enabled`는 필수 설정값 존재 여부로 결정한다.

## 1. 정책

- Pinvi 로그인 식별자는 `users.email` (변경 없음)
...
- provider별 신뢰 신호:
  ...
  - Naver: `email_verified` 없음 → 신규 사용자는 `pending_verification` + Pinvi 자체 verify 메일
...
| 환경변수                           | 비고                                       |
| ---------------------------------- | ------------------------------------------ |
| `PINVI_GOOGLE_OAUTH_CLIENT_ID`     | Google Cloud Console                       |
| `PINVI_GOOGLE_OAUTH_CLIENT_SECRET` |                                            |
| `PINVI_NAVER_OAUTH_CLIENT_ID`      | Naver Developers                           |
| `PINVI_NAVER_OAUTH_CLIENT_SECRET`  |                                            |
| `PINVI_KAKAO_OAUTH_REST_API_KEY`   | Kakao Developers REST API key              |
| `PINVI_KAKAO_OAUTH_CLIENT_SECRET`  | Kakao client secret 사용 설정 시           |
| `PINVI_OAUTH_STATE_TTL_SECONDS`    | `600` (10분)                               |
| `PINVI_OAUTH_HTTP_TIMEOUT_SECONDS` | `5`                                        |
| `PINVI_WEB_BASE_URL`               | redirect base                              |
| `PINVI_OAUTH_CALLBACK_BASE_URL`    | provider 콘솔에 등록한 callback URL의 base |

`GET /auth/oauth/providers`는 Google/Naver/Kakao를 반환한다. Google과 Naver는 client id와
client secret이 모두 있어야 `enabled: true`이고, Kakao는 REST API key가 있으면 `enabled: true`다.
...
| Scope                | (지정 안 함, 기본)                         |
| ID 키                | `response.id`                              |
| 이메일 verified 필드 | **없음** → Pinvi가 별도 verify 발송        |
...
- email_verified 필드 없음 → Pinvi 자체 verify 메일 발송 후 그 link 클릭으로 활성화
- 신규 user는 일단 `status='pending_verification'`으로 생성
- verify 후 `status='pending_profile'`
...
사용자 로그인 상태에서 /profile에서 [Google 연결] 클릭
  ↓
POST /auth/oauth/{provider}/link { return_to: '/profile' }
  ↓ 응답에 authorize_url
...
... (above flow with mode=link, user_id 포함된 state) ...
  ...
  - Google 이메일이 다른 Pinvi 계정 이메일과 일치 → `OAUTH_ACCOUNT_LINK_REQUIRED` (충돌)
...
callback 실패 시 `/login?error=<code>&error_description=<msg>&provider=<provider>` 으로 303:
`mode=link` state를 정상 소비한 뒤 발생한 연결 충돌/검증 실패는
`return_to`(기본 `/profile`)에 같은 query로 303 redirect한다.

| code                          | 의미                                                                                         |
| ----------------------------- | -------------------------------------------------------------------------------------------- |
| `OAUTH_ACCOUNT_LINK_REQUIRED` | 같은 이메일/provider identity가 기존 계정과 충돌. 이메일 로그인 후 프로필에서 명시 연결 필요 |
| `OAUTH_CALLBACK_INVALID`      | callback 필수 query 누락                                                                     |
| `OAUTH_EMAIL_UNVERIFIED`      | Google 또는 Pinvi 이메일 인증 미완료                                                         |
...
| Provider | 콘솔에 등록                                         |
| -------- | --------------------------------------------------- |
| Google   | `http://localhost:12801/auth/oauth/google/callback` |
| Naver    | `http://localhost:12801/auth/oauth/naver/callback`  |
| Kakao    | `http://localhost:12801/auth/oauth/kakao/callback`  |

Docker smoke는 host만 `127.0.0.1:12801`로 바꾸고 같은 provider path를 등록한다.
...
| 항목       | 값                              |
| ---------- | ------------------------------- |
| API base   | `https://pinvi-api.example.com` |
| Web origin | `https://pinvi.example.com`     |

Google Cloud Console 등록값:

| 설정                   | 값                                                         |
| ---------------------- | ---------------------------------------------------------- |
| 승인된 JavaScript 원본 | `https://pinvi.example.com`                                |
| 승인된 리다이렉션 URI  | `https://pinvi-api.example.com/auth/oauth/google/callback` |

운영 provider 콘솔에는 사용하는 provider별 callback을 모두 등록한다.
...
- OAuth 시작 요청의 `return_to`는 상대 경로 또는 `PINVI_WEB_BASE_URL` 하위 경로만
  허용한다. 외부 URL은 거부해 open redirect를 막는다.
- 운영 cookie는 `Secure`, `HttpOnly`, `SameSite=Lax`로 내려간다.
  `PINVI_ENVIRONMENT=production` 누락 시 `Secure`가 빠질 수 있으므로 운영
  배포에서 필수값으로 둔다.
- CORS origin은 웹 origin인 `https://pinvi.example.com`만 허용한다.
  API origin(`https://pinvi-api.example.com`)이나 wildcard는 허용하지 않는다.
...
- [x] `apps/api/app/services/oauth_google.py` httpx 호출 + provider별 안전 매칭 정책
- [x] `apps/api/app/api/v1/oauth.py` Google/Naver/Kakao providers/start/callback/unlink 라우터
- [x] `apps/web/app/(auth)/login/page.tsx` provider 목록 기반 소셜 버튼
- [x] `apps/web/app/(auth)/profile/page.tsx` Google/Naver/Kakao 연결/해제 UI
- [x] OAuth Naver 신규 가입 → 자체 verify 메일 트리거
