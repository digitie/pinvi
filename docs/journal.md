...
- **삭제**: `apps/api/app/etl_bridge/`(kor_travel_map.py + __init__.py) — 마지막 소비처 정리 완료.
...
**검증**:
- NTFS worktree: `rg -n "draft.*active.*archived|/mcp/tokens|api.pinvi.kr|\bMCP_JWT_SECRET\b" docs/architecture/mcp-server.md docs/architecture/mcp-tools.md docs/runbooks/mcp-server.md docs/decisions.md docs/sprints/SPRINT-6.md`
...
**결정**:
- API production URL: `https://pinviapi.digitie.mywire.org` (내부/host port
  `9021`)
- Web production URL: `https://pinvi.digitie.mywire.org` (내부/host port `9022`)
- Google 승인된 JavaScript 원본: `https://pinvi.digitie.mywire.org`
- Google redirect URI:
  `https://pinviapi.digitie.mywire.org/auth/oauth/google/callback`
