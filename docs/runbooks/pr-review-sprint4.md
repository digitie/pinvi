...
5. TripMate 의존 핀 또는 호출부를 동기한다.
6. 검증을 통과한 뒤 TripMate PR을 머지한다.
...
- TripMate 본 저장소의 모든 새 PR과 `ready_for_review` 전환 PR
- 5분마다 실행되는 `.github/workflows/codex-pr-monitor.yml`의 열린 PR 감시 결과
- Sprint 1~4 목표에 직접 영향을 주는 기반 라이브러리 PR
- 특히 다음 저장소와의 경계:
  - `python-krtour-map`
  - `maplibre-vworld-js`
  - `python-kraddr-base`, `python-kraddr-geo`, `python-kraddr-gop`
...
- 단, Sprint 1~4 목표와 무관한 추상화·대규모 재작성은 별도 ADR 또는 후속 backlog로
  분리한다.
- TripMate가 소유하지 않는 책임은 TripMate wrapper로 숨기지 않는다. 소유 라이브러리를
  수정하고 PR → 머지 → TripMate sync 순서로 처리한다.
...
3. 다음 순서로 찾는다.
   ...
   - 기반 라이브러리에 있어야 할 코드가 TripMate에 들어온 경우
...
- `.github/workflows/codex-pr-monitor.yml`은 5분마다 열린 PR을 조회한다.
- draft PR은 건너뛴다.
- PR 최신 head SHA에 `codex-pr-review:head=<sha>` 마커가 달린 Codex 리뷰 코멘트가
  있으면 건너뛴다.
- 새 PR, `ready_for_review` PR, 또는 새 commit이 올라와 head SHA가 바뀐 PR은 다시
  리뷰한다.
- `workflow_dispatch`로 수동 실행할 수 있다.
- GitHub Actions secret `OPENAI_API_KEY`가 없으면 리뷰 job은 실패한다.
...
TripMate 코드가 다음 일을 하려 한다면 TripMate PR을 멈추고 소유 라이브러리부터 고친다.
...
4. TripMate에서 의존 SHA/version을 갱신한다.
5. TripMate 호출부와 문서를 sync한다.
6. 양쪽 PR 링크를 서로 남긴다.

## 검증 기준

- 백엔드: `pytest`, `ruff`, `mypy`, OpenAPI drift, DB 변경 시 Alembic + EXPLAIN
- 프론트엔드: lint, typecheck, build, Playwright smoke, 주요 viewport screenshot
- ETL: Dagster asset dry-run, provider fixture, idempotency 확인
- 지도/UI: `maplibre-vworld-js` 책임인지 TripMate 책임인지 먼저 확인하고, 브라우저에서
  실제 rendering을 확인한다.
- 모든 명령은 WSL ext4 미러에서 실행하고, NTFS 작업본과 sync 상태를 보고한다.

## 머지 기준

- 차단 리뷰 코멘트가 모두 해결됨
- 필요한 기반 라이브러리 PR이 머지되고 TripMate가 sync됨
