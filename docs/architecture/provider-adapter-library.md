...
예외적으로 한국도로공사 OpenAPI는 별도 TripMate client adapter를 두지 않고 `pykex`의 `KexClient`를 직접 사용한다. API key, timeout, retry 값만 `apps/api/app/core/kex.py`에서 TripMate 설정으로 주입하고, ETL loader는 `pykex` typed model의 `raw` payload를 DB raw/serving 변환 입력으로 사용한다.
...
| 한국도로공사 | `pykex` `KexClient` 직접 사용으로 휴게소 master/oil/svcs 구현 | 휴게소 날씨 endpoint를 `pykex`에 먼저 추가하고, 도로공사 좌표계 metadata 통합 |
