...
- **버전 prefix**: 현재 **prefix 없음**(`/features/...`). krtour는 향후 `/v1/...` 도입을
  forward-looking으로 예고(미구현). **krtour ADR-048(PR #316, OPEN)** 가 이를 정식화 —
  외부 사용자 표면 전체를 `/v1`로 옮기고 admin/ops/debug까지 `/v1` 통일 예정. → TripMate
  client는 base path를 **설정값**으로 두어 `/v1` 전환에 대비한다(§6-G). 전환 시 무중단을 위해
  구 unprefixed ↔ `/v1` **이중지원 창** 합의 필요(§7).
...
- **[표준 추종] T-181 — ADR-048(krtour PR #316) 표준 추종**: krtour 외부 표면이 `/v1` +
  RFC7807 + 파라미터 개명으로 안정되면 T-170 client를 일괄 갱신 — (1) base path를 `/v1`로
  (config-driven, 무중단 이중지원 창 동안 전환), (2) `_error_code`를 problem+json 확장 `code`
  파싱으로, (3) 쿼리 빌더 개명 대응(`search` bbox CSV→4 float, in-bounds `limit`→`max_items`,
  `total_count` opt-in `?include_total=true`). frontend는 T-210e codegen이 신 spec을 pin.
  **선행**: §7 ADR-048 합의 5건. krtour spec이 아직 unprefixed라 **현재는 대기**(차단 아님).
...
- **ADR-048 연동 합의 대기(krtour PR #316 코멘트로 질의, 2026-06-09)** — 외부 표면 표준화:
  1. **외부 `/v1` 전환 + 이중지원 창**: 구 unprefixed ↔ `/v1` 동시 지원 기간 + `openapi.user.json`
     에 `/v1` 반영 시점 공지 → TripMate가 base만 바꿔 무중단 추종(T-181).
  2. **에러 `code` 위치/enum 고정**: RFC7807 `application/problem+json` 확장 `code`에 위 표준
     코드 enum 유지 확인(`_error_code` 갱신 기준).
  3. **소비 read 필드 불변 확인**: `*_key`→`*_id` 개명에서 소비 중인 `cluster_key`/`feature_id`/
     `target_key`(복합/자연키·canonical)는 **개명 제외**가 맞는지 + read 응답 `lon`/`lat`/`name`/
     `category`/`marker_*` 유지 전제.
  4. **외부 파라미터명 최종 확정**: `search` bbox CSV→4 float, in-bounds `limit`→`max_items`,
     `total_count` opt-in `?include_total=true` (TripMate 쿼리 빌더 영향).
  5. **반영 순서 공유**: PR #317 T-214b/d(외부 `/v1`)와 ADR-048(admin `/v1`)의 외부 spec 반영 시점.
