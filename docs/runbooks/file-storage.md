...
```bash
wsl.exe -e bash -lc "cd ~/dev/tripmate && docker compose -f infra/docker-compose.yml up -d postgres rustfs rustfs-init"
```

앱 smoke compose도 같은 방식으로 RustFS를 띄운다.

```bash
wsl.exe -e bash -lc "cd ~/dev/tripmate && docker compose -p tripmate-app-smoke -f infra/docker-compose.app.yml up -d app-rustfs app-rustfs-init"
```
