...
현재 주소 체계, Juso/법정동코드/VWorld 경계 DB 스키마, 다른 데이터셋과의 주소 매핑 기준은 `docs/architecture/address-schema.md`를 따른다.
...
- DB에 저장하는 timezone-aware datetime은 KST(`Asia/Seoul`) 기준으로 생성·조회한다.
- PostgreSQL 앱 세션은 연결 시 `SET TIME ZONE 'Asia/Seoul'`을 실행한다. raw/serving의 `collected_at`, ETL 실행 로그 시각, provider 기준시각 변환도 같은 기준을 따른다.
...
| 카테고리 | 데이터셋 | 주 용도 | 상태 |
| --- | --- | --- | --- |
| weather | `weather_short_term` | 현재/초단기/단기 예보 | 부분 구현 |
| weather | `weather_kma_alert` | 기상특보/기상정보/기상속보 Telegram 알림 원천 | 구현 |
| weather | `weather_mid_term` | 여행 일정상의 단기 이후 예보 | 구현 |
| weather | `weather_observed` | 과거 날씨 근사 | 보류 |
| weather | `weather_rest_area` | 휴게소별 날씨 | 확정: 휴게소 master와 매칭하지 않음 |
| air_quality | `air_quality_station` | AirKorea 측정소 목록과 법정동 매핑 | 구현 |
| air_quality | `air_quality_forecast` | AirKorea 미세먼지/오존 예보통보 | 구현 |
| air_quality | `air_quality_sido_measurement` | AirKorea 시도별 실시간 대기질 측정값 | 구현 |
| fuel | `fuel_avg_price` | 전국 일별 평균 유가 | 구현 |
| fuel | `fuel_lowest_station` | OpiNet 지역 기준 최저가 주유소 후보 | 구현 |
| fuel | `fuel_region_code` | OpiNet 유가 지역 코드 기준 | 구현 |
...
| rest_area | `rest_area_master` | 휴게소 기본정보 | 구현 |
| rest_area | `rest_area_oil_price` | 휴게소 주유소 가격/업체 | 구현 |
| rest_area | `rest_area_svcs` | 휴게소 편의시설 | 구현 |
| tour_point | `kma_recommended_tour_course` | 관광코스/관광지 후보 | 구현, 좌표 기반 법정동 매핑 |
...
기상청, AirKorea, 한국도로공사 등 저장 정책이 정리된 소스를 활용해 전국 날씨와 대기질 정보를 제공한다.
구현된 날씨/대기질 DB 스키마의 상세 기준은 `docs/architecture/weather-air-quality-schema.md`를 따른다.

TripMate의 날씨 UX 목표:

- 사용자가 어제 날씨와 오늘 날씨를 직감적으로 비교할 수 있게 한다.
- 오늘 현재 날씨와 내일/모레까지의 초단기·단기예보를 보여준다.
- 여행 일정 날짜가 단기예보 범위를 벗어나면 중기예보를 사용한다.
- 지도에서는 어제/오늘/내일을 오전/오후 단위로 간략 표시하고, 클릭 시 상세를 확장한다.
- 여행 계획 화면에서도 해당 날짜의 오전/오후 날씨를 먼저 보여주고, 클릭 시 상세를 확장한다.
- 미세먼지와 공기질은 별도 화면이 정해지기 전까지 날씨 정보와 함께 저장·표시할 수 있게 원천 필드를 최대한 보존한다.

### 4.1 `weather_short_term`

목적:

- 현재 날씨, 초단기 실황, 오늘/내일/모레 단기 예보
- 여행 장소 주변 날씨 요약
- 지도/여행계획의 어제/오늘/내일 오전·오후 요약 표시
...
- serving: endpoint, nx, ny, base_date, base_time, forecast_date, forecast_time, category

매핑 정책:

- 현재 구현은 시군구 경계 기반 대표 격자를 우선 만든다.
- 대표점은 `region_serving_boundary`의 polygon에서 `ST_PointOnSurface`로 계산한다. `ST_Centroid`는 polygon 밖 점이 나올 수 있으므로 기본값으로 쓰지 않는다.
- 장소 저장/조회 API가 구현되면 장소 좌표를 KMA DFS `nx`, `ny`로 직접 변환하고, 좌표가 없는 경우 행정구역 대표 격자를 fallback으로 사용한다.
- 장소 저장 시 `좌표 → 법정동 point-in-polygon → 좌표 직접 DFS 변환`을 수행하고, 필요한 경우 법정동/시군구 mapping table을 fallback으로 참조한다.
- 동일 `nx + ny + base_date + base_time` 조합은 serving cache를 우선 사용하고 API를 반복 호출하지 않는다.
...
- `nx`
- `ny`
- `mapping_method`: `postgis_point_on_surface`, `manual_override`
...
- `getUltraSrtNcst` 초단기실황 수집
- `getUltraSrtFcst` 초단기예보 수집
- `getVilageFcst` 단기예보 수집
- `weather_short_term_grid_mapping` 자동 생성
- raw row append와 serving upsert
- 기상청 발표 날짜/시간 KST 해석

후속 구현:

- 어제 날씨 비교용 과거 snapshot 저장. 정확한 장기 과거 관측이 아니라 전일 수집 serving snapshot을 우선 사용한다.
- 여행 장소 좌표 기준 조회 API
- 사용자 Telegram 여행 알림 메시지 조립

### 4.2 `weather_mid_term`

목적:

- 단기예보 이후 여행 일정 날짜의 오전/오후 예보
- 지도/여행계획에서 단기예보 범위를 벗어난 여행일 날씨 요약
- 여행 일주일 전 요약 알림
...
- 기상청 예보구역정보 또는 중기예보 활용가이드의 예보구역코드 표

수집:

- 방식: OpenAPI
- 갱신 주기: 하루 2회. 기상청 중기예보 발표시각은 06:00, 18:00 기준이므로 발표 이후 지연 시간을 두고 실행한다.
- freshness target: 24시간 이내
- 수집 범위: 기상청 중기예보 공식 구역코드 전체를 전국 단위로 주기 수집한다. 사용자 저장 장소 수에 따라 API call 수가 늘어나는 on-demand 구조로 만들지 않는다.
...
- 초기 중기예보 구역 seed 설정 파일: `config/kma-mid-term-regions.json`
...
- `regId`는 TripMate가 만든 코드가 아니라 기상청 중기예보 API가 요구하는 예보구역코드다.
- Juso 법정동코드, 시군구코드, 기상청 단기예보 `nx/ny`와 서로 다른 체계다.
- 따라서 `1111010100` 같은 법정동코드를 `regId`로 넘기거나, `regId`를 주소 FK처럼 저장하지 않는다.

매핑 정책:

- 중기예보 `reg_id`는 단기예보 `nx`, `ny`와 별도의 provider region code로 관리한다.
- 중기전망 `stnId`, 중기육상예보 `regId`, 중기기온예보 `regId`를 같은 코드라고 가정하지 않는다.
- 공식 활용가이드/예보구역정보의 코드표를 `weather_mid_forecast_region`에 적재한다.
- TripMate 주소 코드와의 연결은 `weather_mid_region_address_mapping`에 명시적으로 저장한다.
- 초기 매핑은 시도/시군구 단위 수동 검증 seed로 시작한다. 코드명 문자열이 비슷하다는 이유만으로 자동 매핑하지 않는다.
- 여행 장소 좌표는 먼저 법정동 경계 point-in-polygon으로 법정동 코드를 찾고, 법정동 코드에서 시군구/시도 코드를 도출한 뒤 중기예보 mapping을 적용한다.
- 육상예보는 광역 권역 성격이 강하므로 시군구 exact mapping이 없으면 공식 상위 권역 mapping을 사용한다.
- 기온예보는 도시/지역 대표값 성격이 강하므로 공식 코드표에 명시한 대표 지역 또는 수동 검증 mapping만 사용한다.
- fallback을 사용하면 API/알림/UI에 fallback 사용 여부와 기준 지역을 남긴다.
...
여행 장소 기준 fallback 순서:

1. 시군구 exact mapping
2. 시도/권역 parent mapping
3. 공식 코드표상 대표 지역 또는 수동 검증 mapping
4. 제공 불가 처리
...
- 중기예보는 여행 일정상의 날짜가 단기예보 범위를 벗어날 때 사용한다.
- 지도 기본 표시는 어제/오늘/내일 오전·오후까지만 우선 노출한다.
- 중기예보 상세는 여행 계획 날짜 또는 확장 상세에서 보여준다.
- 8~10일처럼 provider가 오전/오후를 나누지 않는 구간은 `forecast_slot='daily'`로 저장하고, UI에서 오전/오후 칸에 같은 요약을 재사용할 수 있게 한다.
...
### 4.5 `weather_kma_alert`

목적:

- 기상특보, 기상정보, 기상속보를 수집해 Telegram 알림 원천으로 사용
- 지도 마커에는 표시하지 않음

출처:

- data.go.kr / 기상청 기상특보 조회서비스
- https://www.data.go.kr/data/15000415/openapi.do

수집:

- 방식: OpenAPI
- endpoint: `getWthrWrnList`, `getWthrInfoList`, `getWthrBrkNewsList`
- 갱신 주기: 30분
- freshness target: 60분 이내
...
- 기상특보/정보/속보 응답은 좌표나 주소가 없으므로 법정동 FK를 붙이지 않는다.
- `stnId`, `stnNm`은 별도 지점코드 테이블에 누적한다.
- 참고문서 기반 전체 지점코드 선적재는 후속으로 구현한다.

승인 확인:

- 로컬 data.go.kr 인증키로 `getWthrWrnList`, `getWthrInfoList` 정상 응답을 확인했다.
- `getWthrBrkNewsList`는 `NO_DATA` 응답을 받았으며 권한 오류가 아니라 현재 기간 데이터 없음으로 판단한다.

### 4.6 `air_quality_station`

목적:

- AirKorea 측정소 목록을 수집하고 좌표를 법정동 경계와 매핑

출처:

- data.go.kr / AirKorea 측정소정보 조회서비스
...
- 갱신 주기: 매일 04:20 KST
- freshness target: 7일 이내
- 일 호출 제한: 500회. 현재 기본 호출량은 17개 시도 기준 17회/일이다.
...
- `dmX`는 위도, `dmY`는 경도로 해석한다. 이 해석은 smoke 응답 기준이며 운영 전 여러 시도 샘플로 재확인한다.
- 좌표가 있으면 EPSG:4326 point로 보고 PostGIS `ST_Covers`로 법정동 경계를 찾는다.
- 매핑되면 `legal_dong_code`, `sigungu_code`를 저장한다.
- 매핑되지 않으면 `mapping_method = 'unmapped'`로 저장하고 FK는 null로 둔다.

### 4.7 `air_quality_forecast`

목적:

- 미세먼지/오존 예보통보를 여행 전 알림과 앱 참고 정보로 제공

출처:

- data.go.kr / AirKorea 대기오염정보 조회서비스
...
- 권역 텍스트 성격이 강하므로 raw payload를 보존하고, 앱 표시 주요 필드를 serving column으로 정규화한다.
- 대기질 UI와 Telegram 문구 임계치는 아직 UI 정책으로 고정하지 않는다.
- 현재는 provider가 주는 PM10, PM25, O3 예보통보 원문, 등급, 원인, 행동요령 등 가능한 정보를 최대한 저장한다.
- 법정동 단위 FK를 붙이지 않는다.

### 4.8 `air_quality_sido_measurement`
...
- 갱신 주기: 매시 25분
- freshness target: 2시간 이내
- 일 호출 제한: 500회. 17개 시도 × 24회 = 408회/일이며, 예보통보 호출과 합산해 약 420회/일이다.
...
- `sido_name`, `station_name`, `data_time`을 serving unique 기준으로 사용한다.
- 통합대기환경지수, 미세먼지, 초미세먼지, 이산화질소, 오존, 일산화탄소, 아황산가스 값과 등급을 최대한 저장한다.
- provider가 주는 각 오염물질 flag 값도 함께 저장한다.
- 대기질은 지도/여행 계획/Telegram에서 날씨와 함께 보여주는 데이터로 취급하되, 표시 임계치와 UI 구성은 별도 화면 설계에서 결정한다.
- AirKorea 대기오염정보 서비스는 retry가 자주 발생하면 500회/일 제한을 넘을 수 있으므로 반복 실패 시 운영자가 DAG 주기를 완화한다.
...
- 로컬 개발에서는 저장소 루트 `.env`와 `apps/api/.env`의 `TRIPMATE_OPINET_API_KEY`로 주입한다. 두 파일은 Git에 포함하지 않는다.
- API 오류, quota 초과, 빈 응답은 백엔드 설정 파일의 데이터셋별 ETL retry 설정을 따른다. 평균가/최저가 기본값은 5분 간격 최대 3회 retry, 지역코드는 30분 간격 최대 3회 retry다.
...
- 가격 단위는 원/L다.
- 가격 기준일/기준시각은 `timestamp` 필드에 저장한다. 원천이 날짜와 시각을 분리 제공하면 KST 기준 timestamp로 변환한다.
...
- 현재 구현은 OpiNet 지역코드를 Juso `address_code_standard.legal_dong_code`의 시도/시군구 행과 이름 기반으로 매핑한다. 법정동 상세 행은 `legal_dong_code → sigungu_code → sido_code` 우선순위로 가장 가까운 OpiNet 지역코드를 찾는다.
- OpiNet 최저가 API의 좌표는 raw/serving에 원문 숫자로 보존한다. 좌표계와 정밀 반경 검색 정책은 확정 전이므로, 현재 "여행지 주변" 요약은 시군구 OpiNet 지역 기준 근사값으로 다룬다.
- "여행지 주변" 유가 정보는 OpiNet 시군구 기준 근사로 유지한다. 주유소 좌표계 확인 후에도 기본 사용자 리포트는 시군구 근사 정책을 우선한다.
- 사용자 증가에 따라 provider API call 수가 선형으로 늘어나지 않도록, 공공/외부 데이터는 가능한 한 전국 또는 기준 지역 전체를 주기 수집하고 serving cache를 조회한다. on-demand 호출은 사용자 입력 검증, 수동 갱신, cache miss 보완처럼 제한된 경우에만 사용한다.
...
- 가격은 원/L로 저장한다.
- 현재 구현은 OpiNet `avgAllPrice.do`의 전국 평균가를 `region_key = national`로 저장한다. 지역별 평균가가 필요한 경우 별도 OpiNet endpoint 또는 자체 집계 기준을 추가 결정한다.
- `trade_date`가 응답에 없으면 serving 중복 방지를 위해 `timestamp`의 KST 날짜를 `YYYYMMDD`로 저장한다. raw에는 원천값을 그대로 둔다.
...
- OpiNet `lowTop10.do`는 provider region code와 fuel code 단위로 호출한다.
- Airflow는 매핑된 전국 시군구 전체를 주기 수집한다. 사용자 요청 시 on-demand로 호출하지 않는다.
- 현재 구현은 요청한 OpiNet 지역의 TOP 후보를 raw/serving에 저장한다. 여행지 반경별 정밀 주유소 목록이 아니라 OpiNet 시군구 지역 단위 후보 cache다.
...
- 내부 `fuel_type`은 `gasoline`, `premium_gasoline`, `diesel`, `lpg` 중 하나로 정규화한다.
- 가격은 원/L로 저장한다.
- 가격 기준일/기준시각은 `timestamp` 필드에 저장한다. `lowTop10.do` 응답에 별도 가격 기준시각이 없으면 수집 시각을 사용한다.
- stable station id는 `UNI_ID`를 우선 사용한다. 없으면 `OS_NM`, `VAN_ADR`, `NEW_ADR`, row hash 순서로 fallback한다.
- 사용자에게 제공하는 "주변 평균"은 최저가 TOP 후보들의 평균이다. 실제 지역 전체 평균가가 아니므로 UI와 API에는 "주변 평균"과 "최저가 후보 평균"을 함께 표기한다.
...
- mapping status는 `matched`, `unmatched`, `ambiguous`를 사용한다. `ambiguous` 또는 `unmatched`는 serving row를 보존하되 앱 join에는 사용하지 않는다.
- 이름 기반 자동 매핑은 중복 후보를 같은 법정동코드 기준으로 dedupe한다.
...
- FK 불일치 row는 Airflow task log와 별도로 `<TRIPMATE_AIRFLOW_LOG_DIR>/etl/rest_area_fk_mismatch/<dataset>/<run_key>.jsonl`에 JSON Lines로 남긴다.
- FK 불일치 로그에는 최소한 `dataset`, `source_endpoint`, `source_key`, `serviceAreaCode2`, `collected_at`, `reason`을 기록한다.
- FK 불일치는 DAG 실패로 즉시 중단하지 않고 skip count metric으로 남긴다.
- 2026-04-26 live smoke 기준 `business/curStateStation`의 주유소 `serviceAreaCode2`는 현재 master endpoint인 `business/serviceAreaRoute`의 휴게소 `serviceAreaCode2`와 교집합이 없었다. 이 경우 정책대로 raw row만 남기고 serving row는 만들지 않는다. 이름/좌표 기반 후보 매칭은 하지 않는다.
...
- 현재 ETL 호출 endpoint: `https://data.ex.co.kr/openapi/business/serviceAreaRoute`
...
- `svar_cd`: 휴게소 join 기준 코드. provider 응답의 `serviceAreaCode2`를 저장한다.
- `provider_service_area_code`: provider 응답의 `serviceAreaCode`
- 같은 `serviceAreaCode2`가 여러 provider row에 반복되면 raw는 모두 저장하고 serving master는 `svar_cd` 1개 row로 병합한다. 다른 문자열 값은 `|`로 합쳐 저장한다.
...
- 현재 ETL 호출 endpoint: `https://data.ex.co.kr/openapi/business/curStateStation`
...
- 하루 2회 수집분을 모두 보존해야 하므로 serving unique 기준은 `svar_cd + provider_fuel_code + collected_at`이다. 같은 날짜의 오전/오후 수집분을 날짜 단위로 덮어쓰지 않는다.
...
- 현재 provider field는 `gasolinePrice`, `diselPrice`, `lpgPrice`다. `diselPrice` 오탈자는 원문 field명을 그대로 사용한다.
...
- 현재 ETL 호출 endpoint: `https://data.ex.co.kr/openapi/business/conveniServiceArea`
...
- provider 편의시설 명칭은 그대로 보존한다.
- 현재 provider는 편의시설별 별도 코드를 주지 않으므로 `provider_service_code`는 편의시설명을 정규화한 값이다.
- 앱 표시명은 초기에는 `display_name = provider_service_name`으로 저장하고, 별도 한국어 표시명 mapping은 추후 필요할 때 추가한다.
...
- 방식: 운영자가 Airflow 컨테이너에서 접근 가능한 CSV/ZIP 파일 경로를 제공
- 갱신 주기: 연 1회 확인

저장:

- raw: `tour_course_raw_kma_point`
- serving: `kma_recommended_tour_course`
- 상세 날씨 raw cache: `tour_course_raw_kma_spot_weather`
- 상세 날씨 serving cache: `tour_course_serving_kma_spot_weather`
...
- 상세 스키마는 `docs/architecture/kma-tour-course-schema.md`를 따른다.
...
| `spot_id` | string | no | CSV `관광지 아이디` |
| `region_id` | string | yes | CSV `지역 아이디` 원문 |
| `spot_name` | string | no | CSV `관광지명` |
| `longitude` | numeric | no | EPSG:4326 경도 |
| `latitude` | numeric | no | EPSG:4326 위도 |
| `course_order` | integer | yes | CSV `코스순서` |
| `travel_time_minutes` | integer | yes | CSV `이동시간` |
| `indoor_type` | string | yes | CSV `실내구분` 원문 |
| `theme_name` | string | yes | CSV `테마명` 원본 표시명 |
| `legal_dong_code` | string | yes | Juso 법정동코드 |
| `sigungu_code` | string | yes | 좌표 기반 법정동 경계 매핑에서 도출한 시군구코드 |
| `sido_code` | string | yes | 좌표 기반 법정동 경계 매핑에서 도출한 시도코드 |
| `address_snapshot` | string | yes | 현재 관광코스 CSV 적재에서는 생성하지 않음. 좌표 기반 법정동 매핑만 사용하므로 일반적으로 null |
| `address_mapping_method` | string | no | `postgis_point_in_polygon` 또는 `unmapped` |
...
- 현재 확인된 `TH05`와 `종교/역사/전통`은 `history_tradition`으로 mapping한다.
...
- serving 데이터의 위경도는 EPSG:4326 `latitude`, `longitude`로 정규화한다.
- `경도(도)`는 `longitude`, `위도(도)`는 `latitude`로 저장한다.
- CSV의 `지역 아이디`는 10자리 코드 형태지만 Juso `legal_dong_code`와 같다고 가정하지 않는다. 원문 `region_id`로만 보존한다.
- 현재 구현은 좌표 기반 PostGIS point-in-polygon으로 `legal_dong_code`, `sigungu_code`, `sido_code`를 채운다.
- 이 데이터셋에서는 V-WORLD reverse geocoding으로 주소 문자열을 만들거나 Juso 도로명주소관리번호/도로명코드/행정동코드까지 확장 매핑하지 않는다.
- V-WORLD reverse geocoding 기반 상세 주소 snapshot 매핑은 검토했던 대안이다. 현재 기준선에서는 구현 대상이나 후속 TODO가 아니며, 향후 새 제품 결정이 있을 때 별도 설계로 다시 연다.
- KMA 관광코스의 주소 계층은 `legal_dong_code`와 여기서 도출한 `sigungu_code`, `sido_code`까지만 사용한다.
- 좌표/명칭 기반 후보 매칭은 하지 않는다.
- 주소 문자열 기반 fuzzy matching도 수행하지 않는다.
- 좌표가 경계 밖이거나 경계 데이터가 없어 매핑되지 않으면 `address_mapping_method='unmapped'`로 두고, 좌표/관광지명/`region_id` 원문은 보존한다.
- 관광코스별 상세 날씨 API는 전체 관광코스를 무조건 주기 수집하지 않는다. 좁은 지역 날씨는 사용자 저장 장소 또는 여행 장소 주변 target 중심으로 cache/수집하고, 시도 단위 광역 날씨만 전국 단위 주기 수집 대상으로 둔다.
- 현재 구현은 주변 target 좌표와 반경을 입력받아 해당 범위의 관광코스만 `getTourStnVilageFcst1`로 갱신한다.
...
- 기상청 관광코스 CSV의 위경도 → 주소 문자열 변환에 V-WORLD Geocoder를 쓰는 방안은 검토했던 대안이다.
- 현재 기준선에서는 KMA 관광코스에 V-WORLD Geocoder를 사용하지 않는다. 좌표 기반 법정동 point-in-polygon만 수행한다.
- 향후 별도 제품 결정으로 Geocoder를 다시 열면 provider call 단위 최대 5회 retry, 실패 로그, 관리자 알림, 원문 미저장 원칙을 새 설계에 명시한다.
...
| weather_kma_alert | alert_type + stn_id + tm_fc + tm_seq + title | 30~60분 |
| weather_mid_term | reg_id + tm_fc | 12~24시간 |
| weather_rest_area | provider source id 또는 source coordinates + observed_at | 1~2시간 |
| air_quality_station | sido_name + station_name + mang_name + address | 7일 |
| air_quality_forecast | inform_code + data_time + inform_data | 12시간 |
| air_quality_sido_measurement | sido_name + station_name + data_time | 1~2시간 |
...
| kma_tour_course_spot_weather | course_id + spot_id + base_date + base_time + forecast_date + forecast_time + category | 사용자 저장 장소/여행 장소 주변 cache |
...
- ETL 실패 로그와 관리자/Telegram 알림 payload에는 외부 API 인증키 원문을 남기지 않는다. `serviceKey`, `certkey`, `apiKey`, `token` 계열 query parameter와 설정 파일·환경변수의 알려진 secret 값은 저장 전에 마스킹한다.
...
아래 값은 DAG/task 수준의 retry 기본값이다. provider call 내부 retry가 별도로 확정된 경우에는 해당 provider 규칙을 따른다. 예를 들어 외부 API adapter가 자체 quota 보호나 짧은 네트워크 retry를 갖는 경우에도 DAG run 실패 판정과 운영 알림은 이 표의 데이터셋별 설정을 기준으로 한다.
실제 운영값은 `config/etl-datasets.json`의 데이터셋별 설정을 따른다.
...
| `weather_kma_alert` | 30분 | 5분 간격 3회 | 3회 | 관리자 페이지 + 관리자 Telegram. 여행 알림에서는 사용자용 에러 메시지 |
...
| `air_quality_station` | 매일 | 30분 간격 3회 | 1회 | 관리자 페이지 + 관리자 Telegram |
| `air_quality_forecast` | 하루 4회 | 10분 간격 3회 | 2회 | 관리자 페이지 + 관리자 Telegram. 여행 알림에서는 사용자용 에러 메시지 |
| `air_quality_sido_measurement` | 매시 | 10분 간격 3회 | 3회 | 관리자 페이지 + 관리자 Telegram. 여행 알림에서는 사용자용 에러 메시지 |
...
- 설정 파일 loader: `apps/api/app/core/etl_config.py`
- 기본 설정 파일: `config/etl-datasets.json`
- 실행 로그 테이블: `etl_run_logs`
- 관리자 로그인 알림 기반 테이블: `admin_notifications`
- 권리자 Telegram 시스템 알림 outbox: `telegram_system_notification_outbox`
- 관리자/권리자 구분 필드: `users.is_admin`, `users.is_privileged`
- 실제 Telegram 발송 worker와 관리자 UI 표시는 후속 구현이다.
...
- `TH05`/`종교/역사/전통` → `history_tradition` mapping 확인
...
- 좌표 기반 PostGIS point-in-polygon으로 `legal_dong_code`, `sigungu_code`, `sido_code`가 채워지는지 확인
- 경계 밖 좌표 또는 경계 데이터 누락 시 `address_mapping_method='unmapped'`와 nullable 주소 FK로 적재되는지 확인
- KMA 관광코스 적재가 V-WORLD reverse geocoding, Juso 상세 주소 key 매핑, 좌표/명칭 후보 매칭을 수행하지 않는지 확인
...
- Airflow 런타임은 `infra/docker-compose.yml`의 `airflow-webserver`, `airflow-scheduler`, `airflow-dag-processor`, `airflow-worker` 서비스로 실행한다.
- Airflow 3.x에서 `airflow-webserver` 서비스의 실제 command는 `api-server`다.
- Airflow task는 `TRIPMATE_DATABASE_URL`, `TRIPMATE_AIRFLOW_DOWNLOAD_DIR`, `TRIPMATE_ETL_CONFIG_PATH`를 사용한다.
...
- 10일 이후 여행계획이 없는 첫 날짜에 전체 주소 ZIP을 다운로드하고 raw/serving snapshot을 갱신한다.

VWorld SHP 운영 command:

- command module: `app.cli.vworld_boundary`
- VWorld ZIP 파일은 자동 다운로드하지 않고 운영자가 전달한다.
...
- `weather_short_term`: 초단기실황/초단기예보/단기예보 raw/serving schema와 시군구 대표 격자 mapping은 구현됨. 후속으로 어제/오늘 비교용 snapshot 조회, 여행 장소 좌표 기준 조회 API, Telegram 여행 알림 조립이 필요하다.
- `weather_kma_alert`: 기상특보/기상정보/기상속보 raw/serving schema와 DAG는 구현됨. 후속으로 참고문서 기반 지점코드 전체 선적재와 Telegram 메시지 필터링이 필요하다.
- `weather_mid_term`: 공식 중기예보 구역 seed, `weather_mid_forecast_region`, `weather_mid_region_address_mapping`, fallback metadata, raw/serving schema, 전국 단위 주기수집 DAG는 구현됨. 운영 전 `config/kma-mid-term-regions.json`을 공식 최신 코드표와 재대조하고, 조회 API/여행 알림 조립을 연결해야 한다.
- `weather_rest_area`: 실제 응답 샘플 기준 필드명과 좌표계 확인
- `air_quality_station`: 측정소 raw/serving schema, 좌표 기반 법정동 매핑, DAG는 구현됨. 운영 전 여러 시도 샘플로 `dmX`/`dmY` 좌표 순서 재확인이 필요하다.
- `air_quality_forecast`: 예보통보 raw/serving schema와 DAG는 구현됨. 후속으로 모든 예보 필드를 조회 API에 노출하고 날씨와 함께 표시/알림 조립하는 로직이 필요하다.
- `air_quality_sido_measurement`: 시도별 측정값 raw/serving schema와 DAG는 구현됨. 오염물질 값/등급/flag 저장도 반영됨. 후속으로 조회 API에 노출하고 날씨와 함께 표시/알림 조립하는 로직이 필요하다.
- `fuel_avg_price`: 조회 API/UI 계약 구현
- `fuel_lowest_station`: 조회 API/UI 계약 구현. UI에는 "주변 평균"과 "최저가 후보 평균"을 함께 표기
- `fuel_region_code`: 행정구역 개편 시 사라진 OpiNet region code 처리 정책 확정
- `etl_dataset_config`: 데이터셋별 schedule, retry 간격/횟수, 관리자 알림 여부, Telegram 알림 여부를 담는 설정 파일은 구현됨. 후속으로 관리자 UI 변경 기능이 필요하다.
- `etl_admin_notifications`: retry 소진 실패 알림 테이블은 구현됨. 후속으로 관리자 로그인 시 표시 API/UI가 필요하다.
- `etl_telegram_notifications`: retry 소진 실패 outbox는 구현됨. 후속으로 실제 Telegram target 조회와 발송 worker가 필요하다.
...
- `user_privileged_flag`: `users.is_admin`, `users.is_privileged` 필드는 구현됨. 앱에서는 승격/강등 UI를 만들지 않음
- `administrative_boundary`: V-WORLD SHP raw EPSG:5179 적재와 serving EPSG:4326 변환 파이프라인은 구현됨. 후속으로 관리자 업로드 UI가 필요하다.
- `legal_dong_boundary`: 법정동 단위 SHP raw EPSG:5179 적재와 serving EPSG:4326 변환, `legal_dong_code` 기반 point-in-polygon은 구현됨.
- `juso_road_address_korean`: 매월 10일 이후 전체 주소 ZIP 다운로드, `rnaddrkor_*.txt` raw/serving 교체, code 문자열 보존은 구현됨. 후속으로 주소 저장 도메인 FK 연결이 필요하다.
- `juso_related_jibun`: 매월 10일 이후 `jibun_rnaddrkor_*.txt` raw/serving 적재와 도로명주소관리번호 연계는 구현됨.
- `place`/주소 저장 도메인: 주소 FK nullable 설계와 저장 당시 주소 snapshot 필드 구현
- `rest_area_query_api`: 휴게소 master/유가/편의시설 serving 테이블을 앱에서 조회하는 API 계약 구현
- `kma_recommended_tour_course`: CSV/ZIP decoding, raw/serving schema, `theme_category_code` 원본 보존, 내부 `theme_category` enum mapping, `unknown` fallback, raw CSV row, file hash, EPSG:4326 `longitude`/`latitude`, 좌표 기반 법정동 point-in-polygon은 구현됨. V-WORLD reverse geocoding과 Juso 상세 주소 key 매핑은 검토했던 대안일 뿐 현재 후속 구현 대상이 아니다. 사용자 저장 장소 또는 여행 장소 주변 target 기반 관광코스 상세 날씨 cache loader도 구현됨. 후속으로는 관광코스 지도/UI와 저장 장소/여행 장소 도메인에서 cache 갱신을 호출하는 연결이 필요하다.
...
- 휴게소 Airflow DAG가 운영 DB에서 실제 한국도로공사 API를 호출해 raw/serving row와 FK 불일치 JSONL 로그를 남기는지 통합 smoke 테스트 추가
- `weather_rest_area`가 `rest_area_master`와 매칭되지 않는지 schema/API 테스트 추가
- 관광코스 CSV `cp949`/`ms949` decoding, delimiter, 좌표 순서, 좌표 기반 법정동 point-in-polygon 테스트 추가
- 관광코스 serving 필드 타입/nullable, `theme_category_code` 원본 보존, 내부 `theme_category` enum mapping, `unknown` fallback 테스트 추가
- 관광코스 적재가 V-WORLD reverse geocoding, Juso 상세 주소 key 매핑, 좌표/명칭 후보 매칭을 수행하지 않는다는 회귀 테스트 유지
- 관광코스 상세 날씨가 전체 코스 무조건 수집이 아니라 사용자 저장 장소 또는 여행 장소 주변 target 중심 cache 정책을 따르는지 회귀 테스트 유지
