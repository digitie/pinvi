...
`TRIPMATE_KMA_TOUR_COURSE_SOURCE_PATH`가 없으면 job는 skip한다. 파일은 운영자가 Dagster 컨테이너에서 접근 가능한 경로에 업로드하거나 배치한다.

관광코스 상세 날씨는 현재 Dagster 정기 job로 전체 수집하지 않는다. 저장 장소/여행 장소 도메인이 구현된 뒤, 해당 좌표 목록을 target으로 넘기는 cache 갱신 job을 별도 연결한다.
