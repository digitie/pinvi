...
1. **worktree 포인터 환경 혼용** — 각 worktree의 `.git` 파일에는 worktree를
   **만든 환경 기준 절대경로**가 박힌다. WSL에서 만들면
   `gitdir: /mnt/f/dev/pinvi/.git/worktrees/...`, Windows git에서 만들면
   `gitdir: F:/dev/pinvi/.git/worktrees/...`. 같은 폴더라도 두 표기가 다르므로
...
같은 문제를 별 저장소 `kor-travel-geo`가 ADR-041로 먼저 해결했고, 본 모델은
그와 동일 패턴이다.

## 1. 디렉터리 레이아웃

```text
F:/dev/pinvi                      # 사람 trunk (사용자 직접 사용, main 점유) — AI 편집 금지
F:/dev/pinvi-claude               # Claude Code worktree (NTFS, git source of truth)
F:/dev/pinvi-codex                # OpenAI Codex worktree
F:/dev/pinvi-antigravity          # Google Antigravity worktree
~/pinvi-workspaces/pinvi-claude     # Claude용 WSL ext4 테스트 미러 (일회용)
~/pinvi-workspaces/pinvi-codex      # Codex용 테스트 미러
~/pinvi-workspaces/pinvi-antigravity# Antigravity용 테스트 미러
```

| 종류 | 위치 | 비고 |
|------|------|------|
| git / 편집 / commit / PR | NTFS worktree `F:/dev/pinvi-<agent>` | Windows `git.exe`만 |
| 의존성·테스트·Docker·장기 실행 | WSL ext4 `~/pinvi-workspaces/pinvi-<agent>` | commit 금지 |
| 프론트 dev/lint/typecheck/build/Vitest | WSL ext4 `~/pinvi-workspaces/pinvi-<agent>` | Linux Node/npm |
| Playwright 브라우저 e2e | Windows `F:/dev/pinvi-<agent>` | Windows Node/브라우저, 대상 dev server는 WSL |
| 데이터 (`dataset/`, `refdocs/`) | NTFS 원본 `/mnt/f/dev/pinvi/dataset/` | ext4에선 심볼릭 링크/절대경로 참조 |
...
```bash
mkdir -p ~/pinvi-workspaces/pinvi-claude
rsync -a --delete \
  --exclude .git \
  --exclude .codegraph \
  --exclude '.venv*' \
...
```bash
cd ~/pinvi-workspaces/pinvi-claude
test -e dataset || ln -s /mnt/f/dev/pinvi/dataset dataset
test -e refdocs || ln -s /mnt/f/dev/pinvi/refdocs refdocs
```
...
rsync -a ~/pinvi-workspaces/pinvi-claude/apps/api/app/ \
        /mnt/f/dev/pinvi-claude/apps/api/app/
...
```bash
cd ~/pinvi-workspaces/pinvi-claude/apps/api
. .venv-wsl/bin/activate
...
```bash
docker run -d --name pinvi-postgis \
  -p 5432:5432 \
  -e POSTGRES_USER=pinvi -e POSTGRES_PASSWORD=changeme -e POSTGRES_DB=pinvi \
  -v pinvi-pgdata:/var/lib/postgresql/data \
  postgis/postgis:16-3.5-alpine
# DSN: postgresql+asyncpg://pinvi:changeme@localhost:5432/pinvi
```
...
`feature` / `provider_sync` schema와 그 extension은 `kor-travel-map`이 소유·적재
한다(ADR-003) — 본 저장소는 `app`/`ops`만 Alembic으로 관리한다.

## 9. 환경변수 (`.env` 예시)

```dotenv
PINVI_DATABASE_URL=postgresql+asyncpg://pinvi:changeme@localhost:5432/pinvi
PINVI_JWT_SECRET_KEY=change-me-32-bytes-minimum-...
...
```powershell
wsl.exe -e bash -lc "cd ~/pinvi-workspaces/pinvi-claude && rsync -a --delete --exclude .git /mnt/f/dev/pinvi-claude/ ~/pinvi-workspaces/pinvi-claude/"
wsl.exe -e bash -lc "cd ~/pinvi-workspaces/pinvi-claude/apps/api && . .venv-wsl/bin/activate && python -m pytest tests/integration -q"
```
...
- ADR-003 — `feature`/`provider_sync` schema 소유권(`kor-travel-map`).
- `docs/runbooks/codegraph-worktrees.md` — worktree 생성·CodeGraph·git 포인터 함정.
- `kor-travel-geo` `docs/dev-environment.md` — 동일 패턴 reference(ADR-041).
