...
| `app.ktm_cache_target_events`            | Map at-least-once 결과의 immutable inbox                        | `event_id` PK, target event tuple partial unique, `cache_target.reconciled`는 네 target tuple이 모두 NULL인 stream receipt, 모든 event의 source/payload fingerprint는 32-byte 필수, exact `event_type` 4종, relay order 양수           |
...
| `app.ktm_cache_target_canary_runs`       | production causal canary의 실행별 crash-safe 감사 정본          | run UUID PK, 고정 synthetic target FK, running target partial unique, command target/generation·event generation·claim ACK composite FK, phase/status와 final local/remote cursor·count·Merkle 및 backlog 3종 all-or-none/equality CHECK |
