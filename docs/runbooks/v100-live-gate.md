...
| Mutating Playwright          | 통과(local dev). N150 Docker runner로 `trip-realtime-mutating` 1 passed, `backup-mutating` 1 passed. 운영 public DB 대상 실행은 금지한다. | 전용 staging Web/API 준비 또는 release 직전 drift가 있으면 같은 phase 재실행 |

따라서 현재 남은 T-273 hard blocker는 repo 코드가 아니라 외부 운영 상태인 geofence 운영 배포 설정이다.
전용 staging Web/API는 release evidence 재실행 조건으로 남긴다. 이 준비 전에는 full live e2e를 반복
실행하지 않는다.
