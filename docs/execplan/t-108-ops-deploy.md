...
ADR-067의 N150 단일 운영을 실제 배포 가능한 절차로 만든다.
N150이 유일한 실행·배포·복구 노드다.
...
3. `scripts/deploy-node.sh`, `scripts/n150-docker-doctor.sh` 추가.
4. `docs/runbooks/deploy.md`, `infra/n150/README.md`, N150 운영 절차 작성.
...
- N150 fallback은 기존 runtime이 없는 별도 fresh project에서만 `PINVI_ENVIRONMENT`/env file/
  credential file을 명시해 수행한다.
- N150에서 doctor가 arch/OS/env/local health를 점검한다.
...
- N150 장애 복구 시 최신 백업과 RustFS snapshot 복구 순서를 확정한다.
