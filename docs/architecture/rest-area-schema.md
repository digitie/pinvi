...
현재 provider 응답은 가격 기준시각을 별도로 주지 않으므로 `price_at = collected_at`, `price_time_source = collected_at`으로 저장한다. 유가 DAG는 하루 2회 실행되므로 serving row의 중복 기준은 날짜가 아니라 수집 시각(`collected_at`)이다. 같은 날 오전/오후 수집분은 서로 다른 스냅샷으로 보존하고, 같은 Airflow logical timestamp 재시도만 같은 스냅샷으로 갱신한다.
...
5. 기본적으로 DAG를 즉시 실패시키지 않는다.

로그 경로:

```text
<TRIPMATE_AIRFLOW_LOG_DIR>/etl/rest_area_fk_mismatch/<dataset>/<run_key>.jsonl
```

기본 로컬 경로:

```text
.tmp/airflow-logs/etl/rest_area_fk_mismatch/
```
...
- `rest_area_oil_price_daily`: 하루 2회 휴게소 주유소 가격 수집. Airflow logical timestamp를 `collected_at`으로 넘겨 retry 중복과 하루 2회 스냅샷 보존을 동시에 만족시킨다.
