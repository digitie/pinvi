...
| `app.ktm_cache_target_events`            | Map at-least-once 결과의 immutable inbox                        | `event_id` PK, target event tuple partial unique, `cache_target.reconciled`는 target tuple이 모두 NULL인 stream receipt, exact `event_type` 4종, relay order 양수           |
