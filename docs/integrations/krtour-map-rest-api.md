...
- **[표준 추종] T-181 — ADR-048(krtour PR #316) 표준 추종 (hard cutover lockstep)**: krtour
  ...
  불변식(`meta`/`request_id` 항상, `next_cursor` null) 계약 테스트. frontend는 T-210e codegen이
  신 spec pin. **선행**: §7 ADR-048 합의(A~F). krtour spec이 아직 unprefixed라 **현재 대기**(차단 아님).
  ※ 2026-06-09 재리뷰로 "무중단 이중지원" 전제 **철회** — 미출시·유일소비자라 hard cutover가 정합.
...
- **ADR-048 연동 합의 대기(krtour PR #316 — 1차 5건 → 2026-06-09 재리뷰로 입장 전환)**:
  소유자 지시(호환성 무시, 일관성/확장성/안정성 우선) + TripMate 미출시·유일소비자 전제로
  **1차의 "무중단/동결" 요청을 철회**하고 본질 기준으로 재합의(krtour PR #316 재리뷰 코멘트):
  - **A. dual-support 창 철회 → hard cutover**: 구 unprefixed alias(shim)는 ADR-046과 모순.
    외부 `/v1` 단일 commit cut + TripMate lockstep 교체(T-181). `Deprecation`/`Sunset` 불요.
  - **B. 좌표명 정렬(동결 해제)**: TripMate DEC-07은 `longitude`/`latitude`인데 krtour read는
    `lon`/`lat` — 동결 대신 **하나로 정렬**(경계 매핑 0). TripMate는 `lon`/`lat` 하향 수용 의향.
    → **DEC-07 좌표명 하위결정**(아래) 선결.
  - **C. `cluster_key`는 merit으로 분류**: 행정구역 자연키면 `cluster_key` 유지, 불투명 surrogate면
    `cluster_id`. "compat 동결"이 아니라 §3.1 규칙(시스템단일 `_id`/복합·자연키 `_key`)으로 결정.
  - **D. `feature_id` 값 안정성 명문화**: #317 v0/v1 행 + soft-delete에서 외부 `feature_id`가
    재적재/편집/버전승급에 **불변**임을 외부 계약에 박기(이름 동결과 별개). TripMate가
    `trip_day_pois.feature_id` FK + snapshot 키로 영속화하므로 값이 흔들리면 일정 깨짐.
  - **E. envelope 불변식 계약화**: `meta`(+`request_id`)를 단건·에러 포함 전 응답 항상 present,
    `next_cursor`는 항상 키 존재·소진 시 `null`(omit 금지).
  - **F. `/vN` major 거버넌스**: pre-1.0 `/v1` 가변(in-place break 허용) → v1.0.0 GA에 `/v1` 동결,
    이후 break는 `/v2` + N-1 동시지원. OpenAPI major별 분리 export.
  - **수용(이견 없음)**: admin/ops/debug `/v1` + 내부 `*_key`→`*_id` 물리 전파(T-180 미구현이라
    무영향), problem+json top-level `code`/enum, `page_size` 2-티어 캡, in-bounds `max_items`,
    bbox 4-float, `state`→`status`.
