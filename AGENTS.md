...
본 저장소는 AI 도구마다 **고정 worktree**를 둔다. trunk(`F:/dev/pinvi` 또는
`~/pinvi-workspaces/pinvi`)는 사람이 직접 만지는 checkout이며, AI 도구는
trunk를 절대 편집하지 않는다.

| AI 도구 | worktree 이름 | 예시 경로 | idle branch |
|---------|--------------|----------|-------------|
| Claude Code | `pinvi-claude` | `F:/dev/pinvi-claude` | `agent/claude-idle` |
| OpenAI Codex (CLI / VS Code) | `pinvi-codex` | `F:/dev/pinvi-codex` | `agent/codex-idle` |
| Google Antigravity 2.0 | `pinvi-antigravity` | `F:/dev/pinvi-antigravity` | `agent/antigravity-idle` |
...
- **Git 실행**: Windows worktree(`F:/dev/pinvi-<agent>`, NTFS)에서 git 명령은
...
- **git / 편집 / commit / push / PR**: NTFS worktree `F:/dev/pinvi-<agent>`에서
...
- **의존성 설치·`pytest`·`docker`·장기 실행**: WSL ext4 미러
  `~/pinvi-workspaces/pinvi-<agent>`에서. 파일 권한·inotify·I/O가 ext4에서
...
- WSL 명령은 `wsl.exe -e bash -lc "cd ~/pinvi-workspaces/pinvi-<agent> && ..."`로 감싼다.
...
이 repo는 **공개**다. 운영 도메인(개인 dynamic-DNS)·시크릿을 추적 파일에 커밋하지
않는다. 실제 값은 gitignore된 `infra/.env.prod`(템플릿 `infra/.env.prod.example`)에만 두고,
배포는 `PINVI_ENV_FILE=infra/.env.prod scripts/deploy-node.sh deploy`로 compose
`--env-file` 주입한다. 추적 문서/예시는 `*.example.com` placeholder로만 쓴다. CI 빌드의
`NEXT_PUBLIC_PINVI_API_URL`은 `secrets.NEXT_PUBLIC_PINVI_API_URL`에서 받는다. Dagster
webserver는 12802 고정(`apps/etl/Dockerfile`, compose profile `etl`).
...
`docs/runbooks/codegraph-worktrees.md` §3.6, 발송 시점은 `docs/agent-workflow.md` §5.
(kor-travel-map PR #229 동일 패턴.)

## 현재 단계 정책

초기 v2 bootstrap 시기의 "코드 작성 금지" 규칙은 Sprint 1 진입 전까지의 임시
제약이었다. 현재 기준선은 **Sprint 1~3 머지 완료, Sprint 4 릴리즈 게이트 충족
및 v0.1.0 tag/릴리즈 노트 정리 단계**다.
...
- Dagster orchestration (Pinvi 자체 job + 외부 서비스 갱신 트리거)
