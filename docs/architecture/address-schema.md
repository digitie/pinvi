...
| --- | --- | --- | --- |
| data.go.kr `국토교통부_전국 법정동` | Airflow가 3개월 1회 다운로드 | 구현 | `address_code_standard`의 canonical 법정동/시도/시군구 코드 기준 |
...
| VWorld reverse geocoding | 검토 이력 | 미구현 | KMA 관광코스에는 사용하지 않는다. 일반 POI에서 좌표→주소 문자열이 꼭 필요해질 경우 별도 결정 후 adapter를 설계한다. |
...
### Kakao/Naver/Google/KTO 같은 POI provider
...
```bash
wsl bash -lc "cd /mnt/f/dev/mapplan/apps/api && .venv-wsl/bin/python -m pytest"
wsl bash -lc "cd /mnt/f/dev/mapplan/apps/api && .venv-wsl/bin/python -m alembic upgrade head"
```
