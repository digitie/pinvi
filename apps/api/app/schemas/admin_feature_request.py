...
승인 시 krtour `/v1/admin/features*` change API로 전송(전송 client = T-180).
"""
...
class AdminFeatureRequestSummary(BaseModel):
    ...
    krtour_ref: dict[str, Any] | None = None
...
class AdminFeatureRequestApprove(BaseModel):
    """승인 — krtour change API 호출 입력.

    krtour `create`(new_place)는 `category`(8자리 코드)/`marker_color`/`marker_icon`이
    필수다(사용자 제안엔 없음 → Admin이 검토하며 채운다). correction은 override로 일부만,
    closure는 불필요. `access_reason`은 TripMate audit 사유, `krtour_reason`은 krtour로
    ...
    krtour_reason: str | None = Field(default=None, max_length=500)
