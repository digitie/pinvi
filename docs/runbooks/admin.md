...
- 직접 개발 관리자 raw/ETL 데이터 브라우저: `http://localhost:3001/admin`
- 직접 개발 관리자 CRUD 콘솔: `http://localhost:3001/admin/data`
...
- 관리자 전용 로그인.
- `/admin`: ETL/공공데이터 테이블 목록 조회.
- `/admin/data`: users/features/trips/POI CRUD.
- 테이블명 검색.
- 행 데이터 전체 검색.
- 컬럼별 부분 일치 필터.
- 컬럼 정렬.
...
- CRUD 콘솔에서 entity별 검색/필터, 상세 key-value, 연관 데이터 링크, feature/POI 좌표 미리보기와 Kakao 지도 링크.
- raw 데이터 우선 그룹: beach/KHOA, OpiNet 유가, KEX 휴게소, VWorld/주소 경계, Dagster ETL, API/email/audit.
...
제외 이유는 개인정보, 인증 세션, 여행 도메인 데이터는 ETL 데이터 브라우저와 목적이 다르기 때문이다. 사용자/여행/POI는 `/admin/data`의 전용 CRUD API로만 다룬다.
