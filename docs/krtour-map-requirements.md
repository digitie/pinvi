...
>
> **2026-06-12 추가 요구**: `python-krtour-map`에 `curated_features` 기능이 추가됐다.
> TripMate는 이를 자체 큐레이션을 대체하는 것이 아니라 **추가 소스**로 소비한다.
> krtour curated feature 1건은 TripMate `curated_trip_plans` 1건으로 1:1 복사되어야 하며,
...
| 포트 | API/Admin API `12301` | 라이브러리는 포트 없음. 디버그 UI만 `8087` |
...
openapi.user.json, 포트 12301, `/tripmate/features/batch` 등)은 krtour-map 저장소에
...
- krtour-map 추가 작업(**큰 신규**): `krtour-map-debug-ui`와 별개로 **인증 있는 운영 API**를 신설하거나 debug-ui를 운영급으로 승격. 포트 12301 정렬, §2의 모든 엔드포인트 신설, `openapi.user.json`(사용자 표면)·`openapi.json`(전체) 생성 + drift gate, 인증/인가(토큰), rate-limit.
...
| K-14 | 운영급 HTTP 서비스 | (신규) | ✅ 구축됨(`krtour-map-admin` 12301) | — | 2026-06-08 완료 |
...
- **TripMate 후속**: T-179(Admin 승인→호출) + T-180(admin API client). 연동 합의 5건
...
> **상태 (2026-06-12)**: krtour-map T-222b가 `openapi.user.json`에
> `/v1/public/beaches*`, `/v1/public/festivals*`를 추가해 본 §6 요구는 1차 충족됐다.
> TripMate T-130도 해당 표면을 소비하도록 연결했다. 아래 요구 표는 왜 public 표면이 필요했는지
> 보존하는 이력이며, 현재 구현 정본은 `docs/api/public.md`와
> `docs/integrations/krtour-map-rest-api.md`다.
...
기존에는 krtour user API(`openapi.user.json`)가 일반 `/v1/features/*` + `/v1/categories`만
노출해 해수욕장/축제의 **풍부한 도메인 필드**(수질·KHOA 예보·축제 상세 등)를 계약에 담지
않았다. 현재는 public view 전용 표면이 생겨 TripMate가 해당 응답을 서버 측에서 프록시한다.
...
- 도메인 필드가 계약에 들어와 TripMate `/public/*` 구현을 완료했다(T-130). 앱 내부 공통
  rate-limit는 T-195 후속이다.

### 6.5 krtour 회신 요청 — 완료

1. 해수욕장/축제 public view 표면 추가 — 완료.
2. 수질·KHOA index는 nullable projection으로 1차 노출 — 후속 데이터 보강은 krtour 쪽 provider
   freshness/field 보강에서 추적.
3. "월별 active 축제" 집계는 krtour `/v1/public/festivals/monthly`가 제공 — 완료.

---

## 7. `curated_features` → TripMate curated trip plans import 요구사항

> **상태 (2026-06-12)**: 사용자 지시 기준으로 krtour-map에 `curated_features` 기능이
> 추가됐다. TripMate 쪽 상세 구현은 아직 시작하지 않는다. 먼저 krtour REST 계약이
> 확정되어야 하며, 본 절은 그 계약에 대한 TripMate 소비 요구사항이다.

### 7.1 제품 의미

TripMate의 curated trip plan은 두 소스를 모두 지원한다.

| 소스 | 의미 |
|------|------|
| TripMate-native 큐레이션 | Admin/운영자/TripMate agent가 TripMate 안에서 직접 기획·작성 |
| krtour `curated_features` import | krtour-map이 제공하는 curated feature 묶음을 TripMate가 REST로 조회해 1:1 복사 |

krtour import는 TripMate-native 큐레이션을 대체하지 않는다. 두 흐름은 모두
`app.curated_trip_plans` / `app.curated_plan_pois`에 저장되고, 사용자는 같은
`/notice-plans/{id}/copy` 흐름으로 자기 trip에 가져온다.

### 7.2 필요한 REST 표면

TripMate가 필요로 하는 최소 능력:

| 능력 | 요청 |
|------|------|
| 목록 조회 | import 후보 목록, publish 상태, updated/version, 지역/카테고리 필터 |
| 상세 조회 | curated feature 1건의 메타 + 하위 item/POI 전체 |
| 증분 확인 | `updated_at` 또는 version/etag 기반으로 TripMate import stale 여부 판단 |
| 안정 id | 같은 krtour curated feature와 item을 재조회해도 동일 id 유지 |

예상 경로는 아직 제안 수준이다. krtour OpenAPI 확정 전까지 TripMate 구현은 대기한다.

```http
GET /v1/curated-features?category=&region=&page_size=&cursor=
GET /v1/curated-features/{curated_feature_id}
```

### 7.3 데이터 매핑

| krtour `curated_features` | TripMate 저장 |
|---------------------------|---------------|
| curated feature id | `curated_trip_plans` 원본 추적 후보 컬럼 또는 import metadata |
| title/summary/category/region/source | `curated_trip_plans.title/summary/category/destination/source_name` |
| publish/visibility | TripMate `is_published`는 기본 false로 import 후 Admin이 확정 |
...
- krtour item이 `feature_id`를 제공하면 TripMate는 같은 plan 안에서 해당
  feature-backed POI를 먼저 찾고, 없으면 생성한다.
- krtour item이 `feature_id`를 제공하지 않으면 `feature_id = null`인 curated POI로 저장한다.
- TripMate-native 큐레이션 역시 feature 없는 자유 POI를 만들 수 있다.
- TripMate는 `curated:<id>` 같은 가짜 feature id를 만들지 않는다.
- krtour feature schema와 cross-schema FK는 두지 않는다.

### 7.5 후속 컬럼 후보

REST 계약이 확정되고 import/refresh/idempotency 추적이 필요해지면 TripMate는 별도
migration으로 다음 후보를 검토한다.

| 후보 컬럼 | 대상 | 용도 |
|-----------|------|------|
| `source_system` | `app.curated_trip_plans` | `tripmate` / `krtour-map` 구분 |
| `source_curated_feature_id` | `app.curated_trip_plans` | krtour curated feature 원본 id |
...
| `source_curated_feature_item_id` | `app.curated_plan_pois` | krtour 하위 item 원본 id |

현 단계에서는 컬럼을 선행 확정하지 않는다. API id/버전/refresh 정책이 먼저 정해져야 한다.

### 7.6 krtour 회신 요청

1. `curated_features` REST 경로와 OpenAPI schema 명세 위치.
2. curated feature와 하위 item의 안정 id, version/etag, updated timestamp 제공 여부.
3. item이 krtour `feature_id`를 필수로 갖는지, nullable인지.
4. item day/order, snapshot, 이미지/첨부/media 참조 셰입.
5. TripMate가 1:1 import 후 refresh할 때 기존 TripMate plan을 갱신해야 하는지,
   새 버전 plan을 만들어야 하는지에 대한 권장 정책.
