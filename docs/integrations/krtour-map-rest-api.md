...
- **[H2] T-179 — Admin 검사/승인 → krtour feature change(DEC-05, admin 도메인) — actionable**:
  ...
  승인 시 §2.9 **`POST/PATCH/DELETE /v1/admin/features*`** 호출(K-15 = krtour PR #317로
  구현됨, krtour ADR-051이 이 흐름을 전송 구간 정본으로 승인). 결과
  `feature_id`/`request_id`/state를 `feature_suggestions`에 저장. review_mode 합의(§7) 선행.
