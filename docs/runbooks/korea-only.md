...
TRIPMATE_GEOFENCE_TRUSTED_PROXY_CIDRS=["172.18.0.0/16"]
TRIPMATE_GEOFENCE_MTLS_VERIFIED_HEADER=
TRIPMATE_GEOFENCE_MTLS_VERIFIED_VALUE=SUCCESS
...
- shared secret: `TRIPMATE_GEOFENCE_TRUSTED_PROXY_SECRET`과
  `TRIPMATE_GEOFENCE_TRUSTED_PROXY_HEADER` 비교.
- source IP: `TRIPMATE_GEOFENCE_TRUSTED_PROXY_CIDRS`에 포함된 reverse proxy / tunnel IP만 허용.
- mTLS: proxy가 client cert 검증 후 주입하는
  `TRIPMATE_GEOFENCE_MTLS_VERIFIED_HEADER` 값이
  `TRIPMATE_GEOFENCE_MTLS_VERIFIED_VALUE`와 일치해야 함.

secret이 없거나 틀리거나, CIDR/mTLS 조건을 통과하지 못하면 country는 `UNKNOWN`으로
처리되며 `TRIPMATE_GEOFENCE_BLOCK_UNKNOWN=true`에서 451로 차단된다. admin/operator/cpo
...
운영에서는 shared secret 단독보다 `TRIPMATE_GEOFENCE_TRUSTED_PROXY_CIDRS` 또는 mTLS
...
| `CF-IPCountry=KR`인데 451 UNKNOWN | trusted proxy secret/header 누락 또는 CIDR/mTLS 불일치 | proxy header 주입, `TRIPMATE_GEOFENCE_TRUSTED_PROXY_SECRET`, `TRIPMATE_GEOFENCE_TRUSTED_PROXY_CIDRS`, mTLS verified header 확인 |
