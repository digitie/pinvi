...
- **[admin client] T-180 — krtour admin HTTP client(API 9011 `/v1/admin/*`)**: §2.9 feature
  change + (운영자) feature-update-request 재적재 proxy를 호출하는 admin-path client.
  T-170의 user client와 base는 같고(9011) 경로/토큰 정책만 다름 — **9012(admin UI)를
  API base로 쓰지 말 것**. `tripmate_krtour_map_admin_base_url` 기본값(9012)/의미 재정의
  + 서비스 토큰.
