...
> ADR-036 이후 POI의 `feature_id`는 nullable이다. krtour-map import가 feature를 제공하면
...
> 모두 정식이다. krtour import는 krtour-map REST API만 호출하며 Python import나 DB 직접
> 접근을 쓰지 않는다.
...
### 2.5.1 krtour `curated_features` import

krtour-map `curated_features`는 TripMate의 **추가 큐레이션 소스**다. 이 흐름은
TripMate가 자체적으로 직접 만든 curated trip plan을 대체하지 않는다.

import 원칙:

- TripMate가 krtour-map REST API `GET /v1/curated-features/{id}/tripmate-copy`로
  copy snapshot을 조회한다.
