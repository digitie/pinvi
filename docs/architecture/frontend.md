...
본 문서는 TripMate v2 프론트엔드의 스택, 디자인 시스템, 코드 공유 전략을
...
| 프레임워크 (모바일) | **Expo SDK 53+** (React Native + Expo Router) | v2 후보 — 본 문서에서 구조만 박음 |
...
본 세 문서가 v1.0 디자인 톤의 단일 기준이다. TripMate 자체 브랜드가 확정되면
...
import preset from '@tripmate/design-tokens/tailwind-preset';
...
import { NoticePlanSchema, NoticePlanListSchema } from '@tripmate/schemas';
...
export const createAuthStore = (storage: StateStorage) => create(
  persist(
    ...
    { name: 'tripmate-auth', storage: createJSONStorage(() => storage) }
  )
);

// apps/web/lib/stores.ts
import { createAuthStore } from '@tripmate/state';
...
```ts
// 공용 schema
import { TripCreateSchema } from '@tripmate/schemas';
...
2. 공용 패키지 import 검증 (`packages/schemas`, `packages/api-client` 등)
3. 화면별 RN 컴포넌트 작성 — 라우트 파일명은 웹과 같게 유지
