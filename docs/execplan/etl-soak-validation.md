...
- KMA 중기전망 `wfSv` 장문 요약이 `weather_serving_mid_term.weather_summary` 255자를 초과해 실패할 수 있어 해당 serving 컬럼을 `TEXT`로 확장
...
- 2026-05-09 KST soak에서 KMA 중기전망 장문 요약이 255자를 초과해 `weather_mid_term`이 실패했다. provider 정상 데이터로 판단해 serving summary 컬럼을 `TEXT`로 넓히고 장문 fixture 테스트를 추가했다.
