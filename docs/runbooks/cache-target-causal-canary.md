...
- 매 실행은 운영자가 공급한 UUID 하나를 stable synthetic target으로 사용한다.
...
- 같은 canary ID는 crash 뒤 재개용이다. 새로운 liveness 증거에는 새 UUID를 쓴다.
...
credential이나 원격 응답 body를 포함하지 않는다. 동일 ID가 중단됐다면 같은 명령으로 재개한다.
