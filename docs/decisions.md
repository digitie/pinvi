...
- **상태**: superseded by ADR-046 (ADR-011 frontend 스택의 지도 항목 정정 — SPEC V8
  A-1 #4 채택 superseded. Kakao 폐기 / VWorld + MapLibre 채택 결정은 유효하고, 웹
  소비 패키지와 의존성 핀 방식은 ADR-046이 대체)
- **날짜**: 2026-05-26
- **결정자**: 사용자
...
## ADR-046: Web 지도 클라이언트도 `maplibre-vworld-react`의 `vworld-map-web`으로 전환
...
ADR-015는 Kakao Maps SDK를 폐기하고 VWorld + MapLibre GL JS 기반 내부 라이브러리를 웹
지도 클라이언트로 채택했다. 이후 별 저장소 `digitie/maplibre-vworld-react`가 공통 core와
웹/모바일 패키지를 함께 소유하는 구조가 됐고, 모바일은 ADR-044에서 이미 `vworld-map-rn` +
`vworld-map-core` vendored tarball 소비로 확정됐다.

사용자는 웹 지도뷰도 "maplibre vworld react"로 변경하고 기존 `maplibre-vworld-js`
의존성을 삭제하라고 지시했다. 현재 `maplibre-vworld-react`는 npm에 발행하지 않는 방침이므로,
웹도 모바일과 같은 self-contained tarball pin 방식이 필요하다.

### 결정

- `apps/web` 지도 클라이언트 의존성은 `maplibre-vworld` / `maplibre-vworld-js`가 아니라
  `maplibre-vworld-react`의 웹 패키지 **`vworld-map-web`**과 공통 패키지
  **`vworld-map-core`**로 둔다.
- `vworld-map-web`은 `apps/web/vendor/vworld-map-web-1.0.0.tgz`를 `file:` 의존으로
  핀한다. `vworld-map-core`는 Mobile이 이미 사용하는
  `apps/mobile/vendor/vworld-map-core-1.0.0.tgz` file spec을 Web도 공유해 npm workspace
  hoist가 같은 package instance를 보도록 한다. CI(`npm ci`)가 외부 release asset이나
  git archive에 의존하지 않도록 한다.
- `maplibre-gl`은 `vworld-map-web`의 peer/runtime 기반이므로 `apps/web` 의존성에 계속 둔다.
- Pinvi 앱 코드는 `apps/web/components/map/vworldPrimitives.tsx`를 단일 import facade로
  유지한다. 단, 이는 Pinvi 도메인 wrapper가 아니라 SSR/dynamic import와 타입 re-export를 모으는
  웹 앱 내부 경계다.
- VWorld 웹 키 정책은 ADR-015의 `NEXT_PUBLIC_VWORLD_API_KEY` + VWorld origin whitelist 모델을
  유지한다. 모바일 키 런타임 정책은 ADR-043/045를 따른다.
- 부족 기능이나 공통 primitive 변경은 `maplibre-vworld-react` 저장소에 이슈/PR로 처리하고,
  Pinvi에는 provider/feature 도메인 wrapper를 만들지 않는다(ADR-005 mirror).

### 근거

- 웹과 모바일이 같은 `maplibre-vworld-react` 계열(`vworld-map-core`)을 공유하면 좌표, 마커,
  클러스터, URL redaction 같은 공통 primitive drift가 줄어든다.
- vendored tarball은 npm 미발행 방침을 유지하면서 lockfile integrity와 repo self-contained
  빌드를 제공한다.
- 기존 `maplibre-vworld` dist의 Next dev `require("react")` 보정 shim은 새 `vworld-map-web`
  패키지 소비에서는 필요하지 않다.

### 결과

- `apps/web/package.json`에서 `maplibre-vworld` 의존성을 제거하고
  `vworld-map-core` / `vworld-map-web` `file:` 의존성을 추가한다.
- `apps/web/next.config.mjs`의 `transpilePackages`는 `vworld-map-core`,
  `vworld-map-web`을 포함한다.
- 웹 지도 컴포넌트는 `vworld-map-web`의 `VWorldMapView`, `ClusterLayer`, `MakiMarker`,
  `Popup`, `UserLocationMarker`, `MapContextMenu`를 사용한다.
- 기존 `maplibre-vworld/style.css`와 dev React `require` shim은 제거한다.

### 후속

- `maplibre-vworld-react` 패키지 갱신 시 `apps/web/vendor/` tarball과 `package-lock.json`을
  함께 갱신하고, source commit / checksum provenance를 통합 문서나 작업 일지에 기록한다.
- v0.1.0 release notes에는 웹 지도 의존성이 `vworld-map-web`으로 바뀐 사실을 반영한다.

### 참조

- ADR-015, ADR-044, ADR-045
- `docs/integrations/maplibre-vworld.md`
- `docs/tasks.md` T-201
...
- `cache_target.reconciled`는 target tuple을 갖지 않는 stream-scoped receipt다. source fingerprint와
  payload의 request ID/fixed snapshot ID/expected·actual Merkle root를 앞서 고정한 durable reconciliation
  expectation에 대조한다. 최신 일반 snapshot은 별도 관측값이므로 expectation을 덮어쓰지 않는다.
  count/high-watermark는 expectation의 request-bound snapshot 값을 유지하고 가짜 target key나 POI
  mutation을 만들지 않는다.
...
## ADR-062: 동의 이력은 현재 상태 테이블이 아니라 append 전용 이벤트 테이블이 갖는다

- **상태**: 채택 (2026-08-24, T-326)
- **맥락**: `app.user_consents`는 `(user_id, consent_type, version)`이 PK인 현재 상태 테이블이다.
  재동의가 같은 row를 in-place로 되살리고(`withdrawn_at → None`, `agreed_at` 덮어쓰기) 모든 화면이
  같은 버전 상수를 쓰므로, 철회했다는 사실이 남지 않는다. `docs/legal/terms-of-service.md` 제4조가
  이용자에게 "동의 이력은 시점·버전과 함께 기록된다"고 고지하므로 그 진술이 거짓인 상태였다.
- **결정**: 현재 상태 테이블은 **그대로 두고** `app.user_consent_events`(append 전용)를 신설한다.
  동의/철회가 일어날 때마다 같은 트랜잭션에서 이벤트를 남긴다.
- **대안과 기각 사유**:
  - `user_consents`를 다행(多行) append로 전환 — 소비자 계약이 깨진다. 웹·모바일 설정 화면이
    type당 **첫 행**을 고르고, 마케팅 발송 게이트는 `withdrawn_at IS NULL` 행의 **존재**만 본다.
    다행이 되면 유효한 동의가 "철회됨"으로 표시되거나 철회자에게 메일이 나간다.
  - version에 suffix를 붙여 새 row 생성 — 같은 문제에 더해 버전 표기가 오염된다.
- **범위 밖**: hash chain·append-only 트리거. 이 테이블의 목적은 법정 무결성 증명이 아니라 이벤트
  유실 방지이며, 애플리케이션에는 UPDATE/DELETE 경로가 없다. 실제 긴장은 `CASCADE` FK이므로
  사용자 하드 삭제 경로가 생기면 보존 정책을 먼저 정한다.
- **결과**: 백필(`0064`)은 현재 상태에서 복원 가능한 것만 넣는다 — 이미 덮어써진 과거 사이클은
...
## ADR-064: 좌표 범위는 사각형 둘(입력 유효 / 서비스 범위)로 두고 행정구역 폴리곤은 도입하지 않는다

- **Status**: Accepted (2026-08-24, T-331)
- **Context**: 저장소에는 오래전부터 lat 범위가 **두 개** 있었고 어느 쪽도 무엇인지 적혀 있지
  않았다 — `33~43`(`/features/nearby`, `/regions/*`, `CoordSchema`)과 `33~39.5`(`new_place` 제안
  검증, `ktm_cache_target_heads`/`poi` CheckConstraint). 이름이 없으니 서비스 지역을 물어야 할
  자리에서 입력 범위를 쓰는 일이 실제로 생겼다: T-325의 지도 자동 센터링이 `CoordSchema`로
  "국내인가"를 판정해 한반도 북단(온성 42.95)까지 통과시켰다. T-331은 "대마도가 포함되니
  폴리곤이 필요하다"로 기표됐다.
- **핵심 사실**: **어떤 위도선도 남북한을 가르지 못한다.** 개성(37.97°N)이 강원 고성(38.38°N)보다
  남쪽이기 때문이다. 즉 사각형을 어떻게 조여도 "국내인가"라는 질문에는 답할 수 없다 — 대마도는
  이 성질의 한 사례일 뿐이고, 상한 조정으로 고쳐지는 문제가 아니다.
- **Decision**:
  - 두 사각형에 이름을 붙여 정본으로 삼는다: `COORD_*`(입력 유효, 한반도 전체)와
    `SERVICE_AREA_*`(서비스 범위, lat ≤ 39.5). Python은 `app/core/coord_range.py`, TS는
    `@pinvi/schemas`의 `COORD_INPUT_BOUNDS`/`SERVICE_AREA_BOUNDS`/`isInServiceArea`.
  - 지도 센터링 판정(`isCoordInServiceArea`)은 서비스 범위를 쓴다. 이 함수는 "국내인가"를
    답한다고 주장하지 않는다 — 답하는 것은 "지도를 여기로 옮길 만한가"이고, 틀렸을 때의 결과는
    사용자가 빈 지도를 보는 것뿐이다.
  - **행정구역 폴리곤을 도입하지 않는다.** 저장소·DB에 폴리곤 데이터가 없고, 좌표 기반 차단
    기능도 없다(ADR-018의 한국 전용 차단은 IP 기반이며 좌표를 보지 않는다). 정확한 판정의
    유일한 소비자가 없는 상태에서 데이터·동기화 비용을 지불할 근거가 없다.
  - 정확한 판정이 필요해지면(좌표 기반 차단 등) 경로는 폴리곤 파일이 아니라 **kor-travel-geo
    행정구역 조회**다 — 이미 계약이 있고(`/regions/covering-point`는 미매치 404), 경계 데이터의
    정본을 그쪽이 소유한다(ADR-025/049).
  - 한계는 문서가 아니라 **테스트로 고정한다**. `mapCenter.test.ts`가 대마도·평양·개성이 통과함을
    단언하고, 개성 < 고성을 단언한다. 폴리곤이 도입되면 그 테스트가 red가 되어 이 결정을 다시
    보게 만든다.
- **Consequences**: "국내 판정"이라는 이름의 부정확한 함수가 사라진다. 두 범위의 용도가 코드에
  드러나 잘못된 쪽을 쓰는 실수가 줄어든다. 대마도·평양 오탐은 **남는다** — 그것이 이 판정의
  성질이며, 지도 배치 용도에서는 수용 가능한 오차다.
- **Alternatives**: (a) 폴리곤 도입 — 소비자가 없고 비용만 발생. (b) 상한 조정 — 개성/고성 관계
...
8. **과거 날씨는 `kor-travel-weather`의 보존 연장으로 해결한다** (사용자 결정
   ...
   **연장 목표치는 15일로 확정됐다**(사용자 결정 2026-09-17, 반영 예정 — 아직 배포되지
   않음). 그쪽 ADR-062가 선언한 3년 목표의 일부만 먼저 배포하는 것이며, 15일은 **여전히
   짧다** — 여행 종료 15일이 지난 뒤 상세를 열람·공유하면 그 날짜의 과거 날씨는 여전히
   `no_data`다. 이 잔여 한계를 감춘 채로 "과거 날씨 지원"이라고 표기하지 않는다.
   **이는 외부 저장소 소관이며 게이트 G-3으로 건다.** 두 결과를 감수한다:
