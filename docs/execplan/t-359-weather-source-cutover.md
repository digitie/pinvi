...
**2026-09-18: 전체 완료(P0~P7, T-359~T-368).** `kor-travel-weather`가 유일한
날씨 소스다. G-1(KMA 전국 커버리지)은 원래 기준 미달(~9%, 수도권·강원 위주)
상태로 사용자 결정에 따라 영구 수용하고 P6(T-365)를 진행했다 — 상세는
`docs/integrations/kor-travel-weather.md` §7-4, `docs/decisions.md` ADR-068
결정 13. 아래 본문은 실행 당시 설계 문서로 원문 그대로 남기고, 완료 후
사실과 어긋난 부분은 해당 절에 갱신 note를 덧붙인다.
...
해결한다. **목표치는 15일로 확정** — **반영 예정, 아직 미배포.** Pinvi는 스냅샷
테이블을 만들지 않는다.

- 소관: **`kor-travel-weather` 저장소**. 그쪽 ADR-062의 3년 목표 중 15일만 먼저
  배포하는 것이다.
- **15일도 완전한 해법이 아니다.** 여행 종료 15일이 지난 뒤 열람하면 그 날짜 날씨는
  여전히 `no_data`다. UI·문서에서 "과거 날씨 지원"으로 과장하지 않는다.
- **소급되지 않는다** — 이미 drop된 partition은 돌아오지 않는다. 연장 시점 이전 여행의
  날씨는 영구 소실이므로 **G-1을 기다리지 말고 지금 요청해 두는 편이 낫다.**
- Pinvi 측 강제 수단: 보존 지평 가드(게이트 I, **임계 15일**). 실효 보존이 15일
  미만이면 실패한다 — 배포 전(현재 2일)에는 항상 실패, 의도된 동작이다.
...
> **✅ 완료(T-365, 2026-09-18).** G-1은 원래 기준 미달(~9%) 상태로 사용자
> 결정에 따라 영구 수용했다 — "기본값 `on`"이 아니라 **flag 자체를 완전히
> 삭제**했다(대체 코드 경로가 없는 flag는 dead config라는 이유, 아래 계획보다
> 한 걸음 더 나갔다). 상세는 `docs/tasks-done.md` 2026-09-18 T-365 항목.
...
| I   | **보존 지평 가드(G-3)** — 실효 보존이 **15일 미만**이면 실패. 외부 설정 회귀로 과거 여행 날씨가 조용히 사라지는 것을 막는 유일한 장치 |
