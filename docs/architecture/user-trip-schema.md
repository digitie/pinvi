...
| `map_feature_id` | UUID FK nullable | N | `python-krtour-map` feature id |
...
아래 `trip_places`는 장소 전용 상세 모델링 초안이다. 현재 구현에서는 일정 타임라인의 1차 테이블을 `trip_plan_items`로 두고, 장소 전용 세부 기능이 필요해질 때 `trip_places` 또는 동등한 세부 테이블을 추가할지 다시 결정한다. feature 계약은 `python-krtour-map`의 [Feature model](https://github.com/digitie/python-krtour-map/blob/main/docs/feature-model.md)을 따른다.

| 컬럼 | 타입 | 필수 | 설명 |
| --- | --- | --- | --- |
| `id` | UUID PK | Y | 여행 장소 ID |
| `map_feature_id` | UUID FK nullable | N | `python-krtour-map` feature id. 지도 클릭/수동 입력처럼 아직 표준 feature가 없으면 비워둘 수 있음 |
| `trip_id` | UUID FK | Y | 여행 |
...
ETL로 적재된 장소/관광지/휴게소/주유소 등 내부 데이터와 여행 장소를 연결한다. feature로 정규화된 데이터는 우선 `trip_places.map_feature_id -> python-krtour-map features.feature_id`로 연결한다. 이 테이블은 아직 표준 feature로 승격되지 않은 개별 ETL row, 보조 데이터, 과거 provider 참조를 남길 때 사용한다.
...
이 테이블은 FK를 강제하지 않는 느슨한 참조로 시작한다. 여러 ETL 테이블을 하나의 FK로 묶기 어렵고, 데이터셋별 갱신/교체 주기가 다르기 때문이다. 대신 service layer에서 `dataset_key + ref_table + ref_id` 유효성을 검증한다. 장기적으로 사용자에게 반복 노출될 장소성 데이터는 `python-krtour-map` feature로 승격하고, `trip_place_data_refs`는 보조 연결로 남긴다.
...
7. `trip_place_data_refs`는 표준 feature로 승격되지 않은 데이터셋 row의 보조 연결로 남긴다. 표준 feature와의 주 연결은 `trip_places.map_feature_id`를 사용한다. 주요 ETL 테이블별 명시 FK 테이블을 추가할지는 데이터셋별 사용량이 늘어난 뒤 결정한다.
