...
| 프레임워크 (모바일) | **Expo SDK 53+** (React Native + Expo Router) | `apps/mobile` 구조 스캐폴드 존재(비활성, Sprint M-1 활성화) — ADR-041 |
...
| **Expo `apps/mobile/` 활성화** | (v1.0 후, v2 단계) | 별도 Sprint M-1 (Mobile) — workspaces 등록 + install + 화면 구현 + EAS |
