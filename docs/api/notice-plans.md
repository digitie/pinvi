...
> ADR-036 이후 POI의 `feature_id`는 nullable이다. 외부/agent 연계가 feature를 제공하면
> 같은 curated plan의 feature-backed POI를 재사용하고, 없으면 새 POI를 만들어 연결한다.
> 생성 소스는 TripMate-native 큐레이션과 krtour-map `curated_features` 1:1 import가
> 모두 정식이다. krtour import REST 상세 계약은 아직 미확정이므로 본 문서는 후속 설계만
> 기록한다.
...
- krtour feature schema와 FK는 만들지 않는다. feature 최신 정보는 필요 시
  `/features/{feature_id}`로 조회한다.

### 2.5.1 krtour `curated_features` import (후속, 미구현)

krtour-map에 `curated_features` 기능이 추가되면 TripMate는 이를 **추가 큐레이션 소스**로
소비한다. 이 흐름은 TripMate가 자체적으로 직접 만든 curated trip plan을 대체하지 않는다.

후속 import 원칙:

- TripMate가 krtour-map REST API로 `curated_features` 상세를 조회한다.
- krtour curated feature 1건을 TripMate `curated_trip_plans` 1건으로 1:1 복사한다.
- 하위 항목/POI는 `curated_plan_pois`로 복사한다.
- krtour 항목이 `feature_id`를 제공하면 feature-backed POI로 저장하고, 없으면
  `feature_id = null`인 자유 curated POI로 저장한다.
- 같은 plan 안에 이미 같은 `feature_id`의 active POI가 있으면 재사용하고, 없으면 새로 만든다.
- 출처 추적이 필요하면 REST 계약 확정 후 plan/POI에 `source_system`,
  `source_curated_feature_id`, `source_curated_feature_version`,
  `source_curated_feature_item_id` 같은 컬럼을 별도 migration으로 추가한다.

예상 TripMate Admin endpoint(계약 확정 전 제안, 현재 구현 아님):

```http
POST /admin/notice-plans/imports/krtour-curated-features
Content-Type: application/json

{
  "curated_feature_id": "krtour-curated-feature-id",
  "mode": "create",
  "is_published": false
}
```
...
```jsonc
{
  "data": {
    "notice_plan_id": "uuid",
    "source_system": "krtour-map",
    "source_curated_feature_id": "krtour-curated-feature-id",
    "copied_poi_count": 8,
...
실제 path, request/response 필드, idempotency/refresh 정책은 krtour
`curated_features` OpenAPI 계약이 확정된 뒤 본 문서와 구현을 함께 갱신한다.
