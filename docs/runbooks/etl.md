...
```bash
wsl.exe -e bash -lc "cd /mnt/f/dev/mapplan && docker compose -f infra/docker-compose.yml up -d postgres dagster"
```
...
```bash
wsl.exe -e bash -lc "cd /mnt/f/dev/mapplan && docker compose --env-file .env -f infra/docker-compose.yml exec -T dagster dagster job list -m app.dagster_etl.definitions"
```
...
```bash
wsl.exe -e bash -lc "cd /mnt/f/dev/mapplan/apps/api && TRIPMATE_LIVE_ETL_TESTS=1 uv run pytest -q tests/test_dagster_etl.py -m live"
```
...
```bash
wsl.exe -e bash -lc "cd /mnt/f/dev/mapplan/apps/api && uv run python -m app.cli.vworld_boundary /path/to/N3A_G0010000.zip /path/to/N3A_G0100000.zip /path/to/N3A_G0110000.zip"
```
