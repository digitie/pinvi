...
│   ├── lib/                    # 웹 전용 어댑터 (vworld-map-web, next-intl 등)
...
> **앱 제작 추가구현**: `apps/mobile`을 실제로 만들기 위해 남은 항목(화면별 RN 구현,
> 지도 `maplibre-vworld-react` 의존, 백엔드 선결, 구현 순서)은
> [`docs/architecture/expo-implementation-plan.md`](expo-implementation-plan.md)가 정본이다.

### 6.1 v1.0 기본 출시 범위 — Web/API/Admin

- `apps/web` 본격 구현
- `packages/*`를 **처음부터 공용 코드 위치로 유지** — 웹 전용 코드 (next-intl,
  next-router, vworld-map-web adapter)는 `apps/web/lib/`에만
- `apps/mobile/`은 활성 Sprint M-1 track이다. 다만 `v1.0.0`은 Web/API/Admin 운영 출시로
  고정하고, 모바일 EAS build·실기기 smoke·store 제출·mobile live e2e는 `v1.0.0` release
  blocker에서 제외한다(T-284). 모바일 범위와 남은 gate는 `apps/mobile/README.md`가 정본이다.
- **(2026-06-15 갱신: 모바일 런타임 기준선은 ADR-043.)** Expo Dev Client + EAS Build를
  ...
  `minSdkVersion >= 24`을 기준으로 둔다.

### 6.2 Expo 추가 구현 시 (Sprint M-1)

순서:

1. `apps/mobile/` Expo Router 초기화 + `package.json` workspaces 등록
2. Linux worktree에서 `npm install` + `expo install --check`로 Expo SDK 56 호환 버전 정합
...
- New Architecture는 SDK 56(RN 0.85) 기본 — `app.json` `newArchEnabled` flag는 SDK 56에서 제거됨.
- Android 최소 SDK는 `expo-build-properties`의 `android.minSdkVersion = 24`으로 고정한다.
...
- `docs/architecture/expo-implementation-plan.md` (Expo 앱 추가구현 계획 — 화면/지도/백엔드 선결)
