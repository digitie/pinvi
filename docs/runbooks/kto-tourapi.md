...
- `TourApiRateLimitError`: 호출 빈도를 줄이고, 같은 request params에 대한 cache hit 여부를 확인한다.
...
- `TarRlteTarService1` 결과는 현재 generic raw record 기반이다. typed model이 필요하면 TripMate에 래퍼를 만들지 말고 `pykrtourapi`에 upstream한다.
