...
구현: `sensors.py`(`pinvi_run_failure_sensor` — ADR-050 실패 통지, T-291).
계획(미구현, `app` schema 소유 job 후보): `pinvi_telegram_weekly`(D-11/D-1). POI 출몰시각은
별도 asset 파일이 아니라 `jobs.py`의 one-shot job(`kasi_poi_rise_set_job`)으로 구현돼 있다.
...
`default_status=RUNNING`)는 retry가 모두 소진된 실패를 `app.telegram_system_notification_outbox`
(status=pending → 기존 outbox worker가 전송)로 적재하고, `SENTRY_DSN`이 설정돼 있으면 Sentry로도
통지한다(Sentry는 선택, outbox가 durable 경로). payload는 PII/secret-free — `job_name`/`run_id`/
예외 **클래스명**만 담고 예외 message·stack은 제외한다(T-291). 개별 asset/job은 Telegram을 직접
호출하지 않는다. 외부 API 호출 실패 시 서비스키나 요청 URL의 `serviceKey` 값은 마스킹한다.
