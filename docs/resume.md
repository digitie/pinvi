...
request 계약 테스트와 현재 문서를 함께 재고정한다. Map #1054의 full/admin 변경은 service
bytes를 바꾸지 않으므로 service provenance revision은 #1051을 가리킨다. 새 exact pair의
CI·두 전문 적대 리뷰·격리 live proof 전에는 receipt를 `complete`로 올리거나 production
sync를 켜지 않는다.

**다음 한 작업**: draft PR의 CI와 두 전문 리뷰를 통과시킨 뒤, Map #1054와 이 PinVi service
vendor를 함께 반영한 T-VN-41C paired acceptance 전제와 남은 M05 activation 조건을 대조한다.
