...
def test_bootstrap_only_accepts_the_declared_postgres_endpoints() -> None:
    bootstrap = (ROOT / "infra" / "postgres" / "bootstrap-pinvi-runtime-role.sh").read_text(
        encoding="utf-8"
    )
    ...
    assert "app-postgres|127.0.0.1" in bootstrap
    assert "PINVI_DB_HOST must be app-postgres or 127.0.0.1" in bootstrap
    assert "PINVI_DB_PORT must be a decimal TCP port" in bootstrap
    ...
    for override, expected_error in (
        ({"PINVI_DB_HOST": "db.example.test"}, "PINVI_DB_HOST must be"),
        ({"PINVI_DB_PORT": "0"}, "PINVI_DB_PORT must be"),
        ({"PINVI_DB_PORT": "65536"}, "PINVI_DB_PORT must be"),
    ):
