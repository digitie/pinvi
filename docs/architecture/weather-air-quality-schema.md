...
AirKorea 대기오염정보 API는 예보통보와 시도별 측정값이 같은 서비스 묶음이므로, 현재 기본 주기는 합산 약 420회/일이다. 여기에 retry가 발생하면 일 500회를 넘을 수 있으므로 운영에서는 반복 실패 시 job 일시정지 또는 주기 완화를 먼저 검토한다.
...
PostGIS 경계가 아직 적재되지 않은 초기 환경에서는 날씨 격자 mapping이 0건일 수 있다. 이 경우 job는 API를 호출하지 않고 0건 적재로 종료할 수 있다. 운영에서는 VWorld 시군구/법정동 경계 적재 후 날씨 job를 활성화한다.
...
- `weather_mid_term`은 공식 중기예보 구역 seed, 주소 코드 mapping table, raw/serving schema, loader, Dagster job까지 구현했다. 다만 `config/kma-mid-term-regions.json`의 수동 seed는 운영 전 공식 최신 코드표와 한 번 더 대조해야 한다.
...
- 기상청 관광코스별 상세 날씨 API는 사용자 저장 장소 또는 여행 장소 주변 target을 받아 cache/수집하는 loader까지 구현했다. 아직 사용자 저장 장소/여행 장소 도메인과 Dagster 자동 연결은 없다.
...
- Dagster job schedule/retry 계약
- Alembic migration 문자열 계약

## 2026-05-16 feature weather 병합 보강

feature에 붙는 날씨/환경 값은 KMA의 초단기실황, 초단기예보, 단기예보, 중기예보 스타일을 기준 timeline으로 삼는다. 다른 provider의 날씨성 데이터는 별도 중간 계층 없이 각 `python-*-api` 라이브러리 공개 client를 직접 호출하고, `python-krtour-map`의 `WeatherValue`와 feature DB 함수로 저장한다.

| provider library | domain | 저장 기준 |
| --- | --- | --- |
| `python-kma-api` | `kma_ultra_short_nowcast`, `kma_ultra_short_forecast`, `kma_short_forecast`, `kma_mid_forecast` | 전체 시간축의 기준. `nowcast`, `ultra_short`, `short`, `mid` 사용 |
| `python-krex-api` | `rest_area_weather` | 휴게소 feature에 붙는 observed/short context |
| `python-krairport-api` | `airport_weather` | 공항 feature의 운항 주변 날씨와 세계/공항 날씨 context |
| `python-visitkorea-api` | `tourist_spot_weather` | 관광지 상세 날씨를 KMA 단기/중기 timeline에 보강 |
| `python-airkorea-api` | `air_quality` | 주변 측정소 또는 시도 단위 대기질을 feature context로 보강 |
| `python-khoa-api` | `beach_marine` | 해수욕장/해양 feature의 파고, 수온, 조석, 해양지수 |

provider 원문과 response hash, feature 연결, provider별 cursor와 재시도 상태는 `python-krtour-map` feature DB 계약에 둔다. TripMate는 별도 feature DB를 만들지 않고 해당 schema와 함수를 import해 사용한다.

## 2026-05-17 weather feature 정규화 기준

`forecast_style` 하나로 KMA식 초단기/단기/중기와 provider 원천 성격을 동시에 표현하지 않는다.

- `forecast_style`: `observed`, `nowcast`, `ultra_short`, `short`, `mid`, `index`, `advisory`처럼 원천값의 성격
- `timeline_bucket`: 앱 조회/표시용 KMA식 카테고리. `ultra_short`, `short`, `mid`

예를 들어 한국도로공사 휴게소 최신 날씨는 예보가 아니므로 `forecast_style='observed'`를 유지하고, 현재/초단기 화면에서 쓰기 위해 `timeline_bucket='ultra_short'`로 저장한다. 산불위험·산사태 위험은 `forecast_style='index'` 또는 `advisory`, `timeline_bucket='short'`로 둔다. KMA 중기예보처럼 실제 3-10일 전망을 제공하지 않는 source는 `mid`로 승격하지 않는다.

`python-krtour-map` weather value 계약의 핵심 필드:

- `timeline_bucket`: `ultra_short`, `short`, `mid`
- `valid_from`, `valid_until`: 특보/위험지수/일 단위 전망의 유효 구간
- `source_metric_key`, `source_metric_name`: provider 원천 metric/code
- `normalization_version`: mapping 규칙 버전. 초기값은 `weather-feature-v1`

Provider별 weather feature 1차 검토:

| provider library | source | domain | style/bucket | 검토 |
| --- | --- | --- | --- | --- |
| `python-kma-api` | 초단기실황/초단기예보/단기/중기 | `kma_*` | endpoint 그대로 | KST 발표시각, DFS grid, 중기 `regId/stnId`, category 단위 |
| `python-krforest-api` | `MountainWeather` | `forest_mountain_weather` | `observed` / `ultra_short` | 관측소 좌표/고도, feature와 거리, 강수/풍속/기압 단위 |
| `python-krforest-api` | 산불위험/산사태 | `forest_fire_risk`, `forest_landslide_risk` | `index|advisory` / `short` | 분석시각, 행정구역 granularity, 위험단계 code/name |
| `python-krex-api` | `RestAreaWeather` | `rest_area_weather` | `observed` / `ultra_short` | `observed_at`, 휴게소 `unit_code`, 노선/방향, `-99` sentinel |
| `python-krairport-api` | `WorldWeather` | `airport_weather` | `observed` / `ultra_short` | 항공편 상대 도시 날씨와 국내 공항 자체 날씨를 구분 |

추가 검토할 정부기관 source:

- 산림청 국립산림과학원 산악기상정보: 산/휴양림/트레일 feature에 맞는 실시간 관측.
- 산림청 산사태예측정보: 호우 이후 산행/계곡/휴양림 안전 알림.
- 기상청 국내 공항기상정보: 국내 공항 자체 weather source로 우선 검토.
- 해양수산부 국립해양조사원 조위관측소 수온/기온/기압, 국립해양측위정보원 해양기상: 해수욕장/항구/섬 feature 보강.
- 농촌진흥청 농업기상 관측지점: 농촌/체험마을 미기상 후보. 트래픽 제한이 작으므로 후순위.
- 한국수자원공사 수문 운영 정보: 댐/보/강변/계곡 여행 안전 context. weather가 아닌 `hydro_weather` 성격으로 분리.
