...
> **2026-06-10 재대조 (krtour `origin/main` `0e45bd7`)**: krtour **ADR-048/T-216a~g 전부
...
> 추가 정정: **krtour admin API는 :12301 `/v1/admin/*`**이다. 본문/`config.py`는
> admin API base도 `12301`을 기본값으로 둔다.
> 결정 반영: 재적재 vs 사용자 제안 분리 흐름(§2.8/2.9)은 krtour **ADR-051(2026-06-10)**로
> 공식 승인됐고, 잔여 합의 5건(§7)은 krtour T-217c가 확정해 회신 예정.
> **2026-06-12 추가**: krtour-map `curated_features`는 TripMate
> `curated_trip_plans`의 추가 import 소스다. TripMate-native 큐레이션을 대체하지 않는다.
...
**구현 완료**됐다. krtour-map은 `packages/krtour-map-admin`(FastAPI, 포트 **12301**)에
TripMate-facing `openapi.user.json`을 export하고, 다음 사용자 표면 엔드포인트를 제공한다:

| 능력 | 엔드포인트 | 직전 상태 → 현재 |
|------|-----------|------------------|
| bbox + 클러스터 | `GET /features/in-bounds` | 클러스터 미지원 → **서버 클러스터(`cluster_unit`) 지원** |
| 단건 상세 | `GET /features/{feature_id}` | ✅ |
| **배치** | `POST /v1/features/batch` (구 `/tripmate/features/batch`) | ❌ → **✅(cap ≤200, 응답 `data.found`)** |
...
| 공개 해수욕장 | `GET /v1/public/beaches*` | krtour T-222b → **✅(목록·상세·marker)** |
| 공개 축제 | `GET /v1/public/festivals*` | krtour T-222b → **✅(월별 목록·상세·marker)** |
| feature 갱신요청 | `POST /v1/admin/feature-update-requests` + `GET .../{id}` (krtour 운영자 전용, §2.8) | ❌ → **✅** |
| provider 신선도 | `GET /providers/{provider}/last-sync` | — → ✅ |
| health/version | `GET /health`, `GET /version` | — → ✅ |

TripMate는 feature read cutover와 drift gate를 완료했고, 2026-06-12에는 public
beach/festival 표면도 소비 측에서 연결했다(T-130). 남은 큰 cross-repo 소비 작업은 krtour
`curated_features` REST 계약 확정 후 TripMate `curated_trip_plans` 1:1 import(T-211)다.
...
- **Base URL**: 로컬 `http://127.0.0.1:12301` — **admin/ops API 포함 전 표면이 :12301**
  (`/v1/admin/*`·`/v1/ops/*` path로 구분, TripMate Admin 프록시 전용·사용자 경로 미노출).
  TripMate 설정은 `TRIPMATE_KRTOUR_MAP_API_BASE_URL`과
  `TRIPMATE_KRTOUR_MAP_ADMIN_BASE_URL` 모두 기본값을 `12301`로 둔다.
- **버전 prefix**: **외부 전 표면 `/v1` (라이브, 2026-06-09)** — krtour PR #318/#319/#321이 `/v1`
  clean cut + batch `/tripmate/features/batch`→`/v1/features/batch` + 파라미터 개명을 머지
  (`openapi.user.json` `krtour-map-user 0.2.0-dev`). `/health`·`/version`만 비버전. **TripMate T-170
  ...
  ※ ~~problem+json 에러 + envelope payload/meta 분리는 krtour 미머지~~ → **2026-06-10
  krtour `0e45bd7`에서 머지 완료** — client의 `{error:{code}}`·`data.next_cursor` 파싱은
...
- **응답 envelope (확정 — krtour 0e45bd7 라이브)**: 성공 = `{ "data": <payload>, "meta": <Meta> }`.
...
- **에러 envelope (확정 — RFC7807 라이브)**: `Content-Type: application/problem+json`,
  ...
  `LOCK_BUSY`(409 + `Retry-After`), `UPSTREAM_UNAVAILABLE`(503). TripMate client
...
- **by-target**: `external_system* target_key*`(등록된 POI cache target) + `radius_km`.
  TripMate POI를 cache target으로 등록하면(krtour `PUT /v1/admin/poi-cache-targets/...`,
...
- "특정 scope를 다시 적재(Dagster job)"하는 **krtour-map 운영자 기능**. krtour-map admin
  콘솔에서 운영. **TripMate 일반 사용자 비노출, 사용자 제안 흐름과 무관.**
  TripMate 제품은 surface하지 않는다.
...
- ① **사용자 제안 큐** (user 도메인, T-177 완료): `app.feature_suggestions` +
  ...
  rate-limit/dedup. **krtour 직접 호출 X.**
...
**TripMate Admin 도메인 전용**(`require_admin_destructive_enabled` + 서비스 토큰 —
**API base는 :12301 `/v1/admin/*`**). 사용자 제안 승인 시 호출.
`place`/`event`만 대상. **krtour ADR-051(2026-06-10)이 이 흐름을 전송 구간 정본으로
...
- provider 신선도(brief/Admin 상태판), liveness, 버전. TripMate Admin 상태판·헬스 체크용.
  `/health`·`/version`만 비버전 경로 (구 `/debug/health|version`은 krtour T-214h로 제거).
...
| 좌표(목록) | 평면 `lon`/`lat` | ✅ T-182 완료(2026-06-09) — TripMate 정본도 `lon`/`lat` 채택 | 잔여: 구모델 매핑(`coord.longitude`) 제거는 T-173 라우터 cutover에서 |
...
- ✅ **T-170/T-171 완료(PR #102)**: `apps/api/app/clients/krtour_map.py` httpx client(계약
  ...
  `tripmate_krtour_map_*` 배선. user API(12301) 소비 준비됨.
...
- admin 도메인 feature change client는 미구현(T-180, PR #317로 대상 API 생김).
  **base는 API 12301 `/v1/admin/*`** 이며, `tripmate_krtour_map_admin_base_url`도
  같은 기본값을 사용해야 한다.
...
3. **cache-target write 위치**: `/v1/admin/poi-cache-targets/*`(admin 표면)로 확정 —
   `/tripmate/*` namespace는 krtour에서 **제거됨**(krtour는 TripMate 전용이 아님).
   by-target nearby 쓸 때 등록 흐름은 admin flow로 협의.
4. **DEC-05 — K-15 해소(krtour PR #317)**: feature change API(`POST/PATCH/DELETE
   /v1/admin/features*`) 신설됨 → T-179 완료. **§7 합의 5건 ✅ 확정(krtour T-217c, 2026-06-11,
   krtour `decisions.md` ADR-051)** + TripMate 반영 완료:
   1. **review_mode**: 기본 `require_review` 2단 검토(TripMate 1차 + krtour 운영자 최종). →
      TripMate는 record status `applied`→`added`, 그 외→`approved`.
   2. **idempotency_key** = `suggestion_id`(request_id) → krtour `make_feature_id(user_request,
      idempotency_key)`로 결정적 feature_id, 재시도 동일.
   3. **출처 태깅**: operator 고정 `"tripmate-admin"`(admin id 미노출, 익명 D-11) + reason
      `[suggestion:<request_id>]` prefix(change-requests 큐가 출처 표시).
   4. **admin 인증**: 12301 `/v1/admin/*`, 코드 인증은 krtour `admin_destructive_enabled`
...
- **[F] T-175 — trip view 배치 연결**: `GET /trips/{id}`에 `trip_view_builder` 연결(감사 C-05) +
  ...
  fallback 매핑 (inactive feature는 `found`+status로 옴 — "철회/폐업" 표시 분기, krtour D-12).
...
- **[H2] ✅ T-179 백엔드 — Admin 검사/승인 → krtour feature change(DEC-05, admin 도메인)** (완료
  ...
  **`POST/PATCH/DELETE /v1/admin/features*`** 호출(suggestion_type별), 결과 `feature_id`/
  `request_id`/state를 `feature_suggestions.krtour_ref`에 저장(status `added`/`approved`). krtour
  호출 먼저 → 성공 시에만 commit(실패 시 pending 유지). idempotency_key=request_id, 출처 태깅
  operator 고정 `"tripmate-admin"` + reason `[suggestion:<id>]` prefix(§7 #3 확정·익명 D-11).
  ⚠️ 재적재(feature-update-request)와 **무관**. **web 검토 UI 완료**
  (`apps/web/app/(admin)/admin/feature-requests/page.tsx` — 검토 큐 + 승인/거절 패널).
  **§7 합의 5건 ✅ 확정(krtour T-217c, 2026-06-11) + TripMate 반영 완료** — §7 참조.
- **[admin client] ✅ T-180 — krtour admin HTTP client(API 12301 `/v1/admin/*`)** (완료 2026-06-11):
  `apps/api/app/clients/krtour_map_admin.py` — `KrtourMapAdminClient` (create/patch/delete_feature
  → `data.request` + change-requests list/approve/reject, 재시도·도메인 예외는 user client 재사용).
  base = :12301 `/v1/admin/*` (`tripmate_krtour_map_admin_base_url` 기본값도 12301),
  `X-Krtour-Service-Token`(`tripmate_krtour_map_admin_service_token`, 미설정 시 공용 토큰 fallback).
  lifespan/`get_krtour_map_admin_client` 의존성 + MockTransport 계약 테스트. **승인 시 호출 배선은 T-179.**
...
- **[표준 추종] T-181 — ADR-048(krtour PR #316) 표준 추종 (hard cutover lockstep)**: krtour
  ...
  **선행**: §7 ADR-048 A~F 수렴 완료(df69057) + 잔여 1(batch `items`→`found`) — **krtour
  `0e45bd7`(2026-06-10)에서 전부 머지·해소됨 → 대기 해제, 즉시 실행 가능.**
  ※ 2026-06-09 재리뷰로 "무중단 이중지원" 전제 **철회**.
  ✅ **2026-06-10 완료(client 계층)**: `apps/api/app/clients/krtour_map.py` — (2) `_error_code`
  ...
  batch `data.found` 파싱. 계약 테스트 `tests/unit/test_krtour_map_client.py`(15). **이 client는 아직
...
- **ADR-048 3차 검토 — 잔여 정합성 2건 → ✅ 전부 수용·머지됨(krtour 0e45bd7, 2026-06-10)**:
...
수기 httpx client(krtour 권고)가 krtour `openapi.user.json`과 silent drift하는 것을 막는다.

- **vendor 스냅샷**: `apps/api/tests/contract/krtour-openapi-user.json` — TripMate가 구현 기준으로
  삼은 krtour user 스펙 pin.
- **계약 테스트**: `apps/api/tests/unit/test_krtour_contract.py` (CI `pytest tests/unit`에서 실행) —
  ...
  (3) 로컬 전용: sibling `python-krtour-map` 스펙과 경로 집합 일치
  (핀 신선도, CI에서는 skip — `TRIPMATE_KRTOUR_OPENAPI_USER_PATH`로 override 가능).
- **갱신 절차** (krtour 스펙 변경 시):
  1. `cp ../python-krtour-map/packages/krtour-map-admin/openapi.user.json
     apps/api/tests/contract/krtour-openapi-user.json`
  2. `pytest apps/api/tests/unit/test_krtour_contract.py` 실행.
  3. 실패하면 사라진/바뀐 경로·필드를 `clients/krtour_map.py` + `features.py`/`public.py` 매핑 +
     `_CLIENT_PATHS`/`_SCHEMA_FIELDS`에 맞춰 갱신(= krtour drift 대응 PR).
- **codegen(선택)**: frontend `openapi-typescript` + Zod mirror는 미도입(후속). 백엔드는 본
  스냅샷 게이트로 충분(krtour 권고: 수기 httpx 유지).
