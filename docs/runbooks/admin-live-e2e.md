...
- UI 기준 live matrix 6,195건 catalog, 로그인 검증 2개, catalog drift 1개, Debug live
  read-only 1개, Grafana live read-only 1개, Backup live read-only 1개, Map marker parity
  read-only 1개를 실행한다.
...
PINVI_ADMIN_LIVE_E2E=1 PINVI_ADMIN_LIVE_CASE_LIMIT=200 npm run test:e2e:admin-live
PINVI_ADMIN_LIVE_E2E=1 PINVI_ADMIN_LIVE_CASE_LIMIT=2000 npm run test:e2e:admin-live
PINVI_ADMIN_LIVE_E2E=1 npm run test:e2e:admin-live
```
