...
| 보존             | 3년 immutable                                | **기본 2일** (day partition drop)                               |
| 기상청(KMA) 예보 | 전국 격자 자동 생성                          | **현재 1개 지점뿐** — §1.2 차단 사유                            |

**한 줄 요약**: 계약 정합은 어렵지 않다. 막는 것은 **데이터 커버리지(G-1)**와
**개인정보 처리방침 정합(G-2)**이다.
...
- **보존 기본 2일**(`KOR_TRAVEL_WEATHER_RETENTION_DAYS=2`, `known_at` day partition
  통째 drop). 3년 immutable인 현재 소스와 근본적으로 다르다 → §4.2.
...
| 안  | 내용                                                 | 비용                        | 평가                    |
| --- | ---------------------------------------------------- | --------------------------- | ----------------------- |
| A   | 과거 날씨 미지원 (`no_data`)                         | 0                           | 기록 기능 후퇴 — 비권장 |
| B   | **Pinvi가 여행 일자 카드를 확정 시점에 스냅샷 저장** | 테이블 1개 + 저장 시점 정의 | **권장**                |
| C   | `kor-travel-weather` 보존 연장 요청                  | 외부 저장소 + 스토리지      | 병행 가능               |

**권장 B**: 여행 날짜가 지나면 그날 카드는 더 변하지 않으므로, Pinvi가
`app.trip_day_weather_snapshots`에 확정 저장한다. 읽기는 "과거면 스냅샷, 현재/미래면
라이브"로 갈린다. 저장 주체가 Pinvi이므로 외부 보존 정책에 종속되지 않는다.

> 참고: `kor-travel-map`의 `docs/etl/weather-feature-normalization.md`는 "PinVi는 별도
> weather DB를 만들지 않는다"고 적고 있다. 그 문장은 **map을 소스로 쓰는 전제**에서
> 쓰였다. 소스가 바뀌면 전제가 사라지므로 ADR-068이 이를 명시적으로 대체한다.
...
| P6   | **G-1 · G-2 해소 확인 후** 기본값 `on` + map weather 소비 제거 | flag `off` 복귀 |
| P7   | 과거 날씨 스냅샷 (§4.2 결정 후)                                | 테이블 additive |

**G-1(기상청 커버리지)과 G-2(처리방침 정합)가 P6 진입 게이트**다. P1~P5는 두 게이트와
무관하게 진행 가능하다. P7은 §7-1 사용자 결정에 달려 있다.
...
1. **과거 날씨 정책** — §4.2의 A / B / C. 권장 B.
2. **상용 provider 표시 허용 여부** — 기상청이 없는 지점에서 OpenWeatherMap 등을
   "날씨"로 표시할 것인가, `no_data`로 둘 것인가.
   **이것은 UX 질문이기 전에 법적 질문이다(G-2).** 표시하기로 하면
   `docs/compliance/data-policy.md`의 위탁 목록과 "국외 이전 의무 발생 안 함" 선언을
   먼저 고쳐야 한다. 처리방침 정비 없이 표시하는 선택지는 없다.
   권장: **처리방침을 먼저 갱신하고, 표시하되 출처를 카드에 명시**.
3. **`kor-travel-map` 쪽 정합** — **`kor-travel-map` ADR-062**는 "weather는 feature의
   속성"이라고 결정했고 weather 투자가 진행 중이다(T-VN-38/39). Pinvi가 소비를 끊는 것은 map의
   결정을 되돌리는 게 아니라 **소비자 하나가 빠지는 것**이다. 다만 map에 그 사실을
   알려 중복 투자를 막을지 여부는 사용자 판단이다.
