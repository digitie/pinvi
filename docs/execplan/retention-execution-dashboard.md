...
- `admin_audit_log` PII 후보는 PII 삭제 후보와 분리된 `audit_retention` summary로 집계한다.
  append-only 감사 원장의 법정/감사 성격 때문에 삭제하지 않고 result의
  `skipped_admin_audit_pii_over_retention`으로 기록한다.
