...
4. **DEC-05 — K-15 해소(krtour PR #317)**: feature change API(`POST/PATCH/DELETE
   /v1/admin/features*`) 신설됨 → T-179 actionable. **잔여 합의(§7)**: review_mode
   (require_review vs immediate), idempotency_key 멱등성, 출처 태깅, closure(DELETE vs
   deactivate), admin 인증 — **krtour ADR-051/T-217c가 확정해 회신 예정** (2026-06-10).
...
- **[H2] ✅ T-179 백엔드 — Admin 검사/승인 → krtour feature change(DEC-05, admin 도메인)** (완료
  ...
  **`POST/PATCH/DELETE /v1/admin/features*`** 호출(suggestion_type별), 결과 `feature_id`/
  ...
  operator=`tripmate-admin:{id}`. ⚠️ 재적재(feature-update-request)와 **무관**. **web 검토 UI 완료**
  (`apps/web/app/(admin)/admin/feature-requests/page.tsx` — 검토 큐 + 승인/거절 패널).
  §7 합의 5건(review_mode/idempotency/출처태깅/admin인증/closure)은 문서화된 기본값 가정(T-217c 확정 시 조정).
