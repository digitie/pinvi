...
krtour-map 비의존 작업 루프 기준으로 T-144 여행/장소 검색 UX + 내보내기 설계를
처리한다.
...
2. **다음 비의존 후보** — T-144 여행/장소 검색 UX + 내보내기(PDF/GPX/print) 설계.
   krtour-map feature read 구현 없이 UX/API 계약 문서를 먼저 정리한다.
