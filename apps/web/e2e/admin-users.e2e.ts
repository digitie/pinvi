...
test('Admin 사용자 목록이 검색어와 상태 필터를 API에 전달한다', async ({ page }) => {
  ...
  await page.route(
    (url) => url.port === '12501' && url.pathname === '/admin/users',
    async (route) => {
  ...
  expect(listRequests.some((url) => url.includes('12301'))).toBe(false);
});
...
test('Admin 사용자 상세가 사유와 함께 이메일 원본 조회 audit을 표시한다', async ({ page }) => {
  ...
  await page.route(
    (url) => url.port === '12501' && url.pathname === `/admin/users/${targetUserId}`,
    async (route) => {
      await route.fulfill({
        contentType: 'application/json',
        body: JSON.stringify({
  ...
  await page.route(
    (url) => url.port === '12501' && url.pathname === `/admin/users/${targetUserId}/reveal-pii`,
    async (route) => {
