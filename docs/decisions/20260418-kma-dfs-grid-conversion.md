...
Superseded by `docs/decisions/20260507-python-kma-api-weather-client-boundary.md`
...
- 2026-05-07부터 로컬 `apps/api/app/geospatial/kma_grid.py` 구현은 제거하고, 공용 라이브러리 `python-kma-api`의 `wgs84_to_kma_grid()`와 `kma_grid_to_wgs84()` 공개 API로 대체한다.
...
- 날씨 source와 Dagster job은 외부 geocoder 저장 없이 장소 좌표를 기상청 격자로 변환할 수 있다.
