...
```
전체 location                  1,432
KMA 격자 앵커(nx is not null)     128   (2026-09-17 대비 1 → 128, 128배 증가)
```

`nx`/`ny`가 새로 채워진 128개는 새 location이 아니라 **기존 AirKorea/KHOA
측정소에 KMA 격자 좌표를 소급 부여**한 것이다(`region_code` 컬럼에 담긴 주소를
보면 경기·강원 지역에 집중돼 있다). 서울시청 `/resolve`는 이제 KMA 단기예보
490행을 받는다(2026-09-17에는 다른 location에서만 왔다) — **하지만** 부산
해운대(`khoa-BCH001`)는 여전히 `source_locations`가 자기 자신 하나뿐이고 KMA
...
**🟡 2026-09-18 재실측 — 진행 중, 아직 15일 미달.** T-367 가드와 같은 방법으로
N150 `kor-travel-weather-db-1`을 직접 조회(`weather_values` 파티션, `known_at`
전역 min/max):

```
가장 오래된 살아있는 known_at   2026-09-08 11:18 UTC
지금(N150 서버 시각)            2026-09-18 00:57 UTC
실효 보존                       약 9.6일
```

`weather_values` 파티션이 2026-09-02까지 미리 생성돼 있지만 09-02~09-07은
**0행**(빈 스캐폴드) — 보존 연장 배포가 09-08 무렵 실제로 걸리기 시작했다는
뜻이다(그 이전 데이터는 옛 2일 보존에서 이미 drop돼 소급되지 않는다, §4.2
대가 1과 정확히 일치하는 관측). 매일 자연히 하루씩 늘어나므로 이 추세면
**2026-09-23 무렵 15일에 도달**할 것으로 보인다. **게이트 G-3도 부분 진행 —
목표는 배포됐지만 "15일 이상"이라는 기준은 아직 실측으로 통과하지 못한다.**
T-367 가드(`pinvi_weather_retention_horizon_guard`)가 배포되면 이 판정을 매일
자동으로 대신한다 — 수동 재실측 대신 그 asset의 최신 run을 확인하는 편이
낫다.
...
- **I. 보존 지평 가드(G-3)** — `kor-travel-weather`의 실효 보존이 **15일 미만**이면
  **실패**한다. 외부 설정 회귀로 과거 여행 날씨가 조용히 사라지는 것을 막는 유일한
  ...
  실패해 `run_failure_sensor`(ADR-050)를 통해 통지된다. 🟡 2026-09-18 수동
  재실측: 실효 보존 약 9.6일 — 진행 중이나 이 게이트를 아직 통과하지 못한다
  (§4.2 참조).
