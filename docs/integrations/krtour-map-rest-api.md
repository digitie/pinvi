...
- **소비처 — Admin 도메인 전용 (DEC-05 확정, 2026-06-08)**: 이건 "특정 scope를 다시
  적재"하는 **운영자 액션**이지 사용자 자유 제출 큐가 아니다. 따라서 TripMate에서
  **`apps/api/app/api/v1/admin/`에서만** 호출한다 — RBAC `admin`/`operator`,
  `append_admin_audit`로 기록. 사용자 `/features/*` 경로에서 호출하지 않는다.
  `run_mode=now` 잠금 충돌 시 `LOCK_BUSY`(409, Retry-After 15) → 기본 `queued` 권장.
- **scope 매핑**: 정정/갱신 = `FeatureIdsScope{feature_ids:[id]}`, 영역 stale =
  `CenterRadiusScope`/`BboxScope`, 폐업/숨김 = `update_policy.prevent_provider_reactivation
  =true` 또는 `/admin/features/{id}/deactivate`, **신규 장소 = krtour offline-upload
  (운영자)** — 재적재로 합성 불가.
- **두 레이어 (둘 다 v1 구현)**: ① **사용자 제안 큐** = TripMate `app.feature_suggestions`
  (user 도메인) — `POST /features/requests`(즉시 201, krtour 동기 호출 X) +
  `GET /features/requests/{id}`. 감사 C-12의 미존재 테이블을 실체화. ② **Admin 검수**가
  승인 시에만 위 krtour 호출(admin 도메인). **사용자 큐는 krtour를 직접 호출하지 않는다.**
...
4. **feature-update-request(DEC-05, 확정)**: 두 레이어 — 사용자 제안 큐
   (`app.feature_suggestions`, user) + Admin 검수→krtour 재적재(admin/operator 전용 호출).
   사용자 큐는 krtour 직접 호출 X.
...
- **[H2] T-179 — Admin 검수 → krtour 재적재(DEC-05, admin 도메인)**: `/admin/feature-requests`
  목록/검수 + approve 시 운영자가 krtour `POST /tripmate/feature-update-requests`(scope) +
  `GET .../{id}` 상태조회. RBAC(admin/operator) + `append_admin_audit`. 신규 장소는
  offline-upload/백로그로 분기.
...
- DEC-05 — **확정(2026-06-08)**: 사용자 제안 큐(app) + Admin 검수→krtour 재적재(admin).
  세부는 §2.8 + `docs/decisions-needed-2026-06-06.md` DEC-05.
