...
> 추가 정정: **krtour admin API는 :9011 `/v1/admin/*`**이다 — :9012는 admin UI(Next.js)
> 이지 API가 아니므로, 본문/`config.py`의 "admin base 9012" 가정은 오류(T-180에서 정정).
...
- **Base URL**: 로컬 `http://127.0.0.1:9011` — **admin/ops API 포함 전 표면이 :9011**
  (`/v1/admin/*`·`/v1/ops/*` path로 구분, TripMate Admin 프록시 전용·사용자 경로 미노출).
  :9012는 krtour admin **UI**(Next.js)라 API base가 아니다. TripMate 설정
  `TRIPMATE_KRTOUR_MAP_API_BASE_URL`(+admin용 base 의미 재정의는 T-180).
...
- **경로 변경(krtour PR #317)**: `/tripmate/feature-update-requests*` alias **제거** →
  **`/v1/admin/feature-update-requests*`** 로 고정(admin spec — **API는 :9011**,
  :9012는 admin UI).
...
- "특정 scope를 다시 적재(Dagster job)"하는 **krtour-map 운영자 기능**. krtour-map admin
  콘솔(UI :9012)에서 운영. **TripMate 일반 사용자 비노출, 사용자 제안 흐름과 무관.**
...
**API base는 :9011 `/v1/admin/*`**, 9012 아님). 사용자 제안 승인 시 호출.
...
- admin 도메인 feature change client는 미구현(T-180, PR #317로 대상 API 생김).
  **base는 API 9011 `/v1/admin/*`** — 9012는 krtour admin UI(Next.js)라 API base로
  쓰면 안 됨 (`tripmate_krtour_map_admin_base_url` 기본값/의미 재정의 필요).
...
4. **DEC-05 — K-15 해소(krtour PR #317)**: feature change API(`POST/PATCH/DELETE
   ...
   4. **admin 인증**: 9011 `/v1/admin/*`, 코드 인증은 krtour `admin_destructive_enabled`
...
- **[admin client] ✅ T-180 — krtour admin HTTP client(API 9011 `/v1/admin/*`)** (완료 2026-06-11):
  ...
  base = :9011 `/v1/admin/*` (9012 아님 — `tripmate_krtour_map_admin_base_url` 기본값 9012→9011 정정),
...
- **PR #317 연동 합의 대기(krtour PR #317 코멘트로 질의)**:
  ...
  4. **admin 인증**: `/v1/admin/features*`(`require_admin_destructive_enabled`, **API 9011**) 호출 토큰 방식.
...
- **계약 테스트**: `apps/api/tests/unit/test_krtour_contract.py` (CI `pytest tests/unit`에서 실행) —
  (1) user client 경로(`/v1/features/*`·`/v1/categories`) ⊆ 스냅샷 paths, (2) 매핑
  (`features.py _*_from_krtour`)이 읽는 응답 필드(FeatureSummary/ClusterSummary/
  FeatureDetailResponse/WeatherCardData/WeatherMetricOut/CategorySummary/FeatureBatchData) ⊆
  스냅샷 component schemas, (3) 로컬 전용: sibling `python-krtour-map` 스펙과 경로 집합 일치
...
- **갱신 절차** (krtour 스펙 변경 시):
  ...
  3. 실패하면 사라진/바뀐 경로·필드를 `clients/krtour_map.py` + `features.py` 매핑 +
