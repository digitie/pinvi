...
- 저장소 명령은 WSL2 Ubuntu의 ext4 원본 작업본(`~/dev/tripmate`)을 최우선으로 실행한다. Windows PowerShell에서는 `wsl.exe -e bash -lc "cd ~/dev/tripmate && ..."` 형태를 우선 사용한다.
- backend test, Docker, PostgreSQL/PostGIS, Alembic, Dagster 검증은 WSL2 Ubuntu의 ext4 원본 작업본 기준으로 실행한다. `F:\dev\tripmate`와 `/mnt/f/dev/tripmate`는 Windows 확인을 위한 export 대상으로만 사용한다.
