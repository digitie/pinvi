...
source pair가 이미 terminal pinset으로 막히는 구조를 확인했다. source revision이나 문서 merge로 이를 우회하면
CI·리뷰·one-shot을 불필요하게 소비하고 historical evidence의 설명도 흔들린다.

후속은 Docker Manager `ktdctl`의 v6 execution identity로 분리한다. canonical execution input은 v5 source
pinset, canonical Manager repository URL, trusted installer Manager revision이며, Manager revision은 user-controlled
CLI/환경값을 받지 않는다. PinVi direct Compose admission과 activation receipt는 이 execution identity를 exact
대조하도록 확장한다. v5 terminal evidence는 legacy audit으로 보존하고 새 v6 identity만 새 one-shot namespace를
갖는다. 문서-only merge는 즉시 병합하지만 runtime tuple/pinset을 바꾸지 않는다. raw E2E forensic은 gitignored
local 파일에서만 보관한다.
...
세션당 1회"를 사양으로 두고 §5에 폴백 체인까지 규정해 뒀는데 구현이 없었다. 양쪽 다 서울시청
고정에서 시작했고 사용자가 버튼을 눌러야만 자기 위치로 갔다. 그 사양을 이행했다.

**착수하자마자 선행 블로커가 나왔다** — `packages/api-client`가 `/users/consents`를 호출하는데
서버 라우터는 `APIRouter(prefix="/users/me")`라 실경로는 `/users/me/consents`다. 라이브에서 옛
경로는 404였고, e2e route mock이 그 경로를 가로채 결함을 가려 왔다. 즉 **웹·앱의 동의 조회/기록/
철회가 전부 죽어 있었다**. 화면은 동의를 받은 것처럼 보였지만 서버에는 아무것도 기록되지 않았다.
이걸 고치지 않으면 "동의가 있는 경우에만 자동 센터링"이 영원히 false다.

**게이트 순서를 계약으로 삼았다**(`packages/domain/src/mapCenter.ts`): 권한(프롬프트 없는 조회) →
동의 → 취득 → 국내 범위. 권한을 먼저 보는 이유는 권한 없는 사용자에게 동의 왕복조차 만들지
않기 위해서고, 실제 취득은 동의까지 확인한 뒤에만 일어난다. 자동 경로는 어떤 모달도 띄우지 않는다.

**적대적 리뷰(전문 2인 × 3각도, 제기 30건)가 차단 3건을 잡았고 전부 내가 만든 결함이었다.**
① 세션 캐시 분기가 두 게이트보다 **앞에** 있어 캐시가 잡힌 뒤에는 동의를 철회해도 계속 그 좌표로
센터링했다. ② 자동 경로가 TTL 60초 동의 캐시를 그대로 믿었고 `invalidateLocationConsent()`는
호출자가 0건인데 docblock은 "철회하면 이 함수가 잠근다"고 단언하고 있었다 — 동의→철회→60초 내
재진입이면 철회한 사용자의 좌표를 취득한다(위치정보법 §16). ③ 모바일 3초 타임아웃이 `resolved`를
세우면 effect deps가 바뀌어 cleanup이 돌고, 진행 중이던 측위 결과가 `!alive` 가드에 걸려 버려졌다.

**위생 사고도 있었다** — `git add -A apps docs packages`가 Dagster 임시 홈(`schedules.db` 11MB
포함 6개)과 추적 대상이 아닌 `uv.lock` 2개를 커밋에 넣었다. 히스토리에서 제거하고 `.gitignore`에
`.tmp_dagster_home_*/`를 추가했다. **경로를 넓게 잡은 `git add`는 스테이징 목록을 눈으로 확인한
뒤에만 커밋한다.**

리뷰의 minor 지적도 같은 PR에 반영했다: MapLibre가 프로그램 카메라 이동에서도 zoomstart 계열을
발화한다는 사실에 맞춰 사용자 조작 감지를 `originalEvent` 기준으로 정정, 모바일 버튼을 지도 mount
전까지 잠금, `docs/api/users.md` §3.1~3.3을 실제 서버 계약(PUT/DELETE, 평면 배열)으로 정정,
