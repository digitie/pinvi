...
> 이 문서는 기존 장소 중심 스키마의 배경과 장소 ETL 기준을 설명하는 참고 문서다. feature DTO/source/weather/price 저장 계약은 `python-krtour-map`의 [Feature model](https://github.com/digitie/python-krtour-map/blob/main/docs/feature-model.md)을 따른다. 아래의 `places`, `place_source_records`, `map_features` 설명은 레거시 구조 이해와 마이그레이션 맥락을 위한 기록이다.

## 목적

이 문서는 TripMate의 장소/POI 제품 도메인과 과거 장소 중심 스키마의 배경을 설명한다. 신규 feature 저장 계약은 `python-krtour-map`의 canonical 문서를 따른다.

TripMate에서 “장소/POI”는 사용자가 여행 일정에 넣고 이름, 메모, 순서, snapshot을 관리하는 제품 엔티티다. 공공데이터포털, 기상청 추천 관광코스, 한국도로공사 휴게소, OpiNet 주유소, Kakao/Naver/Google 같은 외부 provider feature는 `python-krtour-map`에서 정규화한 뒤 TripMate가 feature id로 참조한다.

사용자가 여행 일정에 넣는 방문 장소(`trip_places`)는 사용자 메모, 사용자 지정 이름, 날짜별 참여자, 순서가 붙는 여행 도메인 데이터다. 표준 지도 feature는 이 라이브러리가 아니라 `python-krtour-map`의 `features`가 기준이다. 따라서 TripMate POI snapshot과 공통 feature 저장소는 분리한다.
...
외부 데이터는 바로 TripMate 제품 테이블에 넣지 않는다. 신규 구현은 아래 흐름을 따른다.

1. provider raw/typed model은 provider library의 public API를 직접 사용해 얻는다.
2. source trace, 중복 후보, 공통 feature, weather/price context는 `python-krtour-map`에 저장한다.
3. TripMate는 사용자 저장 장소와 일정 snapshot만 제품 테이블에 저장하고 feature id를 참조한다.

이 문서 아래의 장소 중심 table 설명은 레거시 구조 이해와 마이그레이션 맥락을 위한 기록이다.
...
사용자 검색과 지도에 노출되는 canonical 장소 feature는 가능한 한 법정동코드와 도로명코드까지 해석된 상태여야 한다. 다만 공공데이터 중 도로명코드가 없거나 provider 주소가 불완전한 경우를 잃지 않기 위해 `python-krtour-map` feature 주소 필드는 nullable로 두고, TripMate POI snapshot은 저장 당시 문자열과 좌표를 보존한다.

기상청 전국 해수욕장 카탈로그는 장소 원천으로 취급한다. 원천 xlsx는 해수욕장명, 기상청 `beach_num`, DFS 격자(`nx`, `ny`), WGS84 좌표만 제공하고 주소 문자열은 제공하지 않는다. 따라서 해수욕장은 `python-krtour-map`의 `Feature(kind='place')`와 weather context로 정규화한다. 법정동은 PostGIS point-in-polygon으로 먼저 맞추고 해상/모래사장 좌표로 인해 실패하면 약 5km 이내 가장 가까운 법정동 경계를 보조 매핑으로 사용한다. 도로명주소코드/도로명주소관리번호는 같은 법정동 내 Juso 건물명 정확 일치가 1건일 때만 채운다. 일치하지 않으면 좌표 기반 장소로 보존하고 도로명주소코드는 추정 생성하지 않는다.

추천 기본값:

- `legal_dong_code`, 좌표, 이름, 대표 카테고리가 없으면 feature 노출 대상에서 제외한다.
- `road_name_code`가 없으면 상세 주소 검색 정확도가 낮아지므로 feature detail 또는 data integrity payload에 `partial`/`coordinate_only` 상태를 남긴다.
...
- `python-krtour-map` `features`
- `python-krtour-map` `source_records`
- `python-krtour-map` `source_links`
- `python-krtour-map` `feature_overrides`
- `python-krtour-map` `dedup_review_queue`
- `python-krtour-map` `data_integrity_violations`

TripMate에는 사용자가 저장한 POI snapshot과 일정 연결만 남긴다.
...
공통 컬럼으로 승격하지 않은 장소 유형별 세부값은 `python-krtour-map` `Feature.detail` JSON에 저장한다. 예를 들어 수목원의 대표수종, 휴양림의 입장료와 주요시설, 박물관의 관람료와 휴관정보, 캠핑장의 사이트 수와 안전시설은 JSON으로 보관한다. 검색/필터/정렬에 자주 쓰게 된 필드는 후속 migration에서 이 라이브러리의 typed detail DTO 또는 별도 feature-owned table로 승격한다.
