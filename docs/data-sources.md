...
| weather | `weather_mid_term` | 여행 일정상의 단기 이후 예보 | 정책 확정, 구현 필요 |
...
- 관광코스별 상세 날씨 API를 연결할 때도 전체 관광코스를 무조건 주기 수집하지 않는다. 좁은 지역 날씨는 사용자 저장 장소 또는 여행 장소 주변 중심으로 cache/수집하고, 시도 단위 광역 날씨만 전국 단위 주기 수집 대상으로 둔다.
...
- `weather_short_term`: 초단기실황 raw/serving schema와 시군구 대표 격자 mapping은 구현됨. 후속으로 초단기예보/단기예보 endpoint, 어제/오늘 비교용 snapshot 조회, 여행 장소 좌표 기준 조회 API, Telegram 여행 알림 조립이 필요하다.
- `weather_kma_alert`: 기상특보/기상정보/기상속보 raw/serving schema와 DAG는 구현됨. 후속으로 참고문서 기반 지점코드 전체 선적재와 Telegram 메시지 필터링이 필요하다.
- `weather_mid_term`: API 승인은 확인됨. 후속으로 공식 중기예보 구역코드 선적재, `weather_mid_forecast_region`, `weather_mid_region_address_mapping`, fallback metadata, raw/serving schema, 전국 단위 주기수집 DAG 구현이 필요하다.
...
- `air_quality_sido_measurement`: 시도별 측정값 raw/serving schema와 DAG는 구현됨. 후속으로 모든 오염물질 값/등급을 조회 API에 노출하고 날씨와 함께 표시/알림 조립하는 로직이 필요하다.
...
- 관광코스 적재가 V-WORLD reverse geocoding, Juso 상세 주소 key 매핑, 좌표/명칭 후보 매칭을 수행하지 않는다는 회귀 테스트 추가
- 관광코스 상세 날씨가 전체 코스 무조건 수집이 아니라 사용자 저장 장소 또는 여행 장소 주변 중심 cache 정책을 따르는지 테스트 추가
