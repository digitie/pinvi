...
> krtour-ai-agent) 완성도·정합성 교차 검토. 전체 보고서는 krtour-map
...
> krtour-ai-agent `origin/main` `a443ca0`.
...
| D-05 (krtour ADR-050) | YouTube(krtour-ai-agent) 발 feature는 **검수 통과분만** krtour에 적재됨 | TripMate 도달분은 전부 검수 통과 — UI는 출처 표시에 집중 |
...
| 후보 | 내용 | 비고 |
|---|---|---|
| 출처 배지 UX | YouTube 발 feature(provider `krtour-ai-agent-youtube`, marker P-13)에 출처 배지 + 영상 링크/타임스탬프 카드 | krtour T-217f(evidence 노출 형태 확정) 후속 |
