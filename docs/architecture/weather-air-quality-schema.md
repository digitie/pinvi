...
feature에 붙는 날씨/환경 값은 KMA의 초단기실황, 초단기예보, 단기예보, 중기예보 스타일을 기준 timeline으로 삼는다. 다른 provider의 날씨성 데이터는 별도 중간 계층 없이 각 `python-*-api` 라이브러리 공개 client를 직접 호출하고, KMA 스타일의 `forecast_style`과 `valid_at`/`observed_at` 축에 맞춰 끼워 넣는다.

저장 대상은 `map_feature_weather_values`다.
...
`source_records`에는 provider 원문과 response hash를 보존하고, `map_feature_source_links.source_role='weather_context'`로 feature와 연결한다. provider별 cursor와 재시도 상태는 `provider_sync_state`에 둔다.
