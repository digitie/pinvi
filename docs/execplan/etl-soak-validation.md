...
- 빈 DB soak에서 Airflow scheduler/worker가 migration 전에 실행되지 않도록 `scripts/etl-soak-reset-and-start.sh` 시작 순서 조정
...
2. Postgres, Airflow metadata DB, Redis, Airflow init만 먼저 기동한다.
3. scheduler/worker를 띄우기 전에 빈 DB에 Alembic migration을 적용한다.
4. `dataset/` 하위 법정동코드 CSV와 VWorld SHP ZIP 3종을 먼저 적재한다.
5. 그 다음 Airflow webserver/scheduler/dag-processor/worker를 기동하고 DAG 전체를 수동 trigger한다.
6. 10분 단위로 `scripts/etl-soak-status.sh`를 실행한다.
7. 실패 DAG/task가 있으면 로그를 확인하고 코드/문서/스크립트를 보완한다.
8. 6시간 이후 최종 점검에서 failed/up_for_retry task, 실패 ETL log, row count, lint/typecheck/test, 관리자 페이지 표시를 확인한다.
...
- KHOA/MOF 해수욕장 DAG 여러 개가 동시에 시작하면 KMA 해수욕장 profile 공유 과정에서 unique insert 경합이 날 수 있다. profile/provider/source row는 conflict-safe insert로 유지하며, 같은 문제가 관측되면 observation/forecast/measurement serving unique에도 같은 기준을 적용한다.
