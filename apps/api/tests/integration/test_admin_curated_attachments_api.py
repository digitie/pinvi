...
def _body(**over: Any) -> dict[str, Any]:
    base = {
        ...
        "storage_key": f"curated/{uuid.uuid4().hex}.jpg",
...
async def test_plan_attachment_crud(client, session_factory, auth_cookies) -> None:  # type: ignore[no-untyped-def]
    ...
    r1 = await client.post(
        f"/admin/notice-plans/{plan_id}/attachments", json=_body(), cookies=cookies
    )
...
async def test_unknown_plan_404(client, session_factory, auth_cookies) -> None:  # type: ignore[no-untyped-def]
    ...
    missing = uuid.uuid4()
    r = await client.post(
        f"/admin/notice-plans/{missing}/attachments", json=_body(), cookies=cookies
    )
