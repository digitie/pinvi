...
krtour-map 비의존 작업 루프 기준으로 T-145 backup 핫스왑 동일호스트 schema-swap
정책을 처리한다.
...
2. **다음 비의존 후보** — T-145 backup 핫스왑 동일호스트 schema-swap 확정.
   Odroid M1S/N150 제약에서 2×DB 전제를 폐기하고 T-111 구현 정책을 먼저 정한다.
