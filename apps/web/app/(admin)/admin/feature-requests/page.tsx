...
function ReviewPanel({
  ...
  const approve = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setErr(null);
    try {
      await adminApi(apiClient).approveFeatureRequest(request.request_id, {
  ...
  const reject = async () => {
    ...
    setErr(null);
    try {
      await adminApi(apiClient).rejectFeatureRequest(request.request_id, {
        access_reason: accessReason.trim(),
      });
      onDone('제안을 거절했습니다.');
    } catch (error) {
...
export default function AdminFeatureRequestsPage() {
  const [data, setData] = useState<AdminFeatureRequestPagedResponse | null>(null);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    adminApi(apiClient)
      .listFeatureRequests({ status: statusFilter || undefined, page, limit: 50 })
      .then((res) => {
        if (cancelled) return;
        setData(res);
        setError(null);
  ...
  const columns: DataTableColumn<AdminFeatureRequestSummary>[] = [
    { key: 'type', header: '유형', cell: (r) => TYPE_LABEL[r.type] ?? r.type },
    ...
    { key: 'created_at', header: '등록', cell: (r) => formatDateTime(r.created_at) },
    {
      key: 'action',
      header: '',
      cell: (r) => (
