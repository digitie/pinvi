...
- **용도**: 통합 검색(`GET /search`, features.md §2.6)의 feature 부분. 사용자가
...
### 2.8 feature 갱신/추가 요청 — feature update request **[누락]**
- **용도**: 사용자가 "이 장소 정보가 틀렸어요/새 장소 추가" 요청(features.md §2.5,
  `POST /features/requests`) → Admin 큐 → krtour-map 적재 파이프라인.
- **TripMate 호출 형태**: 입력 `{lon, lat, name, categories[], note}` 또는 기존
  `feature_id` + 수정 제안. 출력 `{request_id, status}`. 상태 조회도 필요.
- **krtour-map 현재 상태**: **없음.** TripMate 문서는 `POST
  /admin/feature-update-requests` + `GET /admin/feature-update-requests/{id}`를
  기대하지만 krtour-map에 해당 표면/테이블이 없다. (krtour-map엔
  `ops.dedup_review_queue`는 있으나 사용자 요청 큐는 별개.)
