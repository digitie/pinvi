...
결론은 **릴리스 보류**다. N150 배포와 기본 smoke, backup snapshot, 최신 main API CI는 확인했지만,
최신 main Web CI/manual evidence, Admin live 2000/full gate, restore staging drill이 아직 닫히지
않았다.

## 대상

- 후보 SHA: `98fb3c2c0d7b7e557dc7a5598f0340d530c4def2`
- 후속 검증 SHA: `4a1b71e273cda443243618eee1df364d350ba3d4` (backup script rerun / API main CI)
- 기준 브랜치: `main`
...
| 최신 main CI                | 부분 통과 | `4a1b71e` API push CI 통과. Web main push check는 path filter로 생성되지 않아 manual evidence 필요                        |
...
- Linux manual attempt:
  - `npm run lint`: 통과
  - `npm run typecheck`: 기존 `node_modules` 상태에서 mobile React Native type/className 오류로 실패
  - `npm ci --no-audit --no-fund`: NTFS worktree에서 장시간 무출력 진행되어 중단. release evidence로
    사용하지 않는다.
...
4. 최신 main SHA에 Web main push CI를 재실행하거나 WSL ext4/N150 clean install 기반 수동 workflow 증거를
   확보한다.
