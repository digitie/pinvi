...
> **핵심 분담**: 한국 공공 API (KMA / VisitKorea / OpiNet / KREX / KHOA / MOIS /
> 산림청 / 국가유산청 / KASI / VWorld / Juso / AirKorea / MCST)의 raw 적재 +
> serving 테이블은 **`python-krtour-map` 소유**. TripMate는 라이브러리 함수 호출
> 결과만 사용 (ADR-003 / ADR-005).
...
| 일출/일몰 (KASI) | `python-kasi-api` | `python-krtour-map/docs/` |
...
- raw long-term 저장 X (라이브러리 source_records만)
- 키는 환경변수 + 마스킹
...
2. 한국 공공 API (도메인 데이터)인가? → **`python-krtour-map`에 PR**.
   본 저장소는 인덱스 cross-ref만 추가
3. 본 README 표 갱신
