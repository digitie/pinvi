...
본 문서는 TripMate가 `python-kasi-api`를 통해 한국천문연구원(KASI) data.go.kr API를
사용하는 계약이다. `python-kasi-api`는 `DATA_GO_KR_SERVICE_KEY`를 기본 서비스키로
사용한다. TripMate는 별도 `KASI_API_KEY` 같은 새 환경변수를 만들지 않는다.

## 1. 사용 API

| TripMate 기능 | `python-kasi-api` 함수 | KASI operation |
|---------------|------------------------|----------------|
...
TripMate 여행계획 POI 생성 시 한 번만 `위치별 해달 출몰시각 정보조회`
(`getLCRiseSetInfo`)를 호출한다.

흐름:

1. `POST /trips/{trip_id}/pois`가 POI를 생성한다.
2. 서비스 레이어가 POI의 `feature_snapshot.coord` 또는 krtour-map batch 조회 결과에서
   `(longitude, latitude)`를 얻는다.
3. 방문일은 `trip_days.date`를 우선 사용한다. 날짜가 아직 없으면 출몰시각 조회를
   보류하고 `status='pending_date'`로 남긴다.
...
- POI 생성 시 1회 업데이트한다.
- 별도 주기 재계산은 없다.
- POI의 날짜나 좌표가 이후 바뀌었을 때 자동 재조회할지 여부는 아직 별도 결정하지
  않았다. 현재 기본값은 "생성 시점 1회"다.
...
| `status` | `pending_date` / `pending_coord` / `success` / `failed` |
...
- [ ] `apps/etl` dependency에 `python-kasi-api` 추가가 필요한지 확인한다.
- [ ] `kasi_special_days_daily` job은 삭제 없이 upsert만 수행한다.
- [ ] POI 생성 경로는 좌표와 방문일이 있을 때 `trip_poi_rise_sets` 1회 갱신을
      enqueue한다.
