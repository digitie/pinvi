...
기상청 해수욕장 카탈로그와 `python-krtour-map` feature를 연결하는 위치 테이블이다.
...
- `beach_name`
- `map_feature_id`: `python-krtour-map` feature id
- `nx`, `ny`: 기상청 DFS 격자
...
feature에 붙는 날씨/환경 값의 DTO, DB schema, provider별 정규화 기준은 TripMate 문서에 중복 정의하지 않는다.
...
이 문서는 TripMate의 raw/serving weather table, 지역 mapping, 사용자 알림 조립, 운영 검증 범위만 다룬다. TripMate는 별도 feature weather table을 만들지 않고 `python-krtour-map`의 `WeatherValue`와 feature DB 함수를 사용한다.
