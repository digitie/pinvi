...
- `apps/api/pyproject.toml`에 `visitkorea` git commit 의존성을 추가한다.
- `app.core.kto`에는 `KrTourApiClient`와 `TourApiHubClient`를 생성하는 설정 함수만 둔다.
- `tests/test_kto_visitkorea.py`에서 TripMate가 visitkorea client와 Pydantic 응답 모델을 직접 사용한다는 계약을 검증한다.
...
## visitkorea 저장소에 줄 추가 Codex 지시문
...
- offline fake response tests, docs, README 예시를 추가하고 `python -m pytest`, `ruff check .`, `mypy visitkorea`를 통과시켜.
```
...
- WSL2에서 `uv run mypy .`
- WSL2에서 `uv run pytest tests/test_kto_visitkorea.py`

## 남은 위험

- `visitkorea`는 GPL-3.0-or-later이므로 TripMate 배포 라이선스와 양립성을 확인해야 한다.
