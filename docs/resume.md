...
krtour-map 비의존 작업 루프 기준으로 T-134 `POST /auth/refresh` + `user_sessions`
영속화를 처리한다.
...
2. **다음 비의존 후보** — T-134 `POST /auth/refresh` + `user_sessions` 영속화.
   로그인/OAuth 계정 매칭 흐름과 직접 연결되며, krtour-map live feature read와
   독립적으로 진행할 수 있다.
