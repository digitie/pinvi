...
```bash
wsl.exe -e bash -lc "cd ~/dev/tripmate/apps/api && . .venv-wsl/bin/activate && pip install -e ."
```
...
```bash
wsl.exe -e bash -lc "cd ~/dev/tripmate/apps/api && . .venv-wsl/bin/activate && pytest -q tests/test_kto_visitkorea.py"
```

전체 backend 검사:

```bash
wsl.exe -e bash -lc "cd ~/dev/tripmate/apps/api && . .venv-wsl/bin/activate && ruff check ."
wsl.exe -e bash -lc "cd ~/dev/tripmate/apps/api && . .venv-wsl/bin/activate && ruff format --check ."
wsl.exe -e bash -lc "cd ~/dev/tripmate/apps/api && . .venv-wsl/bin/activate && mypy ."
wsl.exe -e bash -lc "cd ~/dev/tripmate/apps/api && . .venv-wsl/bin/activate && pytest -q"
```
...
```bash
wsl.exe -e bash -lc "cd ~/dev/tripmate/apps/api && . .venv-wsl/bin/activate && python - <<'PY'
...
- `TourApiParseError`: TourAPI 응답 schema drift 가능성이 있으므로 raw payload를 보존하고 `visitkorea` 이슈로 upstream한다.
...
1. `visitkorea`에서 필요한 기능을 먼저 구현하거나 commit을 확인한다.
2. `apps/api/pyproject.toml`의 `visitkorea` commit을 갱신한다.
3. `tests/test_kto_visitkorea.py`에서 새 공개 계약을 검증한다.
4. `docs/api/kto-tourapi.md`, `docs/data-sources.md`, `docs/decisions/20260506-visitkorea-client-boundary.md`를 갱신한다.
5. WSL2에서 검증 명령을 실행한다.

## 주의 사항

- `visitkorea`는 GPL-3.0-or-later다. 배포 정책 확정 전에 라이선스 영향을 별도 확인한다.
- `TarRlteTarService1` 결과는 `build_kto_hub_client().related_tour` typed helper를 우선 사용한다. 아직 typed helper가 없는 TourAPI 서비스가 필요하면 TripMate에 래퍼를 만들지 말고 `visitkorea`에 upstream한다.
