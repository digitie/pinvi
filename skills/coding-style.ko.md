...
- FastAPI router는 얇게 유지하고, 규칙은 service/loader/adapter 계층으로 둔다.
- SQLAlchemy 모델, Alembic migration, 테스트 fixture의 제약 이름과 타입을 서로 맞춘다.
- 외부 API adapter에는 timeout, retry, quota, 에러 분류를 고려한다.
...
- 저장소 명령은 WSL2 Ubuntu를 최우선으로 실행한다. Windows PowerShell에서는 `wsl.exe -e bash -lc "cd /mnt/f/dev/mapplan && ..."` 형태를 우선 사용한다.
- backend test, Docker, PostgreSQL/PostGIS, Alembic, Airflow 검증은 WSL2 Ubuntu 기준으로 실행한다.
