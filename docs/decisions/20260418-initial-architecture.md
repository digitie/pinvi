...
저장소는 Next.js 단일 앱으로 시작했지만, 제품 요구는 웹앱, FastAPI 백엔드, PostGIS, Airflow, 배포 스크립트까지 포함한다. 또한 기존 일반 여행 앱 brief와 달리 현재 제품 범위는 대한민국 국내 여행으로 한정된다.

## 결정

- 루트는 npm workspaces 진입점으로 둔다.
- 실행 가능한 Next.js 앱은 `apps/web`로 이동한다.
- 향후 백엔드는 `apps/api`, 공용 계약은 `packages/shared`, ETL은 `dags`, 운영 설정은 `infra`, 스크립트는 `scripts`에 둔다.
- 제품 1차 범위는 대한민국 국내 여행으로 고정한다.
- 비회원 모드는 MVP에서 제외한다.

## 대안

- 루트 Next.js 앱 유지: 초기에는 단순하지만 백엔드, Airflow, shared package 추가 시 경계가 흐려진다.
