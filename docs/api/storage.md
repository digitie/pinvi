...
- 라이브러리는 feature 미디어(`feature-media/` prefix)에 별도 적재. TripMate는
...
| 환경변수 | 예시 | 비고 |
|---------|------|------|
| `TRIPMATE_RUSTFS_ENDPOINT_URL` | `http://rustfs:12101` | API 컨테이너 → RustFS 내부 |
| `TRIPMATE_RUSTFS_PUBLIC_ENDPOINT_URL` | `http://127.0.0.1:12101` | 브라우저 → RustFS (presigned host) |
...
| `TRIPMATE_RUSTFS_PUBLIC_BASE_URL` | (선택) | CDN base URL → `public_url` 응답에 |
...
```jsonc
{
  "data": {
    ...
    "upload_url": "http://127.0.0.1:12101/tripmate-media/user-uploads/...?X-Amz-Signature=...",
    "headers": {
      "Content-Type": "image/jpeg"
    },
    "expires_at": "2026-05-25T15:00:00+09:00",
    "max_upload_bytes": 10485760,
    "public_url": null   // TRIPMATE_RUSTFS_PUBLIC_BASE_URL 설정 시 채움
  }
}
```
...
- [ ] python-krtour-map과 RustFS 공유 시 한 쪽 compose만 실행 가이드
