...
- 지도를 열면 **단말기 위치로 중심점을 잡는다**(웹·앱). 위치 동의와 브라우저/OS 권한이 **이미
  있는 경우에만** 동작하며, 지도에 들어왔다는 이유만으로 권한 프롬프트나 동의 창을 띄우지 않는다.
  동의가 없으면 지금까지처럼 기본 위치(서울)에서 시작하고, "내 위치" 버튼은 그대로 쓸 수 있다.
  현재 위치가 국내 서비스 범위 밖이면 센터링하지 않고 이유를 알린다.
- **(fix)** 위치 동의 저장·조회·철회가 실제로 동작한다. 클라이언트가 서버에 없는 경로
  (`/users/consents`)를 호출해 모든 동의 요청이 404로 실패하고 있었다 — 화면은 동의를 받은 것처럼
  보였지만 서버에는 아무것도 기록되지 않았다. 이제 동의 관리 화면의 기록·철회가 서버에 반영된다.
...
- 날짜가 있는 여행 생성 시 기간 전체의 일자 레이어를 자동 생성한다. 여행 상세에서 일자를 다시 추가할
  때는 삭제로 비어 있는 가장 빠른 일자를 먼저 채우고, 일자 설정 팝업에서 날짜를 수정할 수 있다.
...
- 여행 상세 Day Plan의 POI별 단건 날씨 요청을 제거하고, 같은 여행 날짜의 feature를 최대
  ...
  이름 변경·삭제와 파일 업로드는 아이콘 버튼으로 정리했다. 파일 목록은 기본 접힘 상태로 렌더링하며,
  여행 상세 지도 마커는 크기를 줄인 상태에서도 maki 아이콘이 보이도록 렌더링한다.
- 여행 상세 Day Plan에서 날짜가 없는 여행도 패널 내부 `일자 추가` 버튼으로 새 일자를 만들 수 있다.
  Day Plan 안에서 날짜별/장소별 파일 업로드·목록을 바로 처리하고, 날짜와 맞는 현재 날씨·예보·미세먼지
  정보가 있을 때만 함께 표시한다. 모바일에서는 장소 선택 후 지도 화면으로 돌아오며, Day Plan 카드
  테두리와 여백을 줄였다.
...
- Admin `/admin/rbac`를 추가했다. role별 permission matrix를 확인할 수 있고, 사용자 상세의
  ...
  회수를 차단한다.
- Admin 사용자 상세에 lifecycle 패널을 추가했다. 운영자는 세션 목록(IP hash만 노출), 세션 강제
