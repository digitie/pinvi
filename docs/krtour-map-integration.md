...
> **경계 (ADR-025)**: 본 문서는 **Feature 데이터**(place/event/weather/price/
> route/area) 경로만 다룬다. **Geocoding/주소/행정구역**(geocode/reverse/search/
> region)은 krtour-map을 경유하지 않고 **`kraddr-geo` v2 REST API를 직접 HTTP
> 호출**한다 → `docs/integrations/kraddr-geo.md`. krtour-map이 자기 ETL 적재 시
> kraddr-geo v2를 쓰는 것(그쪽 ADR-006)은 라이브러리 내부 책임으로, TripMate
> 사용자 경로와 무관하다. 즉 `kraddr_geo_client` 주입은 krtour-map의 **적재
> (write)** 보강용이고, 사용자 대면 **조회(read)** geocoding은 별도 HTTP 경계다.
