...
| `app.ktm_cache_target_canary_runs`       | production causal canary의 실행별 crash-safe 감사 정본          | run UUID PK, 고정 synthetic target FK, running target partial unique, run별 command/event FK, phase/status와 baseline/put/final cursor·cache generation·count·Merkle all-or-none CHECK |
