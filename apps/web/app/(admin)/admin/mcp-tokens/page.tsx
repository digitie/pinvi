...
export default function AdminMcpTokensPage() {
  ...
  return (
    <AdminPage title="MCP 토큰" description="외부 agent read-only 토큰 관리">
      <FilterBar>
        <form onSubmit={onSearch} className="flex min-w-0 flex-1 flex-wrap items-center gap-2">
          <input
            ...
            aria-label="검색"
          />
          <button type="submit" className="rounded-sm border border-hairline px-3 py-1 text-sm">
            조회
          </button>
        </form>
        <select
      ...
      <Section title="대리 발급">
        <form onSubmit={onIssue} className="grid gap-3 lg:grid-cols-[1.2fr_1fr_120px_1.2fr_auto]">
          <input
            type="text"
            value={userId}
