...
│  app.rate_limit_overrides (Admin abuse TTL block/allow)           │
│                                                                  │
...
| `feature_id`               | `text`                          | nullable. `feature.features.feature_id` 참조 (제약 없음)   |
...
- 좌표는 본 schema에 정규 컬럼으로 갖지 않는다. feature-backed POI는 `feature_id`로
  참조하고, 자유 POI는 `feature_snapshot`에 표시용 이름/좌표 snapshot을 둘 수 있다.
...
- 6개월 retention (Dagster dry-run + `/admin/retention` kill-switch execute)
...
| `last_provider_event_id`, `last_provider_event_at` | queue 상태를 마지막으로 바꾼 Resend event                                                                  |
| `bounce_type`                                      | `hard` / `soft`                                                                                            |
| `attempts`, `last_error`                           |                                                                                                            |

### 8.4 `app.email_suppressions`

Resend hard bounce / complaint / provider suppression 및 manual suppression source.
원문 이메일은 저장하지 않고 normalized email SHA-256 hash만 저장한다.

| 컬럼                                                   | 비고                                                           |
| ------------------------------------------------------ | -------------------------------------------------------------- |
| `suppression_id` (uuid PK)                             |                                                                |
| `email_hash`                                           | normalized email SHA-256. unique                               |
| `reason`                                               | `hard_bounce` / `complaint` / `provider_suppressed` / `manual` |
| `source`                                               | `resend` / `admin`                                             |
| `provider_event_id`                                    | Resend event id                                                |
| `first_seen_at`, `last_seen_at`                        | 최초/최근 감지 시각                                            |
| `released_at`, `released_by_user_id`, `release_reason` | 해제 audit. active suppression은 `released_at IS NULL`         |

### 8.5 `app.resend_webhook_events`

Resend/Svix webhook at-least-once delivery에 대한 idempotency ledger.

| 컬럼               | 비고                                     |
| ------------------ | ---------------------------------------- |
| `event_id` (PK)    | Resend event id 또는 `svix-id`           |
| `svix_id`          | Svix delivery id. unique                 |
| `event_type`       | `email.delivered`, `email.bounced`, ...  |
| `entity_ref`       | `app.email_queue.email_id`               |
| `resend_email_id`  | Resend email id                          |
| `event_created_at` | provider event time                      |
| `payload_summary`  | raw payload 대신 bounded metadata만 저장 |
| `processed_at`     | Pinvi 처리 시각                          |

### 8.6 `app.api_call_log` (SPEC V8 M-6)
...
### 8.7 `app.feature_suggestions` (SPEC V8 H-6 / DEC-05)
...
### 8.8 `app.category_mappings` (SPEC V8 I-6 / M-2, ADR-052)

category taxonomy와 upstream `maki_icon` 정본은 `kor-travel-map` `/v1/categories`다.
Pinvi는 category tree를 복제하지 않고, Admin UX용 presentation override만 저장한다.

| 컬럼                       | 비고                           |
| -------------------------- | ------------------------------ |
| `category_key` (PK)        | `kor-travel-map` category code |
| `display_name_ko`          | Pinvi 표시명 override          |
| `marker_color`             | `P-01` ~ `P-16` override       |
| `marker_icon`              | maki icon override             |
| `created_by_user_id`       | `app.users` FK, 삭제 시 NULL   |
| `updated_by_user_id`       | `app.users` FK, 삭제 시 NULL   |
| `created_at`, `updated_at` | override row lifecycle         |

`PATCH /admin/category-mappings/{category_key}`와 rollback `DELETE`는 `admin` 전용이며
`access_reason`과 `admin_audit_log` 기록을 필수로 한다. `operator`는 merge된 catalog 조회만 가능하다.
rollback은 row 삭제이며 upstream taxonomy에는 mutation을 보내지 않는다.

### 8.9 `app.data_integrity_violations` (SPEC V8 M-11)
