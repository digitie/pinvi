...
- React Native는 **New Architecture 기준**이다. `app.json`의 `newArchEnabled: true`를 유지한다.
- Android는 **`minSdkVersion` 23 이상**을 기준으로 한다. Expo managed/CNG 흐름에서
  `expo-build-properties` config plugin으로 `android.minSdkVersion = 23`을 설정한다.
...
- `apps/mobile/app.json`은 `expo-dev-client`, `expo-build-properties`,
  `newArchEnabled: true`, VWorld server-issued 설정을 함께 갖는다.
- Sprint M-1에서 workspace 등록과 lockfile 갱신을 하더라도 Expo Go로 후퇴하지 않는다.

### 후속

- Sprint M-1 활성화 시 WSL ext4 미러에서 `npm install`과
  `npm --workspace @pinvi/mobile exec -- expo install --check`를 실행해 Expo SDK 53 호환
  버전을 lockfile에 반영한다.
