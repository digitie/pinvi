...
- raw 레이어는 원본 SHP의 EPSG:5186 geometry를 그대로 적재한다.
...
- 원본 비교, SHP 갱신 검증, 재처리는 EPSG:5186 raw 레이어를 기준으로 수행한다.

## 대안

- EPSG:4326 변환본만 저장: 앱 조회는 단순해지지만 원본 비교와 갱신 검증이 어려워진다.
- 모든 조회를 EPSG:5186으로 처리: 원본 보존에는 좋지만 웹 지도/API 응답과의 변환 경계가 흐려진다.
...
- 행정구역 ETL DAG에 원본 적재, 변환, 품질 검증 단계를 둔다.
