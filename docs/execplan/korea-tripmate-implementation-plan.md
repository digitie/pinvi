...
- provider callback 성공 후 기존 `tripmate_access`, `tripmate_refresh` cookie를 발급한다.
- provider token 원문을 DB/로그에 저장하지 않는 테스트를 추가한다.
- 기존 이메일 계정과 provider 계정 자동 연결 방지 정책을 테스트한다.
- 이메일 인증 token과 인증 메일 발송 흐름을 만든다.
- httpOnly cookie 기반 JWT access/refresh token을 구현한다.
- access/refresh cookie에는 JWT를 저장하고, DB에는 refresh token hash와 만료 시각을 저장한다.
- 비밀번호 해시 정책을 문서화한다.
