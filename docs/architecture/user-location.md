...
| 사용처                                                            | 정확도 요구        | 빈도             |
| ----------------------------------------------------------------- | ------------------ | ---------------- |
| 지도 초기 중심점 (앱 진입 시, T-325 구현됨)                       | 시군구 수준 (~1km) | 세션당 1회       |
...
- 지도 초기 중심점 (`<VWorldMap center={...} />` prop 갱신 — `vworld-map-web` 선언형)
...
2) 동의 했지만 OS/브라우저 권한 거부
  ...
  → fallback B: 사용자 프로필의 거주 시군구 사용 (동의 시에만 채워짐) — **미구현**
  → fallback C: 서울 시청 (기본 중심)

3) 권한 허용했지만 위치 획득 실패 (TIMEOUT, POSITION_UNAVAILABLE)
  → **버튼 경로만** 5초 후 1회 재시도. 진입 시 자동 센터링은 재시도하지 않는다 —
    뒤늦게 카메라를 빼앗는 편이 사용자에게 더 나쁘고, 버튼은 언제든 쓸 수 있다(T-325)
...
- **권한 prompt는 사용자의 명시 액션에서만** — "내 위치" 버튼 클릭이 그것이다.
  **지도 진입은 프롬프트 트리거가 아니다**(T-325): 진입 시 자동 센터링은 브라우저/OS 권한이
  **이미 `granted`인 경우에만** 좌표를 취득하고, 권한이 미결정(`prompt`)이면 아무 것도 하지 않는다.
  권한 상태는 프롬프트를 띄우지 않는 조회(웹 Permissions API / `getForegroundPermissionsAsync`)로만
  읽으며, 조회 수단이 없으면 `prompt`로 간주해 자동 취득을 포기한다
