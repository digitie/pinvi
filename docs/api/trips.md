...
공지 plan/POI에 첨부된 파일은 복사 시 사용자 여행 plan/POI 첨부로 함께 복사된다. RustFS object 본문은
복제하지 않고 같은 `storage_key`를 참조하며, 복사 row는 `source_attachment_id`로 원본 공지 첨부를 가리킨다.
...
사용자 여행 plan과 POI에는 RustFS 파일을 첨부할 수 있다. 업로드는 두 단계다.

1. `POST /storage/upload-urls`로 presigned PUT URL을 만든다.
2. 브라우저가 RustFS에 파일을 직접 PUT한 뒤 아래 endpoint에 메타데이터를 등록한다.
...
```json
{
  ...
  "original_filename": "답사자료.pdf",
  "content_type": "application/pdf",
  "byte_size": 204800,
  "public_url": null,
  "role": "document",
  "description": "현장 확인 자료",
  "sort_order": 0
}
```
...
```json
{
  ...
  "bucket": "tripmate-media",
  "storage_key": "user-uploads/plan_attachment/.../guide.pdf",
  "original_filename": "답사자료.pdf",
  "content_type": "application/pdf",
  "byte_size": 204800,
  "public_url": null,
  ...
  "updated_at": "2026-05-22T09:00:00+09:00"
}
```

인가:

- 여행 소유자(`trips.user_id`), 여행 leader(`trips.leader_id`), 관리자가 첨부를 만들거나 삭제할 수 있다.
- 첨부 `DELETE`는 DB 연결만 soft delete 한다. RustFS object hard delete는 관리자 파일 관리 UI/API에서 별도 수행한다.
