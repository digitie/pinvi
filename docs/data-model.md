...
### 8.8 Category mapping 운영 뷰 (SPEC V8 I-6 / M-2)

현재 구현(T-213)은 `app.category_mappings` 테이블을 만들지 않는다. category taxonomy와
`maki_icon` 정본은 `kor-travel-map` `/v1/categories`이며, Pinvi Admin
`/admin/category-mapping`은 read-only 운영 뷰로 upstream catalog와 Pinvi 16색 fallback
preview/drift만 보여준다.

Pinvi-owned override 저장소가 필요하다는 제품 결정이 확정되면 별도 ADR과 migration으로
`app.category_mappings`를 도입한다. 그 경우 최소 컬럼 후보는 아래와 같다:
