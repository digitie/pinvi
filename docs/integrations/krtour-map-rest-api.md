...
**(A) `POST /tripmate/feature-update-requests` = krtour-map Admin 재적재 — TripMate 제품 무관**
- body `FeatureUpdateRequestCreateRequest`(scope/run_mode/providers/update_policy/…),
  200 `FeatureUpdateRequestRecord`(request_id/state/status_url/dagster_run_id/…).
- "특정 scope를 다시 적재(Dagster job)"하는 **krtour-map 운영자 기능**이다. krtour-map admin
  (포트 9012 / admin 콘솔)에서 운영한다. **TripMate 일반 사용자에게 노출되지 않고, 사용자
  제안 흐름에도 들어가지 않는다.** TripMate 제품은 이 엔드포인트를 surface하지 않는다.

**(B) 사용자 feature 제안 = TripMate 소유 → 승인 시 krtour "feature 추가 API"로 추가**
...
- ② **TripMate Admin 검사/승인/거절** (admin 도메인): `/admin/feature-requests` +
  approve/reject(RBAC admin/operator + audit). **승인 시 krtour feature 추가 API 호출.**
- **⚠️ krtour 신규 구축 필요**: feature **추가** API가 **현재 없다**. krtour add 표면은
  `POST /admin/offline-uploads`(파일 CSV/TSV → load)뿐이고 단건 add REST가 없다
  (`/admin/features`는 list + deactivate만). → krtour가 **단건 feature add API**(예
  `POST /admin/features` 또는 `/tripmate/features`)를 새로 만들어야 한다
  (`docs/krtour-map-requirements.md` **K-15**). T-179는 이 API에 의존.
...
4. **DEC-05 확정 + krtour 신규 API 필요**: 재적재(feature-update-request)는 krtour-map admin
   기능(TripMate 무관). 사용자 제안 = TripMate(`app.feature_suggestions`) → Admin 승인 →
   **krtour 단건 feature 추가 API**로 추가. 그 add API가 **현재 없어 krtour가 신규 구축**해야
   한다(**K-15**) → T-179가 이에 의존.
...
- **[H2] T-179 — Admin 검사/승인 → krtour feature 추가(DEC-05, admin 도메인)**:
  `/admin/feature-requests` 목록/검사 + `approve`/`reject`(RBAC admin/operator + audit).
  승인 시 **krtour 단건 feature 추가 API** 호출(현재 미존재 → **K-15 krtour 신규 구축 의존**).
  ⚠️ 재적재(feature-update-request)와 **무관** — 재적재는 krtour-map admin 기능으로 TripMate
  제품이 surface하지 않는다.
...
- DEC-05 — **확정(2026-06-08)**: 재적재(krtour admin, 비노출)와 사용자 제안 완전 분리.
  사용자 제안=TripMate→Admin 승인→**krtour 단건 feature 추가 API**(현재 없음, **K-15 krtour
  신규 구축** 필요). 세부 §2.8 + `decisions-needed-2026-06-06.md` DEC-05 +
  `krtour-map-requirements.md` K-15.
