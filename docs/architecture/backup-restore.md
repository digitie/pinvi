...
핫스왑 cut-over는 `POST /admin/backup/restore-hotswap`과
`scripts/restore-hotswap.sh`가 담당한다.
...
2. precheck
   ...
   - backup/restore 동시 실행 없음 (`pg_try_advisory_lock` + API worker lock)
   ↓
3. pg_restore → 임시 schema
   - schema name: app_restore_YYYYMMDDHHMMSS
   - `scripts/restore-hotswap.sh`가 custom dump의 app schema를 restore schema로 remap
...
5. cut-over:
   ...
   - API/Web 연결 종료는 운영자/오케스트레이터가 API 호출 밖에서 수행
...
- API-triggered restore는 swap 전 `backup.restore_hotswap_started`를 현재 canonical chain에
  먼저 commit하고, swap 성공 후 `backup.restore_hotswap` reflection을
  `app_previous_<restore_id>.admin_audit_log`에 append한다. 오래된 snapshot에 현재 admin
