...
- `tasks.md`에는 완료 항목, 머지 결과, 검증 명령, 장문 계획, 새 task 착수 체크리스트를 두지
  않는다. 반복 적용되는 계획은 본 파일에, 이번 작업의 구체 범위는 `resume.md` 또는
  `journal.md`에 둔다.
- 보류 task는 열린 항목으로 유지하되 보류 사유를 한 줄로만 남긴다.
...
- `tasks.md`의 `현재 선점 / 충돌 회피`는 활성 선점, 열린 충돌 PR, 같은 파일/도메인 충돌 가능성이
  있는 task만 적는다. PR이 머지됐거나 해당 task가 완료되면 다음 task 진입 전 즉시 제거한다.
