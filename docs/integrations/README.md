...
| [krtour-map-rest-api.md](./krtour-map-rest-api.md) | **krtour-map REST API 계약** — feature 데이터 OpenAPI HTTP(포트 12301, ADR-026/027) + TripMate 연결 작업 | 4 |
...
- 사용자별 AI provider 키는 `tripmate-ai-companion`이 소유한다. 본 저장소가
...
4. `apps/api/app/services/<service>.py` 구현 (wrapper 금지 — 직접 사용)
   - AI provider 통합이면 본 저장소에 provider client를 만들지 말고
     `tripmate-ai-companion` repo의 HTTP/MCP 계약을 먼저 정의한다.
