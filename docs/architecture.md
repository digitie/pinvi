...
┌────────────────────────────────────────────────────────────────────────┐
│ Browser / PWA                                                          │
│   apps/web (Next.js App Router + React 18 + TanStack Query + zustand)  │
...
자세한 운영 절차는 코드 작성 단계 진입 후 `docs/runbooks/odroid-docker.md` 작성.

## 6. 보안 / 권한

- 사용자 세션: HttpOnly cookie + CSRF (또는 JWT — 코드 작성 단계 ADR로 결정).
...
1. WSL ext4 미러 `~/tripmate-workspaces/tripmate`에서 git/test/docker 실행
2. `dataset/`, `refdocs/`는 NTFS에 두고 ext4에 심볼릭 링크 또는 직접 참조
3. `python-krtour-map`은 git URL pin + editable install (개발 중에는 sibling
   checkout으로 `uv pip install -e ../python-krtour-map`)
4. 명령 전후로 NTFS ↔ WSL 미러 동기
...
- **WSL ext4 표기 통일** — v1의 ext4 직접 작업본과 NTFS export 표현을 v2에서는
  **WSL 미러** 단일 모델로 단순화. 자세히는 `dev-environment.md`.
