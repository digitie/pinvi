...
행정구역(시도/시군구/법정동) 조회 — `python-kraddr-geo`가 데이터 소유(도로명주소
전자지도 SHP 기반).

> **변경 (ADR-025)**: region/주소 조회는 **`kraddr-geo` v2 REST API 직접 호출**
> 로 구현한다(이전: krtour-map 함수 경유 — 폐기). endpoint 경로/응답 셰입은 유지
> 하되 **내부 구현만 v2 REST**(`docs/architecture/geocoding-open-decisions.md` D1).
> 통합 계약은 `docs/integrations/kraddr-geo.md`가 권위. geocoding 신규 기능
...
## 2. kraddr-geo v2 REST 호출 (ADR-025)

krtour-map 함수 호출이 아니라 **`kraddr-geo` v2 REST를 직접** 부른다
(`docs/integrations/kraddr-geo.md` §5 `GeocodingClient`).
...
- `within-radius` → v2에 1:1 대응 없음. **잠정 보류**
  (`docs/architecture/geocoding-open-decisions.md` D5). 필요 확정 시 kraddr-geo에
  신규 endpoint 요청.

PostGIS 공간 인덱스는 kraddr-geo 서비스 책임 — TripMate는 v2 응답만 신뢰
...
- [ ] `apps/api/app/services/region_view.py` — `GeocodingClient`(kraddr-geo v2) 호출
      (`docs/integrations/kraddr-geo.md` §5)
...
- [ ] Rate limit 적용
- [ ] 통합 테스트 — `httpx.MockTransport`로 v2 응답 stub (실 kraddr-geo 의존 금지)
