...
`pipa.md` §4.3, `privacy-policy.md` §4) 재작성 + `TripWeatherSummary.tsx`
출처(provider) 표시 배지. `docs/tasks.md` 게이트 G-2가 "✅ 해소됨"으로
갱신됐다 — **T-365는 이제 G-1(KMA 커버리지)·G-3(보존 연장) 두 외부 게이트만
남는다.**

다음에 이 영역을 만질 사람이 알아야 할 것:
- 국외 상용 provider(OpenWeatherMap/WeatherAPI.com/Open-Meteo/wttr.in)의
  정확한 소재국은 확인하지 않고 **[변호사 검토 필요]**로 남겨 뒀다 — 프로젝트의
  기존 관례(단정하지 않기)를 그대로 따른 것이며, 실제 국외 이전 고지 문구
  확정 전 반드시 법무 검토가 필요하다.
- T-365/T-367은 여전히 외부 게이트(G-1 KMA 커버리지, G-3 보존 연장) 대기 —
  지금 코드로 진행할 수 없다. **단, T-367(보존 지평 가드)은 Pinvi 소관이고
  게이트와 무관하게 지금 바로 구현 가능하다** — `kor-travel-weather` 실효
  보존이 15일 미만이면 실패하는 점검이며, 배포 전(현재 실효 2일)에는 항상
  실패하는 게 의도된 동작이다.
...
**다음 한 작업**: T-367(보존 지평 가드) — `kor-travel-weather` 실효 보존
horizon을 조회/점검해 15일 미만이면 실패하는 체크를 추가한다. 순수 Pinvi
소관, 게이트 무관, 지금 바로 착수 가능.
...
`6269138f…`은 구조화된 journal/manifest를 남기지 못한 pre-journal 단회 시도로 보존하며, 원문 stderr는 읽지 않는다.
...
리뷰가 밝혔다 — vitest는 unhandled error로 실패시키고, 실제 문제는 요약 줄의 과소 집계다. 오인의
원인은 `bash -lc "...; echo $?"` 하네스가 바깥 셸의 `$?`를 치환해 항상 0을 보고한 것이었다. 리포터
가드는 철회하고, 조사 중 드러난 진짜 구멍만 고쳤다: `packages/{domain,schemas}`의 104테스트가 CI에서
한 번도 실행되지 않던 것을 루트 `npm test`로 교체해 보호 범위에 넣었다.

**다음 한 작업**: T-273(v1.0.0 E2E/Live gate) — 남은 hard blocker는 geofence 운영 설정이다.
모바일 후속(T-311/318/319/320)과 T-323(e2e를 required로 올릴지)은 별도 PR로 분리해 뒀다.
...
`20260821_0060` append-only migration, strict read/ACK transport, event UUID advisory lock local
projection과 default-off paired worker를 구현했다. final local receipt commit 전에는 Map ACK을
호출하지 않으며 partial pair 등 unsafe row는 immutable blocked attempt로만 남긴다.

**다음 한 작업**: admin evidence read UI와 curation receipt-bound/nonterminal target/race/restore
검증을 보강한다. M05 activation은 PinVi consumer/UI, N150 격리 paired live E2E, no-owner restore
drill, 전문 적대 리뷰가 모두 끝날 때까지 `false`다.
...
middleware가 생성한 응답 request ID를 audit·Map 호출에 재사용한다. 409·422·전송/5xx·응답 계약 오류는 Pinvi row/ref/audit를 보존해 재시도할 수 있다. correction/closure는
기존 admin PATCH/DELETE 자격과 경로를 유지하며, 두 client는 유형별로만 지연 해석한다.
Map success envelope의 좌표는 JSON number만 허용하고 correction marker palette는 `P-01`~`P-16`으로
동기화했다.
...
proof는 `PINVI_ENVIRONMENT=smoke` 격리 stack에서만 실행한다.

**다음 한 작업**: PinVi #444와 Map #975의 paired contract CI 및 적대적 재리뷰를 통과한
candidate만 n150 isolated Live UI E2E에 올린다. 성공 전에는 sync enable·완료 receipt·PR
병합을 하지 않으며, #967은 #975 병합 뒤 재배치한다.
...
추가하고, 별도 immutable `ktm_curation_cutover_backfill_receipts` relation으로 actor/key,
sealed mapping receipt/member, legacy feature UUID, 기존 plan, terminal generic import receipt를
한 command evidence로 결박한다. terminal trigger는 completed mapping·동일 actor의 completed
generic receipt·legacy UUID→collection mapping·plan provenance를 대조하고 active legacy source
POI가 남아 있으면 rollback한다. manual POI는 provenance 두 값이 모두 `NULL`이므로 이 guard의
삭제 대상이 아니다. cache-target final boundary schema pin도 `0059`로 forward re-pin했으며
downgrade는 fail-close한다.

**다음 한 작업**: remote canonical collection snapshot을 먼저 검증한 뒤, 같은 `SERIALIZABLE`
transaction에서 legacy preflight 재실행→backfill receipt claim→기존 plan/모든 POI lock→legacy
source POI soft delete→canonical POI/import receipt 재생성→backfill terminal seal→audit을 수행하는
typed command를 구현한다. generic import service를 caller payload로 우회하지 않고 sealed mapping
member의 collection ID와 snapshot tuple을 exact 대조해야 한다.
...
state만 별도 관리한다. paired service provenance는 Map `a345ae7549aa272d5d40ccaa0898cb6d3967add2`로 재고정했다.
...
Map `a345ae7549aa272d5d40ccaa0898cb6d3967add2`의 service OpenAPI를 byte-exact로
...
fail-close한다. 배포 가능한 `0050/0051`은 byte 불변으로 복원·고정했고, 새 forward-only
`20260814_0052`가 receipt별 exact item proof를 immutable relation으로 저장하고,
...
수가 snapshot count와 같으며 plan의 revision/ETag/hash와 실제 active POI set까지 일치할 때만 완료된다.
구 c614 bytes의 dataful `0051→0052` 실제 upgrade와 ORM 조회, direct-completed INSERT,
고아/mismatch/proof-without-POI 음성 검증을 통과했다.

Map snapshot의 무제한 문자열과 PinVi 저장 상한은 아직 paired contract로 수렴하지 않았으므로,
canonical importer를 연결하기 전에 producer bound 또는 무손실 consumer 저장 정책을 확정한다.
...
wrapper가 build/pull/up 전에 해당 image label을 대조하게 했다. 검증한 세 image ID를 Compose image
override로 고정하고 기동 뒤 container ID까지 대조하므로, tag 재지정이 label 검사와 `up` 사이에
끼어들 수 없다.
...
설계·작업 단위 정본은
[`docs/execplan/t-vn-41-f1j-contract-provenance.md`](execplan/t-vn-41-f1j-contract-provenance.md),
결정은 ADR-061, 열린 체크는 `docs/tasks.md`다.
...
**병행 조사(T-273)**: repo 측 geofence 코드/템플릿/스크립트는 완료 — 잔여는 전부 운영 적용(Cloudflare
WAF KR-only rule, edge secret header, kor-travel-docker-manager compose `PINVI_GEOFENCE_*` 주입,
`BLOCK_UNKNOWN=true` 전환, US 451 증적). map #868은 T-273 blocker 아님(T-VN/C7 트랙). **다음 한 작업**:
T-273 운영 적용은 사용자 결정 필요(Cloudflare/N150/secret) — Claude 단독 가능분은 runbook 체크리스트
보강 + docker-manager env 블록 초안 + 법무 placeholder 인덱싱.
...
독립 관찰하고 값이 바뀌면 자기 process-local cache를 전부 비운다. `/health/cache-target-sync`는
default-off/disabled reason, epoch, local/remote cursor, 실제 미ACK applied gap, pending/DLQ command,
snapshot checksum과 reconcile/error code를 credential/host/detail 없이 보여 준다.

Map correction에 따라 `cache_target.reconciled`는 target tuple이 없는 stream receipt로 검증·저장하고,
fixed snapshot checksum/high-watermark만 적용한다. target event만 POI head와 cache generation을
변경한다.

cache-target 집중 33건, 전체 unit 772건, 전체 PostGIS integration 423건(환경 의존 3건 skip), 0044
빈 스키마 downgrade/upgrade 왕복, Ruff format/check, Prettier, strict mypy(202개 source)가 통과했다.

**다음 한 작업**: 전체 local gate 뒤 final Map OpenAPI fixture/SHA와 source revision을 pin한다.
...
`found(card)|no_data|retired|suppressed|missing|unavailable|not_requested` 조합을 강제한다.
고유 날짜 최대 31개, worker 4개, view 전체 10초 budget으로 outbound를 제한하며 상한 초과는
서비스 장애와 다른 `not_requested`다. 부모 request cancellation은 worker에 전파한다. Web은 이
view만 렌더해 단건 `/features/{id}/weather` 호출을 하지 않고 weather 없음·parent lifecycle·
서비스 실패·날짜 상한 미조회를 구분한다.
