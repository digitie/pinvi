# kraddr-geo 통합 — geocoding (v2 REST API 직접 호출)

본 문서는 TripMate(`apps/api`)가 **사용자 대면 geocoding**(주소↔좌표, 검색,
행정구역)을 위해 `python-kraddr-geo`의 **v2 REST API를 직접 HTTP 호출**하는 표준
패턴이다. ADR-025 기준.
...
└───────────────────────────────────────────────────────────┘
                  ...
                  ▼
┌───────────────────────────────────────────────────────────┐
│ python-kraddr-geo REST 서비스 (별 프로세스/컨테이너)        │
│   FastAPI :8888  (운영 docker network: http://kraddr-geo:8888)│
...
- **좌표는 외부 표면에서 항상 `(lon, lat)`** (kraddr-geo `Point`는 `x=lon, y=lat`).
...
- 국가지점번호 의무지역이면 `match_kind="sppn"` 후보가 함께 올 수 있다(`address`
  없을 수 있음). TripMate region label 용도로는 `sppn`을 무시한다.

```bash
curl -X POST "$KRADDR_GEO/v2/reverse" -H 'Content-Type: application/json' \
...
```bash
curl -X POST "$KRADDR_GEO/v2/search" -H 'Content-Type: application/json' \
...
v2 candidate를 TripMate 표준 응답 `{data, meta}`(`docs/api/common.md`)로 래핑한다.
라우터: `apps/api/app/api/v1/geo.py`.

| TripMate endpoint | 호출 | 용도 |
|-------------------|------|------|
...
geocoding은 HTTP 의존이므로 krtour-map과 별개의 httpx client를 lifespan에서
1개 만들어 재사용한다.

```python
# apps/api/app/core/config.py (Settings 발췌)
tripmate_kraddr_geo_base_url: str = "http://localhost:8888"   # 운영: http://kraddr-geo:8888
tripmate_kraddr_geo_timeout_seconds: float = 3.0
tripmate_kraddr_geo_enabled: bool = True       # False면 geocoding 기능 비활성(503)
```
...
class GeocodingClient:
    """kraddr-geo v2 REST 얇은 클라이언트. httpx.AsyncClient 주입."""
    ...
    async def _post(self, path: str, body: dict) -> dict:
        ...
        if resp.status_code >= 500:
            raise GeocodingUnavailableError(f"kraddr-geo {resp.status_code}")
...
async def lifespan(app: FastAPI):
    ...
    if settings.tripmate_kraddr_geo_enabled:
        app.state.kraddr_geo_http = httpx.AsyncClient(
            base_url=settings.tripmate_kraddr_geo_base_url,
            timeout=settings.tripmate_kraddr_geo_timeout_seconds,
        )
    else:
        app.state.kraddr_geo_http = None
    try:
        yield
    finally:
        if app.state.kraddr_geo_http is not None:
            await app.state.kraddr_geo_http.aclose()
```

```python
# apps/api/app/core/deps.py (의존성)
def get_geocoding_client() -> GeocodingClient:
    http = getattr(app_state(), "kraddr_geo_http", None)
    if http is None:
...
- reverse/geocode는 좌표·주소 정규화 후 캐시 키. 좌표는 소수 5자리로 quantize해
  키 폭증 방지(역지오코딩 캐시 키, krtour-map `_coord_str` 패턴과 동일 사고).
- rate-limit은 TripMate의 `slowapi` 미들웨어 경계에서. kraddr-geo 호출 폭주를
  TripMate가 막는다(kraddr-geo는 별 서비스라 TripMate가 자기 쿼터를 책임).
...
| 상황 | TripMate 응답 |
|------|---------------|
| kraddr-geo 5xx / 네트워크 / 타임아웃 | `503 GEOCODING_UNAVAILABLE` (degrade — 지도/검색은 빈 결과로) |
| v2 `status != "OK"` 또는 candidates 빈 | `200 {data: {candidates: []}}` (정상 빈 결과) |
| 잘못된 좌표/파라미터(한국 범위 밖 등) | `422 VALIDATION_ERROR` (TripMate가 먼저 검증) |
| `tripmate_kraddr_geo_enabled=false` | `503 GEOCODING_UNAVAILABLE` |

geocoding 실패가 지도/여행 핵심 흐름을 막지 않도록 **graceful degrade**가 기본
(검색창은 "검색 일시 불가" 안내, 지도 클릭 label은 좌표만 표시).

## 10. 사용처 (TripMate 기능 매핑)
...
- [ ] `apps/api/app/core/config.py` — `tripmate_kraddr_geo_base_url` /
...
```dotenv
TRIPMATE_KRADDR_GEO_BASE_URL=http://localhost:8888   # 운영: http://kraddr-geo:8888
TRIPMATE_KRADDR_GEO_TIMEOUT_SECONDS=3.0
TRIPMATE_KRADDR_GEO_ENABLED=true
```

kraddr-geo REST 서비스의 VWorld/juso API 키·포트·재시도는 **kraddr-geo 서비스
설정**이다(TripMate 무관). TripMate는 base URL + timeout만 안다.
