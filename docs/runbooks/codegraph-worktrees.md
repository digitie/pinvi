...
```
F:/dev/pinvi              ← 사람 trunk (사용자가 직접 만지는 checkout)
  ├── (worktree)
F:/dev/pinvi-claude   ← Claude Code 전용 worktree (영속)
F:/dev/pinvi-codex    ← OpenAI Codex 전용 worktree (영속)
F:/dev/pinvi-antigravity  ← Google Antigravity 2.0 전용 worktree (영속)
```
...
- `.codegraph/` 디렉터리는 `.gitignore` — 로컬 SQLite 인덱스, 도구 / OS /
  worktree마다 별개.
- AI 도구는 **절대 trunk** (`F:/dev/pinvi`)를 직접 만지지 않는다 — 각자
  worktree만 사용.

> **환경 모델 (ADR-024)**: NTFS worktree(`F:/dev/pinvi-<agent>`)가 git source of
> truth이고 git은 Windows git(`git.exe`)으로만 다룬다. WSL ext4
> (`~/pinvi-workspaces/pinvi-<agent>`)는 의존성·테스트·docker 전용 **일회용
...
사람이 직접 만지는 trunk가 `F:/dev/pinvi` (Windows) 또는
`~/pinvi-workspaces/pinvi` (WSL)에 있다고 가정. 아니라면 `git rev-parse
--show-toplevel`로 확인.
...
- idle 브랜치는 **`agent/<agent>-idle`** (kor-travel-geo 컨벤션과 통일). 첫 dummy일 뿐,
...
```bash
cd F:/dev/pinvi-claude   # (혹은 본인 agent의 worktree)
...
```bash
cd F:/dev/pinvi-claude   # 본인 agent의 worktree
git fetch
git switch -c agent/claude-<task> origin/main
codegraph sync                  # incremental — 보통 < 30s
```

- 기준 ref는 **`origin/main`** — 로컬 `main` ref는 trunk(`F:/dev/pinvi`)가
...
**trunk(`F:/dev/pinvi`)가 항상 `main` 브랜치를 점유**한다. git worktree는
같은 브랜치를 한 위치에만 체크아웃할 수 있으므로, 다른 worktree에서
`git switch main`을 시도하면 다음으로 실패한다:

```
fatal: 'main' is already checked out at 'F:/dev/pinvi'
```
...
```bash
cd F:/dev/pinvi
git worktree remove ../pinvi-codex --force
...
/mnt/f/dev/pinvi/.git/worktrees/pinvi-codex`처럼 마운트 경로가, Windows
git에서 만들면 `gitdir: F:/dev/pinvi/.git/worktrees/...`처럼 드라이브 경로가
...
원칙(ADR-024): 각 worktree는 **한 환경 전용**으로 운용한다. Pinvi에서는 NTFS
worktree의 git을 **Windows git(`git.exe`)으로만** 다룬다. WSL에서 `/mnt/f/dev/
pinvi-<agent>`에 `git`을 실행하지 않는다(ext4 미러는 git 대상이 아니다).
...
git worktree repair F:\dev\pinvi-claude
...
`F:/dev/pinvi` (사람 trunk)는 worktree 정책 밖이다. 사용자가 직접
