...
krtour-map 비의존 작업 루프 기준으로 T-142 geofence/admin 문서·구현 정합을 처리한다.
...
2. **다음 비의존 후보** — T-142 geofence admin 우회 RBAC 소스 정정 + nginx 티어 정리.
   krtour-map feature read와 무관하게 문서/설정 경계를 맞춘다.
