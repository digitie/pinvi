...
릴리스는 보류한다. PR #295 merge 후 최신 main `4a1b71e`의 API push CI는 통과했지만 Web clean
CI/manual evidence는 아직 필요하다. N150 Playwright는 Chromium
`libatk`/`libatspi`/`libXdamage`/`libasound` 계열 누락으로 실패했고, Playwright 1.60.0
`install-deps --dry-run chromium`은 Ubuntu 26.04를 지원하지 않았다. Windows fallback은 N150 Web
SSH tunnel 대상 login malformed validation 1건만 통과했다. Admin live 2000/full은 credential
부재, restore staging drill은 staging DB URL 부재로 미실행이다. 다음 작업은 T-259 차단 해소 후
