...
- [/] **T-363** — (P4) Trip view 전환. **백엔드 완료**: batch 1회 → `/markers` 1회 + location별 `/forecast` fanout, `card_key := location_id`, 10초 예산·취소 전파. **2026-09-17 방향 전환(사용자 결정) — feature batch 파이프라인과 완전 독립**: `retired` 판정을 선행 feature batch로 이관하는 대신, weather는 POI 자신의 `feature_snapshot.coord`만 보고 feature 관리 상태(`retired`/`suppressed`/`missing`)를 아예 참조하지 않는다(`trip_weather_batch.py` 신설, `docs/integrations/kor-travel-weather.md` §3.4/§3.5 개정). **남은 일**: e2e 단언(`trip-detail.e2e.ts` 단건 0회, `PINVI_LIVE_WEATHER_*` 픽스처)과 `live-mutating-e2e.md` 게이트를 새 호출 모양으로 재정의 — Playwright는 N150 전용이라 이 개발 세션에서 미착수.
